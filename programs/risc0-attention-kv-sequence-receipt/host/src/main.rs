...
fn image_id_hex() -> String {
    ATTENTION_KV_SEQUENCE_RECEIPT_ID
        .iter()
...
fn write_summary(
    ...
    let summary = json!({
        "schema": "zkai-attention-kv-risc0-sequence-host-summary-v1",
        "mode": mode,
...
fn verify_receipt(
    ...
    receipt
        .verify(ATTENTION_KV_SEQUENCE_RECEIPT_ID)
...
fn prove(input_path: PathBuf, receipt_path: PathBuf, summary_path: PathBuf) {
    ...
    let prove_info = prover
        .prove(env, ATTENTION_KV_SEQUENCE_RECEIPT_ELF)
...
fn usage() -> ! {
    eprintln!("usage: host prove <attention-sequence-input-json> <receipt-out> <summary-out>");
    eprintln!("   or: host verify <attention-sequence-input-json> <receipt-in> <summary-out>");
    std::process::exit(2);
}
...
mod tests {
    ...
    fn expected_journal_carries_kv_state_across_three_steps() {
        ...
        assert_eq!(journal.sequence_length, 3);
        assert_eq!(journal.transitions.len(), 3);
        assert_eq!(journal.transitions[0].selected_position, 0);
        assert_eq!(journal.transitions[0].attention_output, [2, 1]);
        assert_eq!(journal.transitions[1].selected_position, 2);
        assert_eq!(journal.transitions[1].attention_output, [4, 2]);
        assert_eq!(journal.transitions[2].selected_position, 3);
        assert_eq!(journal.transitions[2].attention_output, [5, -2]);
        assert_eq!(
            journal.transitions[1].prior_kv_cache,
            journal.transitions[0].next_kv_cache
        );
        assert_eq!(
            journal.transitions[2].prior_kv_cache,
            journal.transitions[1].next_kv_cache
        );
        assert_eq!(journal.final_kv_cache, journal.transitions[2].next_kv_cache);
    }

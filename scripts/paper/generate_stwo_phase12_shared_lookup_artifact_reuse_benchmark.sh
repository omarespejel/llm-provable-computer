...
TSV_OUT="${TSV_OUT:-$REPO_ROOT/docs/paper/evidence/stwo-phase12-shared-lookup-artifact-reuse-2026-04.tsv}"
JSON_OUT="${JSON_OUT:-$REPO_ROOT/docs/paper/evidence/stwo-phase12-shared-lookup-artifact-reuse-2026-04.json}"
SVG_OUT="${SVG_OUT:-$REPO_ROOT/docs/paper/figures/stwo-phase12-shared-lookup-artifact-reuse-2026-04.svg}"
PNG_OUT="${PNG_OUT-$REPO_ROOT/docs/paper/figures/stwo-phase12-shared-lookup-artifact-reuse-2026-04.png}"
PDF_OUT="${PDF_OUT-$REPO_ROOT/docs/paper/figures/stwo-phase12-shared-lookup-artifact-reuse-2026-04.pdf}"
...
TMP_EVIDENCE_DIR="$(mktemp -d "$EVIDENCE_DIR/stwo-phase12-artifact.XXXXXX")"
TMP_JSON_DIR="$(mktemp -d "$JSON_DIR/stwo-phase12-artifact.XXXXXX")"
TMP_FIGURE_DIR="$(mktemp -d "$SVG_DIR/stwo-phase12-artifact.XXXXXX")"
...
if [[ "$CAPTURE_TIMINGS" == "1" ]]; then
  ...
  python3 scripts/paper/aggregate_stwo_phase12_shared_lookup_artifact_reuse_benchmark.py \
    --inputs "${RUN_INPUTS[@]}" \
...
python3 scripts/paper/generate_stwo_phase12_shared_lookup_artifact_reuse_figure.py "${FIGURE_ARGS[@]}"

if [[ -n "$PNG_OUT" && ! -f "$TMP_PNG" ]]; then
  echo "phase12 artifact figure generation did not produce requested PNG output: $PNG_OUT" >&2
  exit 1
fi
if [[ -n "$PDF_OUT" && ! -f "$TMP_PDF" ]]; then
  echo "phase12 artifact figure generation did not produce requested PDF output: $PDF_OUT" >&2
  exit 1
fi

...
scope: repeated-window scaling sweep plus transformer-specific fold operator plus repeated-window fold tree over Gemma-like repeated richer windows on one shared S-two proof surface
MANIFEST
...
summary_tsv.write_text(
    "artifact\tjson_bytes\n"
    f"gemma_block_v4_proof\t{proof_bytes}\n"
...
index_md.write_text(
    ...
    f"- `gemma-block-v4.stark.json`: shared execution proof surface reused across every repeated-window artifact in `{bundle_name}`.\n"

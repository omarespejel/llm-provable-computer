#!/usr/bin/env python3
"""Render an SVG figure plus optional PNG/PDF companions for the Phase30 source-bound manifest reuse benchmark."""
...
DEFAULT_TSV = (
    ...
    / "paper"
    / "evidence"
    / "stwo-phase30-source-bound-manifest-reuse-2026-04.tsv"
)
...
COLORS = {
    "shared_ordered_manifest": "#2563EB",
    "independent_single_step_manifests": "#D97706",
}
LABELS = {
    "shared_ordered_manifest": "One ordered manifest",
    "independent_single_step_manifests": "N one-step manifests",
}
EXPECTED_STEPS = [1, 2, 3]
VARIANT_ORDER = [
    "shared_ordered_manifest",
    "independent_single_step_manifests",
]
REQUIRED_COLUMNS = {
    "benchmark_version",
    "semantic_scope",
...
EXPECTED_BENCHMARK_VERSION = "stwo-phase30-source-bound-manifest-reuse-benchmark-v1"
EXPECTED_SEMANTIC_SCOPE = "phase30_source_bound_ordered_manifest_reuse_calibration"
...
def validate_rows(rows: list[dict[str, str]], *, source: Path) -> None:
    ...
    for row in rows:
        ...
        seen.add(key)
        if row["primitive"] != "phase30_decoding_step_manifest":
    ...
    expected = {
        ("phase30_decoding_step_manifest", variant, steps)
...
def build_svg(*, grouped: dict[str, list[dict[str, str]]], measured: bool, runs: int) -> str:
    subtitle = (
        f"Median of {runs} measured runs. Verification time measures source-bound Phase30 recomputation against the proof-checked Phase12 chain."
        if measured
    ...
    return "\n".join(
        [
            ...
            render_text(WIDTH / 2, 72, "Phase30 Source-Bound Manifest Reuse", size=38, weight="700"),
            render_text(
                WIDTH / 2,
                108,
                "One ordered manifest verified once against the Phase12 chain versus N one-step source-bound manifests",
                size=21,
                fill="#4B5563",
            ),
            ...
            render_panel(
                ...
                metric_label="Serialized Manifest Bytes",
                panel_x=LEFT_X,
                panel_y=TOP_Y,
            ),
            render_panel(

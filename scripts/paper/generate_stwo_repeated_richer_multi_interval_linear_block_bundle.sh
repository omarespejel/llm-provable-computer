...
BUNDLE_DIR="${BUNDLE_DIR:-$REPO_ROOT/docs/paper/artifacts/stwo-repeated-richer-multi-interval-linear-block-v1-2026-04-22}"
...
if [ "$ALLOW_DIRTY_BUNDLE_BUILD" != "1" ]; then
  if ! git diff --quiet --ignore-submodules -- || ! git diff --cached --quiet --ignore-submodules --; then
    echo "Refusing to generate repeated richer multi-interval Linear-block bundle from a dirty tracked worktree." >&2
    echo "Commit or stash tracked changes first, or set ALLOW_DIRTY_BUNDLE_BUILD=1 to override." >&2
    exit 1
  fi
  if [ -n "$(git ls-files --others --exclude-standard)" ]; then
    echo "Refusing to generate repeated richer multi-interval Linear-block bundle with unrelated untracked files present." >&2
...
SINGLE_WINDOW_JSON="$BUNDLE_DIR/single-window-multi-interval-linear-block-richer-family-accumulation.stwo.json"
REPEATED_JSON="$BUNDLE_DIR/repeated-multi-interval-linear-block-richer-family-accumulation.stwo.json"
FOLDED_REPEATED_JSON="$BUNDLE_DIR/folded-repeated-multi-interval-linear-block-accumulation-prototype.stwo.json"
RICHER_REPEATED_JSON="$BUNDLE_DIR/folded-repeated-multi-interval-linear-block-richer-family.stwo.json"
...
bundle_version: stwo-repeated-richer-multi-interval-linear-block-v1
repo_root: .
...
single_window_artifact: single-window-multi-interval-linear-block-richer-family-accumulation.stwo.json
repeated_artifact: repeated-multi-interval-linear-block-richer-family-accumulation.stwo.json
folded_repeated_artifact: folded-repeated-multi-interval-linear-block-accumulation-prototype.stwo.json
folded_richer_repeated_artifact: folded-repeated-multi-interval-linear-block-richer-family.stwo.json
...
run_timed prepare_single_window_multi_interval_linear-block_richer_family_accumulation \
  "${CARGO_STWO[@]}" \
...
run_timed verify_single_window_multi_interval_linear-block_richer_family_accumulation \
  "${CARGO_STWO[@]}" \
  verify-stwo-multi-interval-linear-block-richer-family-accumulation-artifact \
  "$SINGLE_WINDOW_JSON"

run_timed prepare_repeated_multi_interval_linear-block_richer_family_accumulation \
  "${CARGO_STWO[@]}" \
...
run_timed verify_repeated_multi_interval_linear-block_richer_family_accumulation \
  "${CARGO_STWO[@]}" \
  verify-stwo-repeated-multi-interval-linear-block-richer-family-accumulation-artifact \
  "$REPEATED_JSON"

run_timed prepare_folded_repeated_multi_interval_linear-block_accumulation_prototype \
  "${CARGO_STWO[@]}" \
...
run_timed verify_folded_repeated_multi_interval_linear-block_accumulation_prototype \
  "${CARGO_STWO[@]}" \
  verify-stwo-folded-repeated-multi-interval-linear-block-accumulation-prototype-artifact \
  "$FOLDED_REPEATED_JSON" \
  --source "$REPEATED_JSON"

run_timed prepare_folded_repeated_multi_interval_linear-block_richer_family \
  "${CARGO_STWO[@]}" \
...
run_timed verify_folded_repeated_multi_interval_linear-block_richer_family \
  "${CARGO_STWO[@]}" \
...
comparison_rows = [
    (
        ...
        "same frozen Linear-block proof reused across the repeated-window Phase105, Phase106, and Phase107 surfaces",
    ),
    (
        "provable-transformer-vm",
        "internal-artifact",
...
readme_md.write_text(
    f"# Repeated Richer Multi-Interval Linear-block Bundle\n\n"
    f"This directory freezes a publication-facing tensor-native `stwo` bundle built from:\n\n"
    f"- one `linear_block_v4_with_lookup` S-two execution proof,\n"

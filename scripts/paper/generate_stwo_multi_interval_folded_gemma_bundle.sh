...
BUNDLE_DIR="${BUNDLE_DIR:-$REPO_ROOT/docs/paper/artifacts/stwo-multi-interval-folded-gemma-v1-2026-04-21}"
...
if [ "$ALLOW_DIRTY_BUNDLE_BUILD" != "1" ]; then
  if ! git diff --quiet --ignore-submodules -- || ! git diff --cached --quiet --ignore-submodules --; then
    echo "Refusing to generate multi-interval folded Gemma bundle from a dirty tracked worktree." >&2
    echo "Commit or stash tracked changes first, or set ALLOW_DIRTY_BUNDLE_BUILD=1 to override." >&2
    exit 1
  fi
  if [ -n "$(git ls-files --others --exclude-standard)" ]; then
    echo "Refusing to generate multi-interval folded Gemma bundle with unrelated untracked files present." >&2
...
bundle_version: stwo-multi-interval-folded-gemma-v1
repo_root: .
...
(
  ...
  shasum -a 256 \
    manifest.txt \
    gemma-block-v4.stark.json \
    single-interval-repeated-gemma-slice-accumulation.stwo.json \
    single-interval-folded-gemma-slice-accumulation.stwo.json \
    single-interval-folded-gemma-richer-slice-family.stwo.json \

...
TMP_EVIDENCE_DIR="$(mktemp -d "$EVIDENCE_DIR/stwo-phase30-manifest.XXXXXX")"
TMP_JSON_DIR="$(mktemp -d "$JSON_DIR/stwo-phase30-manifest.XXXXXX")"
TMP_FIGURE_DIR="$(mktemp -d "$SVG_DIR/stwo-phase30-manifest.XXXXXX")"
...
if [[ "$CAPTURE_TIMINGS" == "1" ]]; then
  ...
  python3 scripts/paper/aggregate_stwo_phase30_source_bound_manifest_reuse_benchmark.py \
    --inputs "${RUN_INPUTS[@]}" \
...
python3 scripts/paper/generate_stwo_phase30_source_bound_manifest_reuse_figure.py "${FIGURE_ARGS[@]}"

if [[ -n "$PNG_OUT" && ! -f "$TMP_PNG" ]]; then
  echo "phase30 manifest figure generation did not produce requested PNG output: $PNG_OUT" >&2
  exit 1
fi
if [[ -n "$PDF_OUT" && ! -f "$TMP_PDF" ]]; then
  echo "phase30 manifest figure generation did not produce requested PDF output: $PDF_OUT" >&2
  exit 1
fi

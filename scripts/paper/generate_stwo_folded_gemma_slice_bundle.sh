...
BUNDLE_DIR="${BUNDLE_DIR:-$REPO_ROOT/docs/paper/artifacts/stwo-folded-gemma-slice-family-v1-2026-04-21}"
...
if [ "$ALLOW_DIRTY_BUNDLE_BUILD" != "1" ]; then
  if ! git diff --quiet --ignore-submodules -- || ! git diff --cached --quiet --ignore-submodules --; then
    echo "Refusing to generate folded Gemma-slice bundle from a dirty tracked worktree." >&2
    echo "Commit or stash tracked changes first, or set ALLOW_DIRTY_BUNDLE_BUILD=1 to override." >&2
    exit 1
  fi
  if [ -n "$(git ls-files --others --exclude-standard)" ]; then
    echo "Refusing to generate folded Gemma-slice bundle with unrelated untracked files present." >&2
...
FOLDED_ACCUM_JSON="$BUNDLE_DIR/folded-gemma-slice-accumulation.stwo.json"
FOLDED_FAMILY_JSON="$BUNDLE_DIR/folded-gemma-richer-slice-family.stwo.json"
...
bundle_version: stwo-folded-gemma-slice-family-v1
repo_root: .
...
explicit_accumulation_artifact: repeated-gemma-slice-accumulation.stwo.json
folded_accumulation_artifact: folded-gemma-slice-accumulation.stwo.json
folded_richer_family_artifact: folded-gemma-richer-slice-family.stwo.json
...
run_timed prepare_folded_gemma_slice_accumulation \
  "${CARGO_STWO[@]}" \
  prepare-stwo-folded-gemma-slice-accumulation-artifact \
  --source "$GEMMA_ACCUM_JSON" \
  -o "$FOLDED_ACCUM_JSON"

run_timed verify_folded_gemma_slice_accumulation \
  "${CARGO_STWO[@]}" \
  verify-stwo-folded-gemma-slice-accumulation-artifact \
  "$FOLDED_ACCUM_JSON" \
  --source "$GEMMA_ACCUM_JSON"

run_timed prepare_folded_gemma_richer_slice_family \
  "${CARGO_STWO[@]}" \
...
run_timed verify_folded_gemma_richer_slice_family \
  "${CARGO_STWO[@]}" \
...
readme_lines = [
    '# S-two Folded Gemma Slice Family Bundle V1',
    '',

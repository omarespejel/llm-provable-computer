...
PNG_OUT="${PNG_OUT-$REPO_ROOT/docs/paper/figures/stwo-phase44d-source-emission-2026-04.png}"
PDF_OUT="${PDF_OUT-$REPO_ROOT/docs/paper/figures/stwo-phase44d-source-emission-2026-04.pdf}"
...
if [[ "$CAPTURE_TIMINGS" == "1" && "$ALLOW_HOST_DEPENDENT_OUTPUTS" != "1" ]]; then
  CANONICAL_CAPTURE_PATHS=(
    "$REPO_ROOT/docs/paper/evidence/stwo-phase44d-source-emission-2026-04.tsv"
    "$REPO_ROOT/docs/paper/evidence/stwo-phase44d-source-emission-2026-04.json"
    "$REPO_ROOT/docs/paper/figures/stwo-phase44d-source-emission-2026-04.svg"
  )
  if [[ -n "$PNG_OUT" ]]; then
    CANONICAL_CAPTURE_PATHS+=(
      "$REPO_ROOT/docs/paper/figures/stwo-phase44d-source-emission-2026-04.png"
    )
  fi
  if [[ -n "$PDF_OUT" ]]; then
    CANONICAL_CAPTURE_PATHS+=(
      "$REPO_ROOT/docs/paper/figures/stwo-phase44d-source-emission-2026-04.pdf"
    )
  fi
...
TMP_EVIDENCE_DIR="$(mktemp -d "$EVIDENCE_DIR/stwo-phase44d.XXXXXX")"
TMP_JSON_DIR="$(mktemp -d "$JSON_DIR/stwo-phase44d.XXXXXX")"
TMP_FIGURE_DIR="$(mktemp -d "$SVG_DIR/stwo-phase44d.XXXXXX")"
...
if [[ "$CAPTURE_TIMINGS" == "1" ]]; then
  ...
  python3 scripts/paper/aggregate_stwo_phase44d_source_emission_benchmark.py \
    --inputs "${RUN_INPUTS[@]}" \
...
python3 scripts/paper/generate_stwo_phase44d_source_emission_figure.py "${FIGURE_ARGS[@]}"

if [[ -n "$PNG_OUT" && ! -f "$TMP_PNG" ]]; then
  echo "phase44d source-emission figure generation did not produce requested PNG output: $PNG_OUT" >&2
  exit 1
fi
if [[ -n "$PDF_OUT" && ! -f "$TMP_PDF" ]]; then
  echo "phase44d source-emission figure generation did not produce requested PDF output: $PDF_OUT" >&2
  exit 1
fi

...
BUNDLE_DIR="${BUNDLE_DIR:-$REPO_ROOT/docs/paper/artifacts/stwo-repeated-linear-block-slice-accumulation-v1-2026-04-21}"
...
if [ "$ALLOW_DIRTY_BUNDLE_BUILD" != "1" ]; then
  if ! git diff --quiet --ignore-submodules -- || ! git diff --cached --quiet --ignore-submodules --; then
    echo "Refusing to generate repeated linear-block-slice bundle from a dirty tracked worktree." >&2
    echo "Commit or stash tracked changes first, or set ALLOW_DIRTY_BUNDLE_BUILD=1 to override." >&2
    exit 1
  fi
  if [ -n "$(git ls-files --others --exclude-standard)" ]; then
    echo "Refusing to generate repeated linear-block-slice bundle with unrelated untracked files present." >&2
...
bundle_version: stwo-repeated-linear-block-slice-accumulation-v1
repo_root: .
...
repeated_accumulation_artifact: repeated-linear-block-slice-accumulation.stwo.json
...
scope: repeated linear-block-shaped tensor-native slice accumulation over one shared S-two proof
MANIFEST
...
index_lines = [
    "# Appendix Artifact Index (S-two Repeated Linear-block-Slice Accumulation Bundle V1)",
    "",
...
index_lines.extend([
    ...
    "",
    "## Notes",
    "- This bundle does not claim recursive cryptographic compression. It freezes verifier-bound repeated-slice accumulation over one shared S-two proof and one repeated linear-block-shaped slice template.",
...
readme_lines = [
    "# S-two Repeated Linear-block-Slice Accumulation Bundle V1",
    "",
    ...
    "- one Linear-block richer-slice artifact that binds score, grouped-value, residual, normalization, activation, and selected memory-window invariants, and",
    "- one repeated linear-block-slice accumulation artifact over multiple block-indexed slice members that reuse the same shared proof and shared table registry.",
    "",

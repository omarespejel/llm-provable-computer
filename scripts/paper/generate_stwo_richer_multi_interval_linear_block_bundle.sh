...
BUNDLE_DIR="${BUNDLE_DIR:-$REPO_ROOT/docs/paper/artifacts/stwo-richer-multi-interval-linear-block-v1-2026-04-21}"
...
GENERATOR_SCRIPT_REL="scripts/paper/generate_stwo_richer_multi_interval_linear_block_bundle.sh"
...
if [ "$ALLOW_DIRTY_BUNDLE_BUILD" != "1" ]; then
  if ! git diff --quiet --ignore-submodules -- || ! git diff --cached --quiet --ignore-submodules --; then
    echo "Refusing to generate richer multi-interval Linear-block bundle from a dirty tracked worktree." >&2
    echo "Commit or stash tracked changes first, or set ALLOW_DIRTY_BUNDLE_BUILD=1 to override." >&2
    exit 1
  fi
  if [ -n "$(git ls-files --others --exclude-standard)" ]; then
    echo "Refusing to generate richer multi-interval Linear-block bundle with unrelated untracked files present." >&2
...
RICHER_MULTI_INTERVAL_JSON="$BUNDLE_DIR/folded-multi-interval-linear-block-richer-family.stwo.json"
...
bundle_version: stwo-richer-multi-interval-linear-block-v1
repo_root: .
...
folded_richer_multi_interval_artifact: folded-multi-interval-linear-block-richer-family.stwo.json
...
run_timed prepare_folded_multi_interval_linear_block_richer_family \
  "${CARGO_STWO[@]}" \
...
run_timed verify_folded_multi_interval_linear_block_richer_family \
  "${CARGO_STWO[@]}" \
...
comparison_rows = [
    ('provable-transformer-vm', 'internal-artifact', 'shared_execution_proof_bytes', str(shared_execution_proof_bytes), 'same frozen Linear-block proof reused across Phase99, Phase101.5, and Phase102 surfaces'),
...
readme_md.write_text(
    f"# Richer Multi-Interval Linear-block Bundle\n\n"

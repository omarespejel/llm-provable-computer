#!/usr/bin/env python3
"""Checked fused native Stwo two-head long-sequence bounded Softmax-table gate for issue #498.
...
table membership for a deterministic two-head, sixteen-step-per-head d=8
...
SOURCE_INPUT_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-two-head-longseq-bounded-softmax-table-proof-2026-05.json"
FUSED_ENVELOPE_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-two-head-longseq-fused-softmax-table-proof-2026-05.envelope.json"
SOURCE_INPUT_SCRIPT = ROOT / "scripts" / "zkai_attention_kv_stwo_native_two_head_longseq_bounded_softmax_table_proof_input.py"
JSON_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-two-head-longseq-fused-softmax-table-gate-2026-05.json"
TSV_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-two-head-longseq-fused-softmax-table-gate-2026-05.tsv"
...
CLAIM_BOUNDARY = (
    "ONE_NATIVE_STWO_TWO_HEAD_LONGSEQ_BOUNDED_SOFTMAX_TABLE_ATTENTION_PROOF_WITH_LOGUP_TABLE_MEMBERSHIP_"
...
SOURCE_SCORE_ROW_COMMITMENT = "blake2b-256:d992c3f79798b2d9b1c7c554ebc4701efb09294aa08c787056ebe4c3af5e0449"
SOURCE_FINAL_KV_CACHE_COMMITMENT = "blake2b-256:66825d1ed578ff95dac06fa6a8347be8461dc0660cb0aef225e79f65e5e3c92f"
SOURCE_OUTPUTS_COMMITMENT = "blake2b-256:ad2c0dfadbfa59de36d8833778d2769fe077e4fd2fc0b39717f0d2a510e06bb1"
SOURCE_WEIGHT_TABLE_COMMITMENT = "blake2b-256:dca926ee593cacac32c30dcfd64d40ae9076d4d256019dac5f526d3eeb552b17"
SOURCE_HEAD_COUNT = 2
...
SOURCE_SEMANTIC_SCOPE = "two_head_longseq_d8_bounded_table_softmax_approx_attention_kv_causal_mask_rows_bound_to_statement_receipt"
SOURCE_DECISION = "GO_STWO_NATIVE_ATTENTION_KV_TWO_HEAD_LONGSEQ_BOUNDED_SOFTMAX_TABLE_AIR_PROOF"
SOURCE_INPUT_DECISION = "GO_INPUT_FOR_STWO_NATIVE_ATTENTION_KV_TWO_HEAD_LONGSEQ_BOUNDED_SOFTMAX_TABLE_AIR_PROOF"
SOURCE_TARGET_ID = "attention-kv-d8-causal-mask-two-head-longseq-bounded-softmax-table-v1"
SOURCE_VERIFIER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-two-head-longseq-bounded-softmax-table:v1"
LOOKUP_RELATION = "AttentionKvTwoHeadLongseqFusedSoftmaxTableRelation"
LOOKUP_RELATION_WIDTH = 2
FUSED_BACKEND_VERSION = "stwo-attention-kv-two-head-longseq-fused-bounded-softmax-table-logup-v1"
FUSED_PROOF_SCHEMA_VERSION = "stwo-attention-kv-two-head-longseq-fused-bounded-softmax-table-logup-proof-v1"
FUSED_STATEMENT_VERSION = "zkai-attention-kv-stwo-native-two-head-longseq-fused-softmax-table-logup-statement-v1"
FUSED_SEMANTIC_SCOPE = "two_head_longseq_d8_bounded_softmax_table_attention_arithmetic_and_logup_membership_fused_in_one_native_stwo_proof"
FUSED_TARGET_ID = "attention-kv-two-head-longseq-d8-causal-mask-fused-bounded-softmax-table-logup-v1"
FUSED_VERIFIER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-two-head-longseq-fused-bounded-softmax-table-logup:v1"
...
TABLE_MULTIPLICITIES = (
    ...
    {"gap": 5, "weight": 45, "multiplicity": 7},
    {"gap": 6, "weight": 32, "multiplicity": 5},
    {"gap": 7, "weight": 23, "multiplicity": 1},
    {"gap": 8, "weight": 16, "multiplicity": 266},
)
NON_CLAIMS = (
...
VALIDATION_COMMANDS = (
    ...
    "cargo +nightly-2025-07-14 test --locked attention_kv_native_two_head_longseq_bounded_softmax_table_proof --lib --features stwo-backend",
    "cargo +nightly-2025-07-14 run --locked --features stwo-backend --bin zkai_attention_kv_native_two_head_longseq_bounded_softmax_table_proof -- prove docs/engineering/evidence/zkai-attention-kv-stwo-native-two-head-longseq-bounded-softmax-table-proof-2026-05.json docs/engineering/evidence/zkai-attention-kv-stwo-native-two-head-longseq-bounded-softmax-table-proof-2026-05.envelope.json",
    "cargo +nightly-2025-07-14 run --locked --features stwo-backend --bin zkai_attention_kv_native_two_head_longseq_bounded_softmax_table_proof -- verify docs/engineering/evidence/zkai-attention-kv-stwo-native-two-head-longseq-bounded-softmax-table-proof-2026-05.envelope.json",
...
class AttentionKvTwoHeadLongseqFusedSoftmaxTableGateError(ValueError):
    pass
...
def load_script_module(path: pathlib.Path, module_name: str) -> ModuleType:
    ...
    if spec is None or spec.loader is None:
        raise AttentionKvTwoHeadLongseqFusedSoftmaxTableGateError(f"failed to load {module_name}: {path}")
...
SOURCE_INPUT_MODULE = load_script_module(
    SOURCE_INPUT_SCRIPT, "zkai_attention_kv_stwo_native_two_head_longseq_bounded_softmax_table_proof_input"
)
...
def proof_bytes(envelope: dict[str, Any]) -> bytes:
    ...
    if not isinstance(proof, list):
        raise AttentionKvTwoHeadLongseqFusedSoftmaxTableGateError("proof must be a byte list")
    if len(proof) != FUSED_PROOF_SIZE_BYTES:
        raise AttentionKvTwoHeadLongseqFusedSoftmaxTableGateError("proof byte length drift")
    if any(not isinstance(byte, int) or isinstance(byte, bool) or byte < 0 or byte > 255 for byte in proof):
        raise AttentionKvTwoHeadLongseqFusedSoftmaxTableGateError("proof byte outside uint8 range")
    return bytes(proof)
...
def mutate_same_size_stark_proof_commitment(envelope: dict[str, Any]) -> None:
    ...
    if not isinstance(proof, list) or not proof:
        raise AttentionKvTwoHeadLongseqFusedSoftmaxTableGateError("cannot mutate missing proof bytes")
...
def assert_native_verifier_summary(mapping: dict[str, Any], expected: dict[str, Any], label: str) -> None:
    ...
    if not isinstance(schema, str) or not schema.endswith("-cli-summary-v1"):
        raise AttentionKvTwoHeadLongseqFusedSoftmaxTableGateError(f"{label} schema drift")
    path = mapping.get("fused_envelope_path")
    if not isinstance(path, str) or not path.endswith(".json"):
        raise AttentionKvTwoHeadLongseqFusedSoftmaxTableGateError(f"{label} fused envelope path drift")
...
def verify_fused_envelope_bytes_with_native_cli(envelope_bytes: bytes) -> None:
    if len(envelope_bytes) <= 0 or len(envelope_bytes) > MAX_FUSED_ENVELOPE_JSON_BYTES:
        raise AttentionKvTwoHeadLongseqFusedSoftmaxTableGateError("fused envelope byte size drift")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with subprocess.Popen(
            [
                ...
                "stwo-backend",
                "--bin",
                "zkai_attention_kv_native_two_head_longseq_fused_softmax_table_proof",
                "--",
                "verify",
                str(envelope_path),
            ],
    ...
    *,
    ...
    assert_fields(
        envelope,
        {
            ...
            "decision": DECISION.replace("TWO_HEAD_LONGSEQ_", ""),
    ...
    if envelope.get("source_input") != source_input:
        raise AttentionKvTwoHeadLongseqFusedSoftmaxTableGateError("fused source input split-brain drift")
    if envelope.get("fused_summary") != expected_summary(source_input):
        raise AttentionKvTwoHeadLongseqFusedSoftmaxTableGateError("fused summary drift")
...
def validate_result(result: dict[str, Any], envelope: dict[str, Any], source_input: dict[str, Any]) -> None:
    ...
    for key, value in expected.items():
        ...
        if result.get(key) != value:
            raise AttentionKvTwoHeadLongseqFusedSoftmaxTableGateError(f"result drift for {key}")
    ...
    if not isinstance(mutation_results, list) or len(mutation_results) != EXPECTED_MUTATION_COUNT:
        raise AttentionKvTwoHeadLongseqFusedSoftmaxTableGateError("mutation result shape drift")
    if tuple(item.get("name") for item in mutation_results if isinstance(item, dict)) != EXPECTED_MUTATION_NAMES:
        raise AttentionKvTwoHeadLongseqFusedSoftmaxTableGateError("mutation result name drift")
    if any(item.get("rejected") is not True for item in mutation_results if isinstance(item, dict)):
        raise AttentionKvTwoHeadLongseqFusedSoftmaxTableGateError("mutation accepted")

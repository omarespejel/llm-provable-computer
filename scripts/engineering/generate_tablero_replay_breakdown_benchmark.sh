...
TSV_OUT="${TSV_OUT:-$REPO_ROOT/docs/engineering/evidence/phase44d-carry-aware-experimental-replay-baseline-breakdown-2026-04.tsv}"
JSON_OUT="${JSON_OUT:-$REPO_ROOT/docs/engineering/evidence/phase44d-carry-aware-experimental-replay-baseline-breakdown-2026-04.json}"

if [[ "$CAPTURE_TIMINGS" != "1" ]]; then
  echo "CAPTURE_TIMINGS must be 1 for the checked-in replay breakdown benchmark" >&2
  exit 1
fi
...
TMP_EVIDENCE_DIR="$(mktemp -d "$EVIDENCE_DIR/tablero-replay-breakdown.XXXXXX")"
TMP_JSON_DIR="$(mktemp -d "$JSON_DIR/tablero-replay-breakdown.XXXXXX")"

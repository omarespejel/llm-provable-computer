#!/usr/bin/env python3
"""Render a publication-style figure for the Phase44D carry-aware experimental 2x2 sweep."""
...
EXPECTED_BENCHMARK_VERSION = "stwo-phase44d-source-emission-experimental-2x2-layout-benchmark-v1"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-prefix",
        type=Path,
        default=DEFAULT_OUTDIR / "phase44d-carry-aware-experimental-2x2-scaling-2026-04",
    )
...
def main() -> None:
    ...
    verify_ax.set_title("Verification latency")
    ...
    bytes_ax.set_ylabel("Serialized bytes")
    bytes_ax.set_title("Serialized artifact size")
    ...
    title_suffix = (
        f"Median of {timing_runs} runs"
        if timing_mode == "measured_median"
        else "Single measured run"
    )
    fig.suptitle(
        f"Phase44D carry-aware experimental 2x2 scaling ({title_suffix})",
        y=0.97,
    )
    fig.subplots_adjust(top=0.72, bottom=0.24, wspace=0.18)

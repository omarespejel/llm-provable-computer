#!/usr/bin/env python3
"""Aggregate repeated Phase44D carry-aware experimental scaling runs using median timings.

Audit note (issue #294, post-#292): this aggregator medians `emit_ms` and
`verify_ms` independently across the input runs. The two columns are
orthogonal independent measurements (boundary construction time vs
verification time) and are not components of a shared outer measurement,
so there is no additive identity to preserve. The double-hash /
per-column-median additivity bugs caught in

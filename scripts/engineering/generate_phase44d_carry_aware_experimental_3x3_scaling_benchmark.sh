...
PY
then
  echo "python3 must have matplotlib installed before running the Phase44D 3x3 figure/evidence generator" >&2
  exit 1
fi
...
TMP_EVIDENCE_DIR="$(mktemp -d "$EVIDENCE_DIR/phase44d-experimental-3x3.XXXXXX")"
TMP_JSON_DIR="$(mktemp -d "$JSON_DIR/phase44d-experimental-3x3.XXXXXX")"
TMP_FIGURE_DIR="$(mktemp -d "$SVG_DIR/phase44d-experimental-3x3.XXXXXX")"
...
python3 scripts/engineering/generate_phase44d_carry_aware_experimental_3x3_scaling_figure.py "${FIGURE_ARGS[@]}"

if [[ -n "$PNG_OUT" && ! -f "$TMP_PNG" ]]; then
  echo "phase44d experimental 3x3 figure generation did not produce requested PNG output: $PNG_OUT" >&2
  exit 1
fi
if [[ -n "$PDF_OUT" && ! -f "$TMP_PDF" ]]; then
  echo "phase44d experimental 3x3 figure generation did not produce requested PDF output: $PDF_OUT" >&2
  exit 1
fi

#!/usr/bin/env python3
"""Generate or verify a RISC Zero receipt for a scaled carried attention/KV sequence.

This gate answers issue #444 for the scaled sequence route. It extends the earlier
one-transition RISC Zero result into an eight-transition carried KV-cache
...
The claim is intentionally narrow: RISC Zero proves one fixed eight-step
integer-argmax attention/KV sequence. It is not native Stwo, not Softmax, not
full inference, and not recursion or PCD.
"""
...
PROGRAM_DIR = ROOT / "programs" / "risc0-attention-kv-scaled-sequence-receipt"
PROGRAM_MANIFEST = PROGRAM_DIR / "Cargo.toml"
RECEIPT_OUT = EVIDENCE_DIR / "zkai-attention-kv-risc0-scaled-sequence-receipt-2026-05.bincode"
JSON_OUT = EVIDENCE_DIR / "zkai-attention-kv-risc0-scaled-sequence-receipt-2026-05.json"
TSV_OUT = EVIDENCE_DIR / "zkai-attention-kv-risc0-scaled-sequence-receipt-2026-05.tsv"

SCHEMA = "zkai-attention-kv-risc0-scaled-sequence-receipt-gate-v1"
ISSUE = 444
SOURCE_ISSUE = 442
RESULT = "GO"
DECISION = "GO_ATTENTION_KV_RISC0_SCALED_SEQUENCE_RECEIPT_FOR_CARRIED_KV_STATE"
CLAIM_BOUNDARY = (
    "RISC0_RECEIPT_PROVES_EIGHT_STEP_INTEGER_ARGMAX_ATTENTION_KV_SEQUENCE_"
    "NOT_STWO_NOT_SOFTMAX_NOT_RECURSION_OR_PCD_NOT_LONG_CONTEXT_OR_FULL_INFERENCE_NOT_AGENT_CORRECTNESS"
)
ROUTE_ID = "risc0_attention_kv_scaled_sequence_semantics_receipt"
SYSTEM = "RISC Zero"
JOURNAL_SCHEMA = "zkai-attention-kv-risc0-scaled-sequence-journal-v1"
SEMANTICS = "tiny-single-head-integer-argmax-attention-scaled-sequence-v1"
MASKING_POLICY = "none"
...
def sequence_fixture() -> dict[str, Any]:
    return {
        "case_id": "tiny_single_head_argmax_kv_scaled_sequence_v1",
        "model_config": {
        ...
        "input_steps": [
            ...
            {"token_position": 5, "query": [3, 0], "new_key": [-1, 3], "new_value": [7, 1]},
            {"token_position": 6, "query": [0, 3], "new_key": [3, -2], "new_value": [-3, 4]},
            {"token_position": 7, "query": [2, 2], "new_key": [-2, -1], "new_value": [6, 6]},
            {"token_position": 8, "query": [-2, 1], "new_key": [1, 3], "new_value": [8, -1]},
            {"token_position": 9, "query": [1, -3], "new_key": [2, -2], "new_value": [-5, 5]},
        ],
    }
...
def transition_commitments(journal: dict[str, Any] | None = None) -> list[str]:
    ...
    return [
        blake2b_commitment(row, f"ptvm:zkai:attention-kv-scaled-sequence-transition:{row['step_index']}:v1")
        for row in journal["transitions"]
    ]


def journal_commitment(journal: dict[str, Any] | None = None) -> str:
    return blake2b_commitment(journal or expected_journal(), "ptvm:zkai:attention-kv-risc0-scaled-sequence-journal:v1")


def statement_fields(journal: dict[str, Any], receipt_commitment: str, image_id_hex: str) -> dict[str, Any]:
    fixture = sequence_fixture()
    fields = {
        "verifier_domain": "ptvm:zkai:attention-kv-scaled-sequence:risc0:v1",
        "statement_kind": "attention-kv-cache-scaled-sequence",
        ...
        "outputs_commitment": blake2b_commitment(
            [row["attention_output"] for row in journal["transitions"]], "ptvm:zkai:attention-scaled-sequence-outputs:v1"
        ),
    ...
    fields["public_instance_commitment"] = blake2b_commitment(
        ...
        "ptvm:zkai:attention-kv-scaled-sequence-public-instance:v1",
    )
    fields["statement_commitment"] = blake2b_commitment(
        {key: value for key, value in fields.items() if key not in {"statement_commitment"}},
        "ptvm:zkai:attention-kv-scaled-sequence-statement:v1",
    )
    return fields
...
def toolchain_probe() -> dict[str, Any]:
    ...
    return {
        "probe_scope": "local_risc0_attention_kv_scaled_sequence_receipt_generation_and_verification",
...
def generate_or_verify_receipt(*, prove: bool, receipt_path: pathlib.Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(dir=target_root) as raw_tmp:
        ...
        input_path = tmp / "attention-scaled-sequence-input.json"
...
def validate_core_payload(payload: dict[str, Any], *, strict_receipt: bool = False) -> None:
    ...
    expect_equal(verification["host_summary_schema"], "zkai-attention-kv-risc0-scaled-sequence-host-summary-v1", "host summary schema", layer="receipt_metadata")
    ...
    if strict_receipt:
        ...
        expect_equal(strict_summary["schema"], "zkai-attention-kv-risc0-scaled-sequence-host-summary-v1", "strict host schema", layer="risc0_host")
    ...
    expect_equal(probe["probe_scope"], "local_risc0_attention_kv_scaled_sequence_receipt_generation_and_verification", "probe scope", layer="toolchain_probe")

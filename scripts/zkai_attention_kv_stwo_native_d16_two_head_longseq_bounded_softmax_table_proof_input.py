#!/usr/bin/env python3
"""Build the native Stwo d=16 two-head long-sequence bounded Softmax-table attention/KV proof input.

This answers issue #525 narrowly. It extends the issue #521 d16 two-head bounded
Softmax-table fixture along sequence length while keeping head count fixed and extending sequence length:
...
SOURCE_SCRIPT = ROOT / "scripts" / "zkai_attention_kv_stwo_native_d16_two_head_bounded_softmax_table_proof_input.py"
JSON_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d16-two-head-longseq-bounded-softmax-table-proof-2026-05.json"
TSV_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d16-two-head-longseq-bounded-softmax-table-proof-2026-05.tsv"

SOURCE_SCHEMA = "zkai-attention-kv-stwo-native-d16-two-head-bounded-softmax-table-air-proof-input-v1"
SOURCE_DECISION = "GO_INPUT_FOR_STWO_NATIVE_ATTENTION_KV_D16_TWO_HEAD_BOUNDED_SOFTMAX_TABLE_AIR_PROOF"
...
SOURCE_PAYLOAD_SHA256 = "bd2afa3328ac72ec315f403a0bea9cc4067844d23268696f955fbb64a540324a"

SCHEMA = "zkai-attention-kv-stwo-native-d16-two-head-longseq-bounded-softmax-table-air-proof-input-v1"
DECISION = "GO_INPUT_FOR_STWO_NATIVE_ATTENTION_KV_D16_TWO_HEAD_LONGSEQ_BOUNDED_SOFTMAX_TABLE_AIR_PROOF"
ISSUE = 525
SOURCE_ISSUE = 521
TARGET_ID = "attention-kv-d16-two-head-longseq-causal-mask-bounded-softmax-table-v1"
REQUIRED_BACKEND_VERSION = "stwo-attention-kv-d16-two-head-longseq-causal-mask-bounded-softmax-table-v1"
PROOF_VERSION = "stwo-attention-kv-d16-two-head-longseq-causal-mask-bounded-softmax-table-air-proof-v1"
STATEMENT_VERSION = "zkai-attention-kv-stwo-native-d16-two-head-longseq-bounded-softmax-table-statement-v1"
SEMANTIC_SCOPE = "d16_two_head_longseq_bounded_table_softmax_approx_attention_kv_causal_mask_rows_bound_to_statement_receipt"
VERIFIER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d16-two-head-longseq-bounded-softmax-table:v1"
...
ROW_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d16-two-head-longseq-bounded-softmax-table-score-rows:v1"
INITIAL_KV_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d16-two-head-longseq-bounded-softmax-table-initial-kv:v1"
INPUT_STEPS_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d16-two-head-longseq-bounded-softmax-table-input-steps:v1"
FINAL_KV_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d16-two-head-longseq-bounded-softmax-table-final-kv:v1"
OUTPUTS_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d16-two-head-longseq-bounded-softmax-table-outputs:v1"
PUBLIC_INSTANCE_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d16-two-head-longseq-bounded-softmax-table-public-instance:v1"
PROOF_NATIVE_PARAMETER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d16-two-head-longseq-bounded-softmax-table-proof-parameters:v1"
WEIGHT_TABLE_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d16-two-head-longseq-bounded-softmax-table-weight-table:v1"
...
NEXT_BACKEND_STEP = "fuse the same d16 two-head long-sequence bounded Softmax-table arithmetic with LogUp table membership in one native Stwo proof"
...
VALIDATION_COMMANDS = [
    ...
    "cargo +nightly-2025-07-14 run --features stwo-backend --bin zkai_attention_kv_native_d16_two_head_longseq_bounded_softmax_table_proof -- prove docs/engineering/evidence/zkai-attention-kv-stwo-native-d16-two-head-longseq-bounded-softmax-table-proof-2026-05.json docs/engineering/evidence/zkai-attention-kv-stwo-native-d16-two-head-longseq-bounded-softmax-table-proof-2026-05.envelope.json",
    "cargo +nightly-2025-07-14 run --features stwo-backend --bin zkai_attention_kv_native_d16_two_head_longseq_bounded_softmax_table_proof -- verify docs/engineering/evidence/zkai-attention-kv-stwo-native-d16-two-head-longseq-bounded-softmax-table-proof-2026-05.envelope.json",
...
class AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError(ValueError):
    pass
...
def _load_source_module() -> Any:
    ...
    if spec is None or spec.loader is None:
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError(f"failed to load source script: {SOURCE_SCRIPT}")
...
def require_nonnegative_bit_bound(value: int, *, bits: int, label: str) -> int:
    ...
    if checked < 0 or checked >= (1 << bits):
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError(f"{label} outside {bits}-bit range")
    return checked


def next_power_of_two(value: int) -> int:
    if value <= 0:
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError("trace row count input must be positive")
    return 1 << (value - 1).bit_length()


def require_vector(value: Any, *, width: int, label: str) -> list[int]:
    if not isinstance(value, list):
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError(f"{label} must be a list")
    if len(value) != width:
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError(f"{label} width drift")
...
def require_head_index(value: Any, label: str) -> int:
    ...
    if head_index < 0 or head_index >= HEAD_COUNT:
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError(f"{label} outside head range")
    return head_index


def require_kv_item(value: Any, *, label: str) -> dict[str, Any]:
    if not isinstance(value, dict):
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError(f"{label} must be an object")
    return {
...
def source_payload() -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError("source payload must be an object")
    expected_scalars = {
    ...
    for key, expected in expected_scalars.items():
        if payload.get(key) != expected:
            raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError(f"source payload {key} drift")
    observed = sha256_hex_bytes(canonical_json_bytes(payload))
    if observed != SOURCE_PAYLOAD_SHA256:
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError("source payload commitment drift")
    return payload
...
def weight_from_gap(score_gap: int) -> int:
    if score_gap < 0:
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError("negative score gap")
...
def rows_commitment(rows: list[dict[str, Any]]) -> str:
    return commitment_from_parts(
        [
            ("encoding", "attention_kv_stwo_native_d16_two_head_longseq_bounded_softmax_table_score_rows_v1"),
...
def outputs_material(steps: list[dict[str, Any]], outputs: list[list[int]]) -> list[list[int]]:
    if len(steps) != len(outputs):
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError("output/input step length mismatch")
...
def fixture_initial_kv() -> list[dict[str, Any]]:
    ...
    if len(items) != INITIAL_KV_ITEMS:
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError("source initial KV length drift")
    return items
...
def fixture_input_steps() -> list[dict[str, Any]]:
    ...
    if len(seed_items) != HEAD_COUNT * SOURCE_SEQUENCE_LENGTH:
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError("source input step length drift")
    ...
    for head_index, head_steps in enumerate(per_head):
        if len(head_steps) != SOURCE_SEQUENCE_LENGTH:
            raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError(f"source head {head_index} step count drift")
        for local_step, item in enumerate(head_steps):
            ...
            if item["token_position"] != expected_position:
                raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError(
    ...
    if len(extended) != HEAD_COUNT * SEQUENCE_LENGTH:
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError("extended input step length drift")
    return extended
...
def build_score_rows(initial_kv: list[dict[str, Any]], input_steps: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[list[int]]]:
    ...
    for global_step_index, step in enumerate(steps):
        ...
        if step_index >= SEQUENCE_LENGTH:
            raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError(f"input_steps[{global_step_index}] exceeds per-head sequence length")
    ...
    if local_step_counts != [SEQUENCE_LENGTH for _ in range(HEAD_COUNT)]:
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError("per-head input step count drift")
    return rows, current, outputs


def validate_payload(payload: dict[str, Any]) -> None:
    if not isinstance(payload, dict):
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError("payload must be object")
    expected_scalars = {
        "schema": SCHEMA,
        "decision": DECISION,
        "issue": ISSUE,
    ...
    if extra:
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError(f"unknown field(s): {extra}")
    for key, expected in expected_scalars.items():
        if payload.get(key) != expected:
            raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError(f"{key} drift")
    if payload.get("non_claims") != NON_CLAIMS:
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError("non claims drift")
    if payload.get("proof_verifier_hardening") != PROOF_VERIFIER_HARDENING:
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError("proof verifier hardening drift")
    if payload.get("validation_commands") != VALIDATION_COMMANDS:
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError("validation commands drift")
    if payload.get("weight_table_commitment") != weight_table_commitment():
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError("weight table commitment drift")
    ...
    if len(initial) != payload.get("initial_kv_items"):
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError("derived initial KV count drift")
    if len(final_cache) != payload.get("final_kv_items"):
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError("derived final KV count drift")
    if len(rows) != payload.get("score_row_count"):
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError("derived score row count drift")
    if next_power_of_two(len(rows)) != payload.get("trace_row_count"):
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError("derived trace row count drift")
    if payload.get("initial_kv_cache") != initial:
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError("initial KV drift")
    if payload.get("input_steps") != steps:
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError("input steps drift")
    if payload.get("score_rows") != rows:
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError("score rows drift")
    if payload.get("final_kv_cache") != final_cache:
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError("final KV drift")
    if payload.get("attention_outputs") != outputs:
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError("attention outputs drift")
    ...
    if payload.get("statement_commitment") != statement:
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError("statement commitment drift")
    if payload.get("public_instance_commitment") != public_instance_commitment(statement):
        raise AttentionKvD16TwoHeadLongseqBoundedSoftmaxTableInputError("public instance commitment drift")

#!/usr/bin/env python3
"""Checked fused native Stwo d16-two-head-longseq bounded Softmax-table gate for issue #525.
...
SOURCE_INPUT_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d16-two-head-longseq-bounded-softmax-table-proof-2026-05.json"
SOURCE_ENVELOPE_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d16-two-head-longseq-bounded-softmax-table-proof-2026-05.envelope.json"
SIDECAR_ENVELOPE_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d16-two-head-longseq-softmax-table-logup-sidecar-proof-2026-05.envelope.json"
FUSED_ENVELOPE_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d16-two-head-longseq-fused-softmax-table-proof-2026-05.envelope.json"
SOURCE_INPUT_SCRIPT = ROOT / "scripts" / "zkai_attention_kv_stwo_native_d16_two_head_longseq_bounded_softmax_table_proof_input.py"
JSON_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d16-two-head-longseq-fused-softmax-table-gate-2026-05.json"
TSV_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d16-two-head-longseq-fused-softmax-table-gate-2026-05.tsv"
...
CLAIM_BOUNDARY = (
    "ONE_NATIVE_STWO_D16_TWO_HEAD_LONGSEQ_BOUNDED_SOFTMAX_TABLE_ATTENTION_PROOF_WITH_LOGUP_TABLE_MEMBERSHIP_"
...
NON_FUSED_STATUS = "GO_MATCHED_D16_TWO_HEAD_LONGSEQ_SOURCE_PLUS_LOGUP_SIDECAR_COMPARATOR_RECORDED"
...
SOURCE_STATEMENT_COMMITMENT = "blake2b-256:d96619770216d949051980f8c259e437fd70282d30b57bb80461b9c2784f7f34"
SOURCE_PUBLIC_INSTANCE_COMMITMENT = "blake2b-256:66bc04f5540b8c696d450dfe4b85aaeed71b299a66871ffab5c7b93a96b83d0c"
SOURCE_SCORE_ROW_COMMITMENT = "blake2b-256:64731d89c34c6c136c38da11ffab1b043de61a93c2e2cab9b09b5f1386eedee6"
SOURCE_FINAL_KV_CACHE_COMMITMENT = "blake2b-256:60c67f0e8a8555e4f34acb87b87cd54d9513bc132521feba5b7ac61f0ad2fd23"
SOURCE_OUTPUTS_COMMITMENT = "blake2b-256:e1ccf3e6774e1462acb777f4d7fdb81acb78d853bd44b9e73dcd967122c402d2"
SOURCE_WEIGHT_TABLE_COMMITMENT = "blake2b-256:91209e6712c85480d41c2a0953f91149e611a4cd570b154ffaee35134ecdc8a8"
SOURCE_HEAD_COUNT = 2
...
SOURCE_SEMANTIC_SCOPE = "d16_two_head_longseq_bounded_table_softmax_approx_attention_kv_causal_mask_rows_bound_to_statement_receipt"
SOURCE_DECISION = "GO_STWO_NATIVE_ATTENTION_KV_D16_TWO_HEAD_LONGSEQ_BOUNDED_SOFTMAX_TABLE_AIR_PROOF"
SOURCE_INPUT_DECISION = "GO_INPUT_FOR_STWO_NATIVE_ATTENTION_KV_D16_TWO_HEAD_LONGSEQ_BOUNDED_SOFTMAX_TABLE_AIR_PROOF"
...
SOURCE_VERIFIER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d16-two-head-longseq-bounded-softmax-table:v1"
LOOKUP_RELATION = "AttentionKvD16TwoHeadLongseqFusedSoftmaxTableRelation"
SIDECAR_LOOKUP_RELATION = "AttentionKvD16TwoHeadLongseqSoftmaxTableLookupRelation"
LOOKUP_RELATION_WIDTH = 2
SOURCE_PROOF_COMMITMENTS = 3
SIDECAR_PROOF_COMMITMENTS = 4

FUSED_BACKEND_VERSION = "stwo-attention-kv-d16-two-head-longseq-fused-bounded-softmax-table-logup-v1"
FUSED_PROOF_SCHEMA_VERSION = "stwo-attention-kv-d16-two-head-longseq-fused-bounded-softmax-table-logup-proof-v1"
FUSED_STATEMENT_VERSION = "zkai-attention-kv-stwo-native-d16-two-head-longseq-fused-softmax-table-logup-statement-v1"
FUSED_SEMANTIC_SCOPE = "d16_two_head_longseq_bounded_softmax_table_attention_arithmetic_and_logup_membership_fused_in_one_native_stwo_proof"
FUSED_TARGET_ID = "attention-kv-d16-two-head-longseq-causal-mask-fused-bounded-softmax-table-logup-v1"
FUSED_VERIFIER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d16-two-head-longseq-fused-bounded-softmax-table-logup:v1"
FUSED_PROOF_COMMITMENTS = 4
FUSED_TRACE_COMMITMENTS = 3

TABLE_MULTIPLICITIES = ({"gap": 0, "weight": 256, "multiplicity": 41}, {"gap": 1, "weight": 181, "multiplicity": 4}, {"gap": 2, "weight": 128, "multiplicity": 2}, {"gap": 3, "weight": 91, "multiplicity": 4}, {"gap": 4, "weight": 64, "multiplicity": 1}, {"gap": 5, "weight": 45, "multiplicity": 3}, {"gap": 6, "weight": 32, "multiplicity": 1}, {"gap": 7, "weight": 23, "multiplicity": 2}, {"gap": 8, "weight": 16, "multiplicity": 278})
NON_CLAIMS = (
...
class AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError(ValueError):
    pass
...
def load_script_module(path: pathlib.Path, module_name: str) -> ModuleType:
    ...
    if spec is None or spec.loader is None:
        raise AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError(f"failed to load {module_name}: {path}")
...
SOURCE_INPUT_MODULE = load_script_module(
    SOURCE_INPUT_SCRIPT, "zkai_attention_kv_stwo_native_d16_two_head_longseq_bounded_softmax_table_proof_input"
)


def read_bounded_bytes(path: pathlib.Path, max_bytes: int, label: str) -> bytes:
    if not path.is_file():
        raise AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError(f"missing {label}: {path}")
    size = path.stat().st_size
    if size <= 0 or size > max_bytes:
        raise AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError(f"{label} size drift: got {size}, max {max_bytes}")
    try:
        raw = path.read_bytes()
    except OSError as err:
        raise AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError(f"failed to read {label}: {err}") from err
    if len(raw) != size:
        raise AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError(f"{label} read size drift: stat={size}, read={len(raw)}")
    return raw
...
def parse_bounded_json_bytes(raw: bytes, label: str) -> Any:
    try:
        ...
        raise AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError(f"failed to decode {label}: {err}") from err
...
def parse_stark_proof(
    ...
    *,
    ...
    if not isinstance(proof_bytes, list):
        raise AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError(f"{label} proof must be a byte list")
    if len(proof_bytes) != expected_bytes:
        raise AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError(f"{label} proof byte length drift")
    if any(not isinstance(byte, int) or isinstance(byte, bool) or byte < 0 or byte > 255 for byte in proof_bytes):
        raise AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError(f"{label} proof bytes must be uint8 values")
    try:
        payload = json.loads(bytes(proof_bytes).decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as err:
        raise AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError(f"failed to decode {label} proof payload: {err}") from err
    if set(payload) != {"stark_proof"}:
        raise AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError(f"{label} proof payload schema drift")
    stark_proof = payload["stark_proof"]
    if not isinstance(stark_proof, dict):
        raise AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError(f"{label} stark_proof must be an object")
    commitments = stark_proof.get("commitments")
    if not isinstance(commitments, list) or len(commitments) != expected_commitments:
        raise AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError(f"{label} proof commitment count drift")
    return stark_proof
...
def same_digit_int_mutation(value: int, label: str) -> int:
    for candidate in (value + 1, value - 1):
        if 0 <= candidate <= 255 and len(str(candidate)) == len(str(value)):
            return candidate
    raise AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError(f"no same-width byte mutation for {label}")
...
def mutate_same_size_stark_proof_commitment(envelope: dict[str, Any]) -> None:
    ...
    if len(proof_bytes) != len(envelope["proof"]):
        raise AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError("same-size proof mutation changed byte length")
...
def verify_fused_envelope_bytes_with_native_cli(envelope_bytes: bytes, label: str) -> None:
    ...
    command = [
        ...
        "stwo-backend",
        "--bin",
        "zkai_attention_kv_native_d16_two_head_longseq_fused_softmax_table_proof",
        "--",
        "verify",
        str(tmp_path),
    ]
    try:
        ...
        raise AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError(f"native fused verifier failed to run for {label}: {err}") from err
    finally:
        try:
            tmp_path.unlink(missing_ok=True)
        except OSError:
    ...
    if completed.returncode != 0:
        detail = (completed.stderr or completed.stdout).strip().splitlines()
        suffix = detail[-1] if detail else f"exit code {completed.returncode}"
        raise AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError(f"native fused verifier rejected {label}: {suffix}")
    try:
        summary = json.loads(completed.stdout)
    except json.JSONDecodeError as err:
        raise AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError(f"native fused verifier emitted malformed JSON: {err}") from err
    expected = {
        "mode": "verify",
        "verified": True,
...
def validate_source_input_contract(source_input: dict[str, Any]) -> None:
    ...
    if len(source_input.get("score_rows", [])) != SOURCE_SCORE_ROWS:
        raise AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError("source score row count drift")
    if len(source_input.get("weight_table", [])) != SOURCE_TABLE_ROWS:
        raise AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError("source weight table row count drift")
...
def validate_source_artifacts(
    ...
    *,
    ...
    try:
        ...
        raise AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError(f"source input validation drift: {err}") from err
    ...
    verify_envelope_bytes_with_native_cli(
        ...
        binary="zkai_attention_kv_native_d16_two_head_longseq_bounded_softmax_table_proof",
        expected_summary={
            "mode": "verify",
            "verified": True,
    ...
    assert_fields(
        ...
        {
            "proof_backend": "stwo",
            "proof_backend_version": "stwo-attention-kv-d16-two-head-longseq-softmax-table-logup-sidecar-proof-v1",
            "statement_version": "zkai-attention-kv-stwo-native-d16-two-head-longseq-softmax-table-logup-sidecar-statement-v1",
            "semantic_scope": "d16_two_head_longseq_bounded_softmax_table_membership_constrained_by_native_stwo_logup_sidecar",
            "decision": "GO_NATIVE_STWO_AIR_CONSTRAINED_SOFTMAX_TABLE_LOOKUP_RELATION_SIDECAR",
            "verifier_domain": "ptvm:zkai:attention-kv-stwo-native-d16-two-head-longseq-softmax-table-logup-sidecar:v1",
        },
        "sidecar envelope",
    )
    ...
    if not isinstance(lookup_summary, dict):
        raise AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError("sidecar lookup summary missing")
    expected_sidecar = {
        "source_statement_commitment": SOURCE_STATEMENT_COMMITMENT,
    ...
    verify_envelope_bytes_with_native_cli(
        ...
        binary="zkai_attention_kv_native_d16_two_head_longseq_softmax_table_lookup_proof",
        expected_summary={
            "mode": "verify",
            "verified": True,
...
def run_gate() -> dict[str, Any]:
    ...
    if mutation_names != EXPECTED_MUTATION_NAMES:
        raise AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError("mutation case order/name drift")
...
def evaluate_mutation_results(fused_envelope: dict[str, Any], source_input: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for name, mutated, run_native in mutation_cases(fused_envelope):
        ...
        except AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError as err:
...
def validate_result(result: dict[str, Any]) -> None:
    ...
    if missing:
        raise AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError(f"missing result keys: {sorted(missing)}")
    ...
    for item in mutation_results:
        ...
        if set(item) != {"name", "rejected", "error"}:
            raise AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError("mutation result schema drift")
    mutation_names = tuple(item["name"] for item in mutation_results)
    if mutation_names != EXPECTED_MUTATION_NAMES:
        raise AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError("mutation result name drift")
    for item in mutation_results:
        if (
            ...
            raise AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError("mutation result rejection drift")
...
def write_tsv(path: pathlib.Path, result: dict[str, Any]) -> None:
    ...
    try:
        ...
        if rows != [expected_row]:
            raise AttentionKvD16TwoHeadLongseqFusedSoftmaxTableGateError("TSV round-trip drift")
        tmp_path.replace(path)
    except Exception:
        tmp_path.unlink(missing_ok=True)
        raise

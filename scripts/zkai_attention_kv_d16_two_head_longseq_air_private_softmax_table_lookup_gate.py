#!/usr/bin/env python3
"""Checked AIR-constrained d16 two-head long-sequence Softmax-table lookup sidecar gate for issue #525.

This gate records the narrow positive result: the existing native d16 two-head long-sequence
...
LOOKUP_ENVELOPE_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d16-two-head-longseq-softmax-table-logup-sidecar-proof-2026-05.envelope.json"
SOURCE_INPUT_SCRIPT = ROOT / "scripts" / "zkai_attention_kv_stwo_native_d16_two_head_longseq_bounded_softmax_table_proof_input.py"
JSON_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d16-two-head-longseq-softmax-table-logup-sidecar-gate-2026-05.json"
TSV_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d16-two-head-longseq-softmax-table-logup-sidecar-gate-2026-05.tsv"

MAX_SOURCE_INPUT_JSON_BYTES = 1_048_576
MAX_LOOKUP_ENVELOPE_JSON_BYTES = 4_194_304

SCHEMA = "zkai-attention-kv-stwo-native-d16-two-head-longseq-softmax-table-logup-sidecar-gate-v1"
ISSUE = 525
SOURCE_ISSUE = 525
DECISION = "GO_NATIVE_STWO_AIR_CONSTRAINED_D16_TWO_HEAD_LONGSEQ_SOFTMAX_TABLE_LOOKUP_RELATION_SIDECAR"
CLAIM_BOUNDARY = (
    "NATIVE_STWO_D16_TWO_HEAD_LONGSEQ_BOUNDED_SOFTMAX_TABLE_LOOKUP_MEMBERSHIP_CONSTRAINED_BY_LOGUP_SIDECAR_"
...
LOOKUP_PROOF_VERSION = "stwo-attention-kv-d16-two-head-longseq-softmax-table-logup-sidecar-proof-v1"
LOOKUP_STATEMENT_VERSION = "zkai-attention-kv-stwo-native-d16-two-head-longseq-softmax-table-logup-sidecar-statement-v1"
LOOKUP_SEMANTIC_SCOPE = "d16_two_head_longseq_bounded_softmax_table_membership_constrained_by_native_stwo_logup_sidecar"
LOOKUP_VERIFIER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d16-two-head-longseq-softmax-table-logup-sidecar:v1"
LOOKUP_TARGET_ID = "attention-kv-d16-two-head-longseq-causal-mask-bounded-softmax-table-logup-sidecar-v1"
LOOKUP_RELATION = "AttentionKvD16TwoHeadLongseqSoftmaxTableLookupRelation"
LOOKUP_RELATION_WIDTH = 2
LOOKUP_PROOF_SIZE_BYTES = 24_828
LOOKUP_ENVELOPE_SIZE_BYTES = 1_088_326
LOOKUP_PROOF_COMMITMENTS = 4
LOOKUP_TRACE_COMMITMENTS = 3
LOOKUP_TABLE_MULTIPLICITIES = (
    {"gap": 0, "weight": 256, "multiplicity": 41},
    ...
    {"gap": 8, "weight": 16, "multiplicity": 278},
)
...
PROOF_SIZE_RATIO = "1.372623"
ENVELOPE_SIZE_RATIO = "2.479261"
...
VALIDATION_COMMANDS = (
    "cargo +nightly-2025-07-14 test --locked attention_kv_d16_two_head_longseq_softmax_table_lookup --lib --features stwo-backend",
    "cargo +nightly-2025-07-14 run --locked --features stwo-backend --bin zkai_attention_kv_native_d16_two_head_longseq_softmax_table_lookup_proof -- prove docs/engineering/evidence/zkai-attention-kv-stwo-native-d16-two-head-longseq-bounded-softmax-table-proof-2026-05.json docs/engineering/evidence/zkai-attention-kv-stwo-native-d16-two-head-longseq-softmax-table-logup-sidecar-proof-2026-05.envelope.json",
    "cargo +nightly-2025-07-14 run --locked --features stwo-backend --bin zkai_attention_kv_native_d16_two_head_longseq_softmax_table_lookup_proof -- verify docs/engineering/evidence/zkai-attention-kv-stwo-native-d16-two-head-longseq-softmax-table-logup-sidecar-proof-2026-05.envelope.json",
    "python3 scripts/zkai_attention_kv_d16_two_head_longseq_air_private_softmax_table_lookup_gate.py --write-json docs/engineering/evidence/zkai-attention-kv-stwo-native-d16-two-head-longseq-softmax-table-logup-sidecar-gate-2026-05.json --write-tsv docs/engineering/evidence/zkai-attention-kv-stwo-native-d16-two-head-longseq-softmax-table-logup-sidecar-gate-2026-05.tsv",
...
def verify_lookup_envelope_bytes_with_native_cli(envelope_bytes: bytes, label: str) -> None:
    ...
    command = [
        ...
        "stwo-backend",
        "--bin",
        "zkai_attention_kv_native_d16_two_head_longseq_softmax_table_lookup_proof",
        "--",
        "verify",
        str(tmp_path),
    ]
...
def build_payload() -> dict[str, Any]:
    ...
    payload["gate_commitment"] = blake2b_commitment(
        ...
        "ptvm:zkai:attention-kv-d16-two-head-longseq-softmax-table-logup-sidecar-gate:v1",
    )

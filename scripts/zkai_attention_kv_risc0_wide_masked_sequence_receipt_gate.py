#!/usr/bin/env python3
"""Generate or verify a RISC Zero receipt for a wide masked carried attention/KV sequence.

This gate answers issue #446 for the wide masked sequence route. It extends
issue #444 from a fixed d=2/no-mask sequence to an eight-transition d=8
causal-prefix carried KV-cache sequence. The guest computes every integer-argmax
attention step and commits a journal containing every intermediate cache row, so
deletion, reordering, or relabeling of intermediate state is rejected by the
checked artifact gate.

The claim is intentionally narrow: RISC Zero proves one fixed eight-step d=8
integer-argmax attention/KV sequence with causal-prefix masking. It is not native
Stwo, not Softmax, not full inference, and not recursion or PCD.
"""
...
PROGRAM_DIR = ROOT / "programs" / "risc0-attention-kv-wide-masked-sequence-receipt"
PROGRAM_MANIFEST = PROGRAM_DIR / "Cargo.toml"
RECEIPT_OUT = EVIDENCE_DIR / "zkai-attention-kv-risc0-wide-masked-sequence-receipt-2026-05.bincode"
JSON_OUT = EVIDENCE_DIR / "zkai-attention-kv-risc0-wide-masked-sequence-receipt-2026-05.json"
TSV_OUT = EVIDENCE_DIR / "zkai-attention-kv-risc0-wide-masked-sequence-receipt-2026-05.tsv"

SCHEMA = "zkai-attention-kv-risc0-wide-masked-sequence-receipt-gate-v1"
ISSUE = 446
SOURCE_ISSUE = 444
RESULT = "GO"
DECISION = "GO_ATTENTION_KV_RISC0_WIDE_MASKED_SEQUENCE_RECEIPT_FOR_CARRIED_KV_STATE"
CLAIM_BOUNDARY = (
    "RISC0_RECEIPT_PROVES_EIGHT_STEP_D8_CAUSAL_PREFIX_INTEGER_ARGMAX_ATTENTION_KV_SEQUENCE_"
    "NOT_STWO_NOT_SOFTMAX_NOT_RECURSION_OR_PCD_NOT_LONG_CONTEXT_OR_FULL_INFERENCE_NOT_AGENT_CORRECTNESS"
)
ROUTE_ID = "risc0_attention_kv_wide_masked_sequence_semantics_receipt"
SYSTEM = "RISC Zero"
JOURNAL_SCHEMA = "zkai-attention-kv-risc0-wide-masked-sequence-journal-v1"
SEMANTICS = "tiny-single-head-integer-argmax-attention-wide-masked-sequence-v1"
...
GO_CRITERION = (
    ...
    "d=8 causal-prefix integer-argmax attention/KV sequence and rejects deletion, reordering, or relabeling of intermediate state"
)
NON_CLAIMS = [
    ...
    "not native Stwo proving; this is an external zkVM control for the d=8/masking route",
]
VALIDATION_COMMANDS = [
    "just gate-fast",
    "PATH=\"$HOME/.risc0/bin:$HOME/.cargo/bin:$PATH\" CARGO_TARGET_DIR=target/risc0-attention-kv-wide-masked-sequence-receipt cargo test --manifest-path programs/risc0-attention-kv-wide-masked-sequence-receipt/Cargo.toml",
    "PATH=\"$HOME/.risc0/bin:$HOME/.cargo/bin:$PATH\" python3 scripts/zkai_attention_kv_risc0_wide_masked_sequence_receipt_gate.py --verify-existing --write-json target/zkai-attention-kv-risc0-wide-masked-sequence-receipt-verify.json --write-tsv target/zkai-attention-kv-risc0-wide-masked-sequence-receipt-verify.tsv",
    "PATH=\"$HOME/.risc0/bin:$HOME/.cargo/bin:$PATH\" python3 -m unittest scripts.tests.test_zkai_attention_kv_risc0_wide_masked_sequence_receipt_gate",
    "python3 -m py_compile scripts/zkai_attention_kv_risc0_wide_masked_sequence_receipt_gate.py scripts/tests/test_zkai_attention_kv_risc0_wide_masked_sequence_receipt_gate.py",
...
class AttentionKvRisc0WideMaskedSequenceReceiptError(ValueError):
...
def require_object(value: Any, label: str, *, layer: str = "parser_or_schema") -> dict[str, Any]:
    if not isinstance(value, dict):
        raise AttentionKvRisc0WideMaskedSequenceReceiptError(f"{label} must be an object", layer=layer)
    return value


def require_list(value: Any, label: str, *, layer: str = "parser_or_schema") -> list[Any]:
    if not isinstance(value, list):
        raise AttentionKvRisc0WideMaskedSequenceReceiptError(f"{label} must be a list", layer=layer)
    return value


def expect_equal(actual: Any, expected: Any, label: str, *, layer: str = "parser_or_schema") -> None:
    if actual != expected:
        raise AttentionKvRisc0WideMaskedSequenceReceiptError(f"{label} mismatch", layer=layer)


def expect_keys(value: dict[str, Any], expected: set[str], label: str, *, layer: str = "parser_or_schema") -> None:
    keys = set(value)
    if keys != expected:
        raise AttentionKvRisc0WideMaskedSequenceReceiptError(
...
def _resolved_under_root(path: pathlib.Path, *, label: str, layer: str) -> pathlib.Path:
    ...
    try:
        ...
        raise AttentionKvRisc0WideMaskedSequenceReceiptError(f"{label} path escapes repository root", layer=layer) from err
    return resolved
...
def load_json(path: pathlib.Path, *, layer: str = "parser_or_schema") -> Any:
    try:
        ...
        raise AttentionKvRisc0WideMaskedSequenceReceiptError(f"failed to load JSON {path}: {err}", layer=layer) from err


def sequence_fixture() -> dict[str, Any]:
    return {
        "case_id": "tiny_single_head_argmax_kv_wide_masked_sequence_v1",
        "model_config": {
        ...
        "initial_kv_cache": [
            {"position": 0, "key": [1, 0, 0, 0, 1, -1, 0, 2], "value": [2, 1, 0, -1, 3, 0, 1, 2]},
            {"position": 1, "key": [0, 1, 1, -1, 0, 2, -1, 0], "value": [-1, 3, 2, 0, 1, -2, 4, 0]},
        ],
        "input_steps": [
            {"token_position": 2, "query": [1, 1, 0, 0, 1, 0, -1, 1], "new_key": [1, -1, 0, 2, 0, 1, 0, -1], "new_value": [4, 2, 1, 0, -1, 3, 2, 1]},
            {"token_position": 3, "query": [2, -1, 1, 0, 0, 1, 1, -1], "new_key": [0, 2, -1, 1, 1, 0, -2, 1], "new_value": [5, -2, 0, 3, 1, 1, -1, 2]},
            {"token_position": 4, "query": [-1, 2, 0, 1, 1, -1, 0, 2], "new_key": [2, 1, 1, -1, 0, 2, 1, 0], "new_value": [0, 6, -2, 1, 3, -1, 2, 4]},
            {"token_position": 5, "query": [3, 0, -1, 1, 0, 2, -2, 1], "new_key": [-1, 3, 0, 0, 2, -1, 1, 2], "new_value": [7, 1, 2, -2, 0, 5, -3, 1]},
            {"token_position": 6, "query": [0, 3, 2, -1, 1, 0, 1, -2], "new_key": [3, -2, 1, 2, -1, 1, 0, 1], "new_value": [-3, 4, 1, 2, 6, 0, -1, 3]},
            {"token_position": 7, "query": [2, 2, -1, 0, 3, -1, 1, 1], "new_key": [-2, -1, 2, 1, 0, 3, -1, 2], "new_value": [6, 6, -2, 0, 2, 1, 3, -1]},
            {"token_position": 8, "query": [-2, 1, 3, 1, -1, 2, 0, 1], "new_key": [1, 3, -2, 0, 2, -1, 1, 1], "new_value": [8, -1, 3, 2, -2, 4, 0, 1]},
            {"token_position": 9, "query": [1, -3, 0, 2, 2, -1, 1, 0], "new_key": [2, -2, 1, 3, -1, 0, 2, -1], "new_value": [-5, 5, 1, -3, 4, 2, -2, 0]},
        ],
    }
...
def dot(lhs: list[int], rhs: list[int]) -> int:
    if len(lhs) != len(rhs):
        raise AttentionKvRisc0WideMaskedSequenceReceiptError("dot-product width mismatch", layer="sequence_journal")
...
def apply_step(step_index: int, prior_kv_cache: list[dict[str, Any]], input_step: dict[str, Any]) -> dict[str, Any]:
    ...
    scores = [
        ...
        if item["position"] <= input_step["token_position"]
    ]
...
def transition_commitments(journal: dict[str, Any] | None = None) -> list[str]:
    ...
    return [
        blake2b_commitment(row, f"ptvm:zkai:attention-kv-wide-masked-sequence-transition:{row['step_index']}:v1")
        for row in journal["transitions"]
    ]


def journal_commitment(journal: dict[str, Any] | None = None) -> str:
    return blake2b_commitment(journal or expected_journal(), "ptvm:zkai:attention-kv-risc0-wide-masked-sequence-journal:v1")


def statement_fields(journal: dict[str, Any], receipt_commitment: str, image_id_hex: str) -> dict[str, Any]:
    fixture = sequence_fixture()
    fields = {
        "verifier_domain": "ptvm:zkai:attention-kv-wide-masked-sequence:risc0:v1",
        "statement_kind": "attention-kv-cache-wide-masked-sequence",
        ...
        "outputs_commitment": blake2b_commitment(
            [row["attention_output"] for row in journal["transitions"]], "ptvm:zkai:attention-wide-masked-sequence-outputs:v1"
        ),
    ...
    fields["public_instance_commitment"] = blake2b_commitment(
        ...
        "ptvm:zkai:attention-kv-wide-masked-sequence-public-instance:v1",
    )
    fields["statement_commitment"] = blake2b_commitment(
        {key: value for key, value in fields.items() if key not in {"statement_commitment"}},
        "ptvm:zkai:attention-kv-wide-masked-sequence-statement:v1",
    )
    return fields
...
def risc0_env() -> dict[str, str]:
    ...
    env.setdefault("CARGO_TARGET_DIR", str(ROOT / "target" / "risc0-attention-kv-wide-masked-sequence-receipt"))
...
def toolchain_probe() -> dict[str, Any]:
    ...
    return {
        "probe_scope": "local_risc0_attention_kv_wide_masked_sequence_receipt_generation_and_verification",
...
def local_risc0_toolchain_available() -> tuple[bool, str]:
    ...
    except (AttentionKvRisc0WideMaskedSequenceReceiptError, OSError, subprocess.SubprocessError) as err:
...
def generate_or_verify_receipt(*, prove: bool, receipt_path: pathlib.Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(dir=target_root) as raw_tmp:
        ...
        input_path = tmp / "attention-wide-masked-sequence-input.json"
        ...
        if not receipt_path.is_file():
            raise AttentionKvRisc0WideMaskedSequenceReceiptError(f"receipt artifact is missing: {receipt_path}", layer="receipt_artifact")
...
def build_payload(*, prove: bool = False, receipt_path: pathlib.Path = RECEIPT_OUT, previous_evidence: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if not receipt_bytes or len(receipt_bytes) > MAX_RECEIPT_BYTES:
        raise AttentionKvRisc0WideMaskedSequenceReceiptError("receipt artifact size outside allowed bound", layer="receipt_artifact")
...
def mutation_cases(payload: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    def add(name: str, surface: str, mutate: Any) -> dict[str, Any]:
        ...
        except AttentionKvRisc0WideMaskedSequenceReceiptError as err:
    ...
    return [
        ...
        add("intermediate_prior_kv_relabeling", "sequence_journal", lambda p: p["journal"]["transitions"][1]["prior_kv_cache"][0].__setitem__("value", [99, 99, 99, 99, 99, 99, 99, 99])),
        add("intermediate_next_kv_relabeling", "sequence_journal", lambda p: p["journal"]["transitions"][1]["next_kv_cache"][2].__setitem__("key", [9, 9, 9, 9, 9, 9, 9, 9])),
        add("intermediate_input_query_relabeling", "sequence_journal", lambda p: p["journal"]["transitions"][1]["input_step"].__setitem__("query", [9, 9, 9, 9, 9, 9, 9, 9])),
        add("intermediate_attention_output_relabeling", "sequence_journal", lambda p: p["journal"]["transitions"][1].__setitem__("attention_output", [0, 0, 0, 0, 0, 0, 0, 0])),
        add("intermediate_score_trace_relabeling", "sequence_journal", lambda p: p["journal"]["transitions"][1]["scores"][0].__setitem__("score", 999)),
        add("initial_kv_cache_relabeling", "sequence_journal", lambda p: p["journal"]["initial_kv_cache"][0].__setitem__("key", [7, 7, 7, 7, 7, 7, 7, 7])),
        add("final_kv_cache_relabeling", "sequence_journal", lambda p: p["journal"]["final_kv_cache"][3].__setitem__("value", [8, 8, 8, 8, 8, 8, 8, 8])),
...
def validate_core_payload(payload: dict[str, Any], *, strict_receipt: bool = False) -> None:
    ...
    if not isinstance(artifact["path"], str):
        raise AttentionKvRisc0WideMaskedSequenceReceiptError("receipt artifact path must be a string", layer="receipt_artifact")
    ...
    if not receipt_path.is_file():
        raise AttentionKvRisc0WideMaskedSequenceReceiptError("receipt artifact missing", layer="receipt_artifact")
    receipt_bytes = receipt_path.read_bytes()
    if not receipt_bytes or len(receipt_bytes) > MAX_RECEIPT_BYTES:
        raise AttentionKvRisc0WideMaskedSequenceReceiptError("receipt artifact size outside allowed bound", layer="receipt_artifact")
    ...
    expect_equal(verification["host_summary_schema"], "zkai-attention-kv-risc0-wide-masked-sequence-host-summary-v1", "host summary schema", layer="receipt_metadata")
    ...
    if not isinstance(verification["image_id_hex"], str) or len(verification["image_id_hex"]) != 64:
        raise AttentionKvRisc0WideMaskedSequenceReceiptError("image id must be 32-byte hex", layer="receipt_metadata")
    ...
    if strict_receipt:
        ...
        expect_equal(strict_summary["schema"], "zkai-attention-kv-risc0-wide-masked-sequence-host-summary-v1", "strict host schema", layer="risc0_host")
    ...
    expect_equal(probe["probe_scope"], "local_risc0_attention_kv_wide_masked_sequence_receipt_generation_and_verification", "probe scope", layer="toolchain_probe")
    ...
    for command_id in REQUIRED_COMMANDS:
        ...
        if entry.get("available") is not True:
            raise AttentionKvRisc0WideMaskedSequenceReceiptError(f"required command {command_id} unavailable", layer="toolchain_probe")
    ...
    if metrics["proof_generation_time_source"] not in {"current_prove_run", "carried_from_existing_evidence_not_remeasured", "not_remeasured_in_verify_existing"}:
        raise AttentionKvRisc0WideMaskedSequenceReceiptError("proof_generation_time_source mismatch", layer="proof_metrics")
    if metrics["proof_generation_time_source"] == "not_remeasured_in_verify_existing":
        ...
        raise AttentionKvRisc0WideMaskedSequenceReceiptError("proof_generation_time_ms must be positive", layer="proof_metrics")
    if not isinstance(metrics["verifier_time_ms"], (int, float)) or metrics["verifier_time_ms"] <= 0:
        raise AttentionKvRisc0WideMaskedSequenceReceiptError("verifier_time_ms must be positive", layer="proof_metrics")
...
def validate_payload(payload: Any, *, strict_receipt: bool = False) -> None:
    ...
    if payload["all_mutations_rejected"] is not True:
        raise AttentionKvRisc0WideMaskedSequenceReceiptError("not all attention/KV sequence receipt mutations rejected", layer="mutation_suite")
...
def write_text_checked(path: pathlib.Path, text: str) -> None:
    ...
    if resolved.exists() and resolved.is_dir():
        raise AttentionKvRisc0WideMaskedSequenceReceiptError("output path must be a file, not a directory", layer="output_path")
...
def previous_evidence_path_for_verify(write_json_path: pathlib.Path) -> pathlib.Path:
    """Use the canonical checked evidence as the source of carried prove-time metrics."""
    if write_json_path.resolve() == JSON_OUT.resolve():
...
def main() -> int:
    ...
    if receipt_path is None:
        raise AttentionKvRisc0WideMaskedSequenceReceiptError("receipt path is required", layer="output_path")
    receipt_path = require_allowed_receipt_path(receipt_path, label="receipt", layer="output_path")
    ...
    if args.verify_existing:
        ...
        previous_json_path = previous_evidence_path_for_verify(json_path)

#!/usr/bin/env python3
"""Checked fused native Stwo eight-head bounded Softmax-table gate for issue #496.
...
LogUp table-membership for a deterministic eight-head d=8 fixture. It remains
...
SOURCE_INPUT_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-eight-head-bounded-softmax-table-proof-2026-05.json"
FUSED_ENVELOPE_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-eight-head-fused-softmax-table-proof-2026-05.envelope.json"
SOURCE_INPUT_SCRIPT = ROOT / "scripts" / "zkai_attention_kv_stwo_native_eight_head_bounded_softmax_table_proof_input.py"
JSON_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-eight-head-fused-softmax-table-gate-2026-05.json"
TSV_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-eight-head-fused-softmax-table-gate-2026-05.tsv"

MAX_SOURCE_INPUT_JSON_BYTES = 2_097_152
MAX_FUSED_ENVELOPE_JSON_BYTES = 4_194_304
FUSED_VERIFY_TIMEOUT_SECONDS = 300

SCHEMA = "zkai-attention-kv-stwo-native-eight-head-fused-softmax-table-gate-v1"
ISSUE = 496
SOURCE_ISSUE = 496
SIDECAR_ISSUE = 514
...
CLAIM_BOUNDARY = (
    "ONE_NATIVE_STWO_EIGHT_HEAD_BOUNDED_SOFTMAX_TABLE_ATTENTION_PROOF_WITH_LOGUP_TABLE_MEMBERSHIP_"
...
NON_FUSED_STATUS = "GO_MATCHED_EIGHT_HEAD_SOURCE_PLUS_LOGUP_SIDECAR_COMPARATOR_RECORDED"
TIMING_POLICY = "proof_existence_and_byte_accounting_only_not_public_benchmark"

SOURCE_PROOF_SIZE_BYTES = 52_392
SOURCE_ENVELOPE_SIZE_BYTES = 1_151_543
SOURCE_PLUS_SIDECAR_RAW_PROOF_BYTES = 74_086
FUSED_PROOF_SIZE_BYTES = 59_375
FUSED_ENVELOPE_SIZE_BYTES = 1_210_413
FUSED_OVER_SOURCE_PROOF_BYTES = FUSED_PROOF_SIZE_BYTES - SOURCE_PROOF_SIZE_BYTES
FUSED_SAVES_VS_SOURCE_PLUS_SIDECAR_BYTES = 14_711
FUSED_TO_SOURCE_PLUS_SIDECAR_RATIO = 0.801433

SOURCE_STATEMENT_COMMITMENT = "blake2b-256:7356acc1a770a9cc5e9e952cf4d1f7e0315c2a20ad167f184397dcb0539f33e0"
SOURCE_PUBLIC_INSTANCE_COMMITMENT = "blake2b-256:5db2d62c333a4d3636e155e7cdb4060b7f9e74d4497d516623fba47dc4f84e4f"
SOURCE_SCORE_ROW_COMMITMENT = "blake2b-256:c3f172e88b7fac3b28f40fe72c07fc240f51c778907f4714940bee5c41154e81"
SOURCE_FINAL_KV_CACHE_COMMITMENT = "blake2b-256:e46608cd8f539f865d00b32f371e4d2d9567f7917c2304eb263ed2f6fb99ee09"
SOURCE_OUTPUTS_COMMITMENT = "blake2b-256:45ebc4cebb88e1ea1a4ff7ebbf571e4309b82a826ecc3a76f91b5d9cf33846c6"
SOURCE_WEIGHT_TABLE_COMMITMENT = "blake2b-256:3feb652428a7a093407684fbbb1dce7825fc33ecd5438a0be7cc121ff0135268"
SOURCE_HEAD_COUNT = 8
SOURCE_WEIGHT_POLICY = "exp2_half_gap_table_clipped_8_floor_division"
SOURCE_SCORE_GAP_CLIP = 8
SOURCE_SCORE_ROWS = 416
...
SOURCE_SEMANTIC_SCOPE = "eight_head_d8_bounded_table_softmax_approx_attention_kv_causal_mask_rows_bound_to_statement_receipt"
SOURCE_DECISION = "GO_STWO_NATIVE_ATTENTION_KV_EIGHT_HEAD_BOUNDED_SOFTMAX_TABLE_AIR_PROOF"
SOURCE_INPUT_DECISION = "GO_INPUT_FOR_STWO_NATIVE_ATTENTION_KV_EIGHT_HEAD_BOUNDED_SOFTMAX_TABLE_AIR_PROOF"
SOURCE_TARGET_ID = "attention-kv-d8-causal-mask-eight-head-bounded-softmax-table-v1"
SOURCE_VERIFIER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-eight-head-bounded-softmax-table:v1"
LOOKUP_RELATION = "AttentionKvEightHeadFusedSoftmaxTableRelation"
LOOKUP_RELATION_WIDTH = 2
FUSED_BACKEND_VERSION = "stwo-attention-kv-eight-head-fused-bounded-softmax-table-logup-v1"
FUSED_PROOF_SCHEMA_VERSION = "stwo-attention-kv-eight-head-fused-bounded-softmax-table-logup-proof-v1"
FUSED_STATEMENT_VERSION = "zkai-attention-kv-stwo-native-eight-head-fused-softmax-table-logup-statement-v1"
FUSED_SEMANTIC_SCOPE = "eight_head_d8_bounded_softmax_table_attention_arithmetic_and_logup_membership_fused_in_one_native_stwo_proof"
FUSED_TARGET_ID = "attention-kv-eight-head-d8-causal-mask-fused-bounded-softmax-table-logup-v1"
FUSED_VERIFIER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-eight-head-fused-bounded-softmax-table-logup:v1"
...
TABLE_MULTIPLICITIES = (
    ...
    {"gap": 5, "weight": 45, "multiplicity": 5},
    {"gap": 6, "weight": 32, "multiplicity": 1},
    {"gap": 7, "weight": 23, "multiplicity": 2},
    {"gap": 8, "weight": 16, "multiplicity": 323},
)
NON_CLAIMS = (
...
VALIDATION_COMMANDS = (
    ...
    "cargo +nightly-2025-07-14 run --locked --features stwo-backend --bin zkai_attention_kv_native_eight_head_fused_softmax_table_proof -- prove docs/engineering/evidence/zkai-attention-kv-stwo-native-eight-head-bounded-softmax-table-proof-2026-05.json docs/engineering/evidence/zkai-attention-kv-stwo-native-eight-head-fused-softmax-table-proof-2026-05.envelope.json",
    ...
    "python3 scripts/zkai_attention_kv_eight_head_fused_softmax_table_native_gate.py --write-json docs/engineering/evidence/zkai-attention-kv-stwo-native-eight-head-fused-softmax-table-gate-2026-05.json --write-tsv docs/engineering/evidence/zkai-attention-kv-stwo-native-eight-head-fused-softmax-table-gate-2026-05.tsv",
...
class AttentionKvEightHeadFusedSoftmaxTableGateError(ValueError):
    pass
...
def load_script_module(path: pathlib.Path, module_name: str) -> ModuleType:
    ...
    if spec is None or spec.loader is None:
        raise AttentionKvEightHeadFusedSoftmaxTableGateError(f"failed to load {module_name}: {path}")
...
SOURCE_INPUT_MODULE = load_script_module(
    SOURCE_INPUT_SCRIPT, "zkai_attention_kv_stwo_native_eight_head_bounded_softmax_table_proof_input"
)


def read_bounded_bytes(path: pathlib.Path, max_bytes: int, label: str) -> bytes:
    if not path.is_file():
        raise AttentionKvEightHeadFusedSoftmaxTableGateError(f"missing {label}: {path}")
    size = path.stat().st_size
    if size <= 0 or size > max_bytes:
        raise AttentionKvEightHeadFusedSoftmaxTableGateError(f"{label} size drift: got {size}, max {max_bytes}")
...
def read_bounded_json(path: pathlib.Path, max_bytes: int, label: str) -> Any:
    ...
    try:
        ...
        raise AttentionKvEightHeadFusedSoftmaxTableGateError(f"{label} is not JSON: {err}") from err


def assert_exact_keys(mapping: dict[str, Any], expected_keys: set[str], label: str) -> None:
    actual = set(mapping)
    if actual != expected_keys:
        raise AttentionKvEightHeadFusedSoftmaxTableGateError(
...
def proof_bytes(envelope: dict[str, Any]) -> bytes:
    ...
    if not isinstance(proof, list):
        raise AttentionKvEightHeadFusedSoftmaxTableGateError("proof must be a byte list")
    if len(proof) != FUSED_PROOF_SIZE_BYTES:
        raise AttentionKvEightHeadFusedSoftmaxTableGateError("proof byte length drift")
    if any(not isinstance(byte, int) or isinstance(byte, bool) or byte < 0 or byte > 255 for byte in proof):
        raise AttentionKvEightHeadFusedSoftmaxTableGateError("proof byte outside uint8 range")
    return bytes(proof)
...
def mutate_same_size_stark_proof_commitment(envelope: dict[str, Any]) -> None:
    ...
    if not isinstance(proof, list) or not proof:
        raise AttentionKvEightHeadFusedSoftmaxTableGateError("cannot mutate missing proof bytes")
...
def assert_native_verifier_summary(mapping: dict[str, Any], expected: dict[str, Any], label: str) -> None:
    ...
    if not isinstance(schema, str) or not schema.endswith("-cli-summary-v1"):
        raise AttentionKvEightHeadFusedSoftmaxTableGateError(f"{label} schema drift")
    path = mapping.get("fused_envelope_path")
    if not isinstance(path, str) or not path.endswith(".json"):
        raise AttentionKvEightHeadFusedSoftmaxTableGateError(f"{label} fused envelope path drift")
...
def verify_fused_envelope_bytes_with_native_cli(envelope_bytes: bytes) -> None:
    if len(envelope_bytes) <= 0 or len(envelope_bytes) > MAX_FUSED_ENVELOPE_JSON_BYTES:
        raise AttentionKvEightHeadFusedSoftmaxTableGateError("fused envelope byte size drift")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with subprocess.Popen(
            [
                ...
                "stwo-backend",
                "--bin",
                "zkai_attention_kv_native_eight_head_fused_softmax_table_proof",
                "--",
                "verify",
                str(envelope_path),
            ],
    ...
    *,
    ...
    assert_fields(
        envelope,
        {
            ...
            "decision": DECISION.replace("EIGHT_HEAD_", ""),
    ...
    if envelope.get("source_input") != source_input:
        raise AttentionKvEightHeadFusedSoftmaxTableGateError("fused source input split-brain drift")
    if envelope.get("fused_summary") != expected_summary(source_input):
        raise AttentionKvEightHeadFusedSoftmaxTableGateError("fused summary drift")
...
def validate_result(result: dict[str, Any], envelope: dict[str, Any], source_input: dict[str, Any]) -> None:
    ...
    for key, value in expected.items():
        ...
        if result.get(key) != value:
            raise AttentionKvEightHeadFusedSoftmaxTableGateError(f"result drift for {key}")
    ...
    if not isinstance(mutation_results, list) or len(mutation_results) != EXPECTED_MUTATION_COUNT:
        raise AttentionKvEightHeadFusedSoftmaxTableGateError("mutation result shape drift")
    if tuple(item.get("name") for item in mutation_results if isinstance(item, dict)) != EXPECTED_MUTATION_NAMES:
        raise AttentionKvEightHeadFusedSoftmaxTableGateError("mutation result name drift")
    if any(item.get("rejected") is not True for item in mutation_results if isinstance(item, dict)):
        raise AttentionKvEightHeadFusedSoftmaxTableGateError("mutation accepted")

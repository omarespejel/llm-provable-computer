#!/usr/bin/env python3
"""Checked fused native Stwo d8 bounded Softmax-table gate for issue #478.
...
SOURCE_INPUT_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d8-bounded-softmax-table-proof-2026-05.json"
SOURCE_ENVELOPE_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d8-bounded-softmax-table-proof-2026-05.envelope.json"
SIDECAR_ENVELOPE_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d8-softmax-table-logup-sidecar-proof-2026-05.envelope.json"
FUSED_ENVELOPE_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d8-fused-softmax-table-proof-2026-05.envelope.json"
SOURCE_INPUT_SCRIPT = ROOT / "scripts" / "zkai_attention_kv_stwo_native_d8_bounded_softmax_table_proof_input.py"
JSON_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d8-fused-softmax-table-gate-2026-05.json"
TSV_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d8-fused-softmax-table-gate-2026-05.tsv"
...
CLAIM_BOUNDARY = (
    "ONE_NATIVE_STWO_D8_BOUNDED_SOFTMAX_TABLE_ATTENTION_PROOF_WITH_LOGUP_TABLE_MEMBERSHIP_"
...
SOURCE_PROOF_SIZE_BYTES = 44_692
SOURCE_ENVELOPE_SIZE_BYTES = 451_982
SIDECAR_PROOF_SIZE_BYTES = 14_745
...
SOURCE_PUBLIC_INSTANCE_COMMITMENT = "blake2b-256:24007e2e093726eaeabfefc32de4b0633babe0f8ccb80945036dd93579f7d05c"
SOURCE_SCORE_ROW_COMMITMENT = "blake2b-256:1279d23d93288d6ddce174aaae45b895f8c0ba690754c0a3035a84a556efb5ec"
SOURCE_WEIGHT_TABLE_COMMITMENT = "blake2b-256:8c45ca7eec1032a0ffa5d5a1e842bebd5f6268d75f82d696b29ec7cf9a420e13"
...
FUSED_VERIFIER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d8-fused-bounded-softmax-table-logup:v1"
...
TABLE_MULTIPLICITIES = (
    ...
    {"gap": 8, "weight": 16, "multiplicity": 29},
)
NON_CLAIMS = (
...
VALIDATION_COMMANDS = (
    ...
    "cargo +nightly-2025-07-14 run --features stwo-backend --bin zkai_attention_kv_native_d8_fused_softmax_table_proof -- prove docs/engineering/evidence/zkai-attention-kv-stwo-native-d8-bounded-softmax-table-proof-2026-05.json docs/engineering/evidence/zkai-attention-kv-stwo-native-d8-fused-softmax-table-proof-2026-05.envelope.json",
    ...
    "python3 scripts/zkai_attention_kv_d8_fused_softmax_table_native_gate.py --write-json docs/engineering/evidence/zkai-attention-kv-stwo-native-d8-fused-softmax-table-gate-2026-05.json --write-tsv docs/engineering/evidence/zkai-attention-kv-stwo-native-d8-fused-softmax-table-gate-2026-05.tsv",
    "python3 -m unittest scripts.tests.test_zkai_attention_kv_d8_fused_softmax_table_native_gate",
    "just lib",
    "just gate-fast",
    "just gate",
)
...
class AttentionKvD8FusedSoftmaxTableGateError(ValueError):
    pass
...
def load_script_module(path: pathlib.Path, module_name: str) -> ModuleType:
    ...
    if spec is None or spec.loader is None:
        raise AttentionKvD8FusedSoftmaxTableGateError(f"failed to load {module_name}: {path}")
...
SOURCE_INPUT_MODULE = load_script_module(
    SOURCE_INPUT_SCRIPT, "zkai_attention_kv_stwo_native_d8_bounded_softmax_table_proof_input"
)


def read_bounded_bytes(path: pathlib.Path, max_bytes: int, label: str) -> bytes:
    if not path.is_file():
        raise AttentionKvD8FusedSoftmaxTableGateError(f"missing {label}: {path}")
    size = path.stat().st_size
    if size <= 0 or size > max_bytes:
        raise AttentionKvD8FusedSoftmaxTableGateError(f"{label} size drift: got {size}, max {max_bytes}")
    try:
        raw = path.read_bytes()
    except OSError as err:
        raise AttentionKvD8FusedSoftmaxTableGateError(f"failed to read {label}: {err}") from err
    if len(raw) != size:
        raise AttentionKvD8FusedSoftmaxTableGateError(f"{label} read size drift: stat={size}, read={len(raw)}")
    return raw
...
def parse_stark_proof(proof_bytes: list[Any], expected_bytes: int, label: str) -> dict[str, Any]:
    if not isinstance(proof_bytes, list):
        raise AttentionKvD8FusedSoftmaxTableGateError(f"{label} proof must be a byte list")
    if len(proof_bytes) != expected_bytes:
        raise AttentionKvD8FusedSoftmaxTableGateError(f"{label} proof byte length drift")
    if any(not isinstance(byte, int) or isinstance(byte, bool) or byte < 0 or byte > 255 for byte in proof_bytes):
        raise AttentionKvD8FusedSoftmaxTableGateError(f"{label} proof bytes must be uint8 values")
    try:
        payload = json.loads(bytes(proof_bytes).decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as err:
        raise AttentionKvD8FusedSoftmaxTableGateError(f"failed to decode {label} proof payload: {err}") from err
    if set(payload) != {"stark_proof"}:
        raise AttentionKvD8FusedSoftmaxTableGateError(f"{label} proof payload schema drift")
    stark_proof = payload["stark_proof"]
    if not isinstance(stark_proof, dict):
        raise AttentionKvD8FusedSoftmaxTableGateError(f"{label} stark_proof must be an object")
...
def verify_fused_envelope_bytes_with_native_cli(envelope_bytes: bytes, label: str) -> None:
    ...
    command = [
        ...
        "stwo-backend",
        "--bin",
        "zkai_attention_kv_native_d8_fused_softmax_table_proof",
        "--",
        "verify",
        str(tmp_path),
    ]
    try:
        ...
        raise AttentionKvD8FusedSoftmaxTableGateError(f"native fused verifier failed to run for {label}: {err}") from err
    finally:
        try:
    ...
    if completed.returncode != 0:
        ...
        raise AttentionKvD8FusedSoftmaxTableGateError(f"native fused verifier rejected {label}: {suffix}")
    try:
        summary = json.loads(completed.stdout)
    except json.JSONDecodeError as err:
        raise AttentionKvD8FusedSoftmaxTableGateError(f"native fused verifier emitted malformed JSON: {err}") from err
    expected = {
        "mode": "verify",
        "verified": True,
...
def validate_source_artifacts(source_input: dict[str, Any], source_envelope: dict[str, Any], sidecar_envelope: dict[str, Any]) -> None:
    ...
    if source_envelope.get("input") != source_input:
        raise AttentionKvD8FusedSoftmaxTableGateError("source envelope/input split-brain drift")
    if len(source_envelope.get("proof", [])) != SOURCE_PROOF_SIZE_BYTES:
        raise AttentionKvD8FusedSoftmaxTableGateError("source proof size drift")
    if len(sidecar_envelope.get("proof", [])) != SIDECAR_PROOF_SIZE_BYTES:
        raise AttentionKvD8FusedSoftmaxTableGateError("sidecar proof size drift")
    lookup_summary = sidecar_envelope.get("lookup_summary")
    if not isinstance(lookup_summary, dict):
        raise AttentionKvD8FusedSoftmaxTableGateError("sidecar lookup summary missing")
    expected_sidecar = {
        "source_statement_commitment": SOURCE_STATEMENT_COMMITMENT,
    ...
    for key, expected_value in expected_sidecar.items():
        if lookup_summary.get(key) != expected_value:
            raise AttentionKvD8FusedSoftmaxTableGateError(f"sidecar summary drift for {key}")
...
def validate_fused_envelope(envelope: dict[str, Any], source_input: dict[str, Any], *, run_native: bool) -> None:
    ...
    if envelope.get("semantic_scope") != FUSED_SEMANTIC_SCOPE:
        raise AttentionKvD8FusedSoftmaxTableGateError("fused semantic scope drift")
    if envelope.get("decision") != DECISION:
        raise AttentionKvD8FusedSoftmaxTableGateError("fused decision drift")
    if envelope.get("target_id") != FUSED_TARGET_ID:
        raise AttentionKvD8FusedSoftmaxTableGateError("fused target id drift")
    if envelope.get("verifier_domain") != FUSED_VERIFIER_DOMAIN:
        raise AttentionKvD8FusedSoftmaxTableGateError("fused verifier domain drift")
    if envelope.get("source_input") != source_input:
        raise AttentionKvD8FusedSoftmaxTableGateError("fused source input split-brain drift")
    if envelope.get("fused_summary") != expected_summary(source_input):
        raise AttentionKvD8FusedSoftmaxTableGateError("fused summary drift")
...
def run_gate() -> dict[str, Any]:
    ...
    if mutation_names != EXPECTED_MUTATION_NAMES:
        raise AttentionKvD8FusedSoftmaxTableGateError("mutation case order/name drift")
    rejected = sum(1 for result in mutation_results if result["rejected"])
    if rejected != EXPECTED_MUTATION_COUNT:
        raise AttentionKvD8FusedSoftmaxTableGateError(f"mutation rejection drift: got {rejected}")

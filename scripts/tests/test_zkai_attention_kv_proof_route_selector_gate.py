...
class AttentionKvProofRouteSelectorGateTests(unittest.TestCase):
    ...
    def test_regression_issue_448_records_native_stwo_and_external_control_routes(self) -> None:
        ...
        self.assertEqual(payload["multihead_quantized_softmax_receipt"]["profiles_checked"], 3)
        self.assertEqual(payload["multihead_quantized_softmax_receipt"]["head_counts_checked"], [2, 4, 8])
        self.assertEqual(payload["multihead_quantized_softmax_receipt"]["lookup_claims_total"], 728)
        self.assertEqual(payload["multihead_quantized_softmax_receipt"]["score_rows_total"], 728)
        self.assertEqual(payload["multihead_quantized_softmax_receipt"]["fused_proof_size_bytes_sum"], 163426)
        self.assertEqual(payload["multihead_quantized_softmax_receipt"]["max_fused_proof_size_bytes"], 60450)
        ...
        self.assertEqual(len(payload["multihead_quantized_softmax_receipt"]["profile_fused_envelope_commitments"]), 3)
        self.assertEqual(len(payload["multihead_quantized_softmax_receipt"]["profile_fused_proof_commitments"]), 3)
        self.assertTrue(
            all(

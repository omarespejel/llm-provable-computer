...
BENCHMARK_PATH = ROOT / "scripts" / "zkai_stwo_statement_bound_transformer_block_benchmark.py"
SPEC = importlib.util.spec_from_file_location("zkai_stwo_statement_bound_transformer_block_benchmark", BENCHMARK_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(f"failed to load Stwo transformer-block benchmark from {BENCHMARK_PATH}")
...
class ZkAIStwoStatementBoundTransformerBlockBenchmarkTests(unittest.TestCase):
    def test_baseline_claims_transformer_block_profile(self) -> None:
        envelope = BENCH.baseline_envelope()

        self.assertEqual(envelope["statement"]["statement_kind"], "transformer-block")
        self.assertEqual(
            envelope["statement"]["model_id"],
            "urn:zkai:ptvm:rmsnorm-gated-affine-residual-block-v1",
        )
        profile = BENCH.verify_block_profile(
            envelope["stwo_proof"],
            BENCH._repo_path(BENCH.STWO_PROGRAM_PATH),
            BENCH._load_json(BENCH.ARTIFACT_DIR / "metadata.json"),
        )
        self.assertEqual(profile["logical_width"], 4)
        self.assertIn("gated_value_mix", profile["operation_ids"])
        self.assertEqual(profile["witness"]["primary_norm_sq"], 16)
        self.assertEqual(profile["witness"]["primary_inv_sqrt_q8"], 64)

    def test_block_profile_rejects_missing_operation_metadata(self) -> None:
        envelope = BENCH.baseline_envelope()
        metadata = BENCH._load_json(BENCH.ARTIFACT_DIR / "metadata.json")
        metadata["transformer_block_profile"]["operation_ids"] = [
            "rmsnorm_scale_lookup",
            "quantized_affine_projection",
            "residual_add",
        ]

        with self.assertRaisesRegex(BENCH.StwoEnvelopeError, "operation IDs"):
            BENCH.verify_block_profile(
                envelope["stwo_proof"],
                BENCH._repo_path(BENCH.STWO_PROGRAM_PATH),
                metadata,
            )

    def test_block_profile_rejects_witness_drift(self) -> None:
        envelope = BENCH.baseline_envelope()
        envelope["stwo_proof"]["claim"]["final_state"]["memory"][14] = 65

        with self.assertRaisesRegex(BENCH.StwoEnvelopeError, "primary_inv_sqrt_q8"):
            BENCH.verify_block_profile(
                envelope["stwo_proof"],
                BENCH._repo_path(BENCH.STWO_PROGRAM_PATH),
                BENCH._load_json(BENCH.ARTIFACT_DIR / "metadata.json"),
    ...
    def test_main_records_effective_argv_override(self) -> None:
        ...
        self.assertEqual(captured["command"][1], "scripts/zkai_stwo_statement_bound_transformer_block_benchmark.py")
        ...
        with mock.patch.dict(
            os.environ,
            {
                "ZKAI_STWO_BENCHMARK_COMMAND_JSON": json.dumps(
                    ["env", "python3", "scripts/zkai_stwo_statement_bound_transformer_block_benchmark.py"]
                )
            },
        ):
            self.assertEqual(

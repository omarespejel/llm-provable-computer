...
from scripts import zkai_attention_kv_d16_two_head_longseq_air_private_softmax_table_lookup_gate as gate


class AttentionKvD16TwoHeadLongseqAirPrivateSoftmaxTableLookupGateTests(unittest.TestCase):
    ...
    def test_build_payload_records_logup_sidecar_go(self):
        ...
        self.assertEqual(receipt["table_multiplicities"][-1]["multiplicity"], 278)
        self.assertEqual(payload["fixed_d16_two_head_comparison"]["fixed_d16_two_head_lookup_claims"], 104)
        self.assertEqual(payload["fixed_d16_two_head_comparison"]["proof_size_ratio"], "1.372623")
        self.assertEqual(payload["fused_comparator"]["source_plus_sidecar_raw_proof_bytes"], 108158)
        self.assertEqual(payload["fused_comparator"]["fused_saves_vs_source_plus_sidecar_bytes"], 23290)
        self.assertEqual(payload["fused_comparator"]["fused_to_source_plus_sidecar_ratio"], "0.784667")
    ...
    def test_tsv_summary_matches_payload(self):
        ...
        self.assertIn("24828", tsv)
        self.assertIn("108158", tsv)
        self.assertIn("23290", tsv)
        self.assertIn("1.372623", tsv)

...
from scripts import zkai_attention_kv_d8_bounded_softmax_table_native_gate as gate
...
class AttentionKvBoundedSoftmaxTableNativeGateTests(unittest.TestCase):
    ...
    def test_build_payload_records_weighted_go(self):
        ...
        self.assertEqual(receipt["proof_size_bytes"], 44692)
        self.assertEqual(receipt["envelope_size_bytes"], 451982)
        self.assertEqual(receipt["attention_outputs"][0], [1, 1, 0, -1, 1, -1, 2, 1])
        self.assertEqual(receipt["attention_outputs"][-1], [-3, 3, 0, -2, 3, 1, -1, 0])

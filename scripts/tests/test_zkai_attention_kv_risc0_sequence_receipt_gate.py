...
SCRIPT_PATH = ROOT / "scripts" / "zkai_attention_kv_risc0_sequence_receipt_gate.py"
SPEC = importlib.util.spec_from_file_location("zkai_attention_kv_risc0_sequence_receipt_gate", SCRIPT_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(f"failed to load attention/KV RISC Zero sequence receipt gate from {SCRIPT_PATH}")
GATE = importlib.util.module_from_spec(SPEC)
sys.modules[SPEC.name] = GATE
SPEC.loader.exec_module(GATE)


class ZkAiAttentionKvRisc0SequenceReceiptGateTests(unittest.TestCase):
    @classmethod
    ...
    def test_checked_payload_records_carried_sequence_go(self) -> None:
        ...
        self.assertEqual(payload["route_id"], "risc0_attention_kv_sequence_semantics_receipt")
        self.assertEqual(payload["system"], "RISC Zero")
        self.assertEqual(payload["issue"], 442)
        self.assertEqual(payload["source_issue"], 441)
    ...
    def test_guest_journal_binds_three_carried_kv_transitions(self) -> None:
        payload = self.fresh_payload()
        ...
        self.assertEqual(journal["sequence_length"], 3)
        self.assertEqual(len(journal["transitions"]), 3)
        self.assertEqual([row["selected_position"] for row in journal["transitions"]], [0, 2, 3])
        self.assertEqual([row["attention_output"] for row in journal["transitions"]], [[2, 1], [4, 2], [5, -2]])
        self.assertEqual(journal["transitions"][1]["prior_kv_cache"], journal["transitions"][0]["next_kv_cache"])
        self.assertEqual(journal["transitions"][2]["prior_kv_cache"], journal["transitions"][1]["next_kv_cache"])
        self.assertEqual(journal["final_kv_cache"], journal["transitions"][2]["next_kv_cache"])
        self.assertEqual(len(journal["final_kv_cache"]), 5)
    ...
    def test_receipt_artifact_and_reverification_are_bound_to_current_journal(self) -> None:
        ...
        self.assertEqual(verification["host_summary_schema"], "zkai-attention-kv-risc0-sequence-host-summary-v1")
    ...
    def test_rejects_transition_deletion(self) -> None:
        ...
        with self.assertRaisesRegex(GATE.AttentionKvRisc0SequenceReceiptError, "journal mismatch") as err:
    ...
    def test_rejects_intermediate_kv_relabeling(self) -> None:
        payload = self.fresh_payload()
        payload["journal"]["transitions"][1]["next_kv_cache"][2]["value"] = [0, 0]
    ...
    def test_rejects_statement_commitment_relabeling(self) -> None:
        ...
        with self.assertRaisesRegex(GATE.AttentionKvRisc0SequenceReceiptError, "statement fields mismatch") as err:
            GATE.validate_payload(payload)
    ...
    def test_rejects_metric_smuggling(self) -> None:
        ...
        with self.assertRaisesRegex(GATE.AttentionKvRisc0SequenceReceiptError, "verify time") as err:
            GATE.validate_payload(payload)
    ...
    def test_verify_existing_requires_existing_json_for_prove_time(self) -> None:
        ...
        try:
            ...
            with self.assertRaisesRegex(GATE.AttentionKvRisc0SequenceReceiptError, "requires --write-json"):
                GATE.main()
        finally:
            sys.argv = old_argv
    ...
    def test_tsv_contains_stable_sequence_receipt_row(self) -> None:
        ...
        self.assertIn("risc0_attention_kv_sequence_semantics_receipt", lines[1])

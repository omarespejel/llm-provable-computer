...
from scripts import zkai_attention_kv_d16_fused_softmax_table_native_gate as gate


class AttentionKvD16FusedSoftmaxTableNativeGateTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_fused_summary_counts_table_multiplicities(self):
        ...
        self.assertEqual(summary["table_multiplicities"][-1]["multiplicity"], 40)

    def test_all_declared_mutations_reject(self):
        for name, mutated, run_native in gate.mutation_cases(self.fused_envelope):
            with self.assertRaises(gate.AttentionKvD16FusedSoftmaxTableGateError, msg=name):
    ...
    def test_gate_checks_source_sidecar_and_fused_envelope_byte_sizes(self):
        ...
        for label, path, expected_size in cases:
            ...
            with self.assertRaisesRegex(gate.AttentionKvD16FusedSoftmaxTableGateError, f"{label} size drift"):

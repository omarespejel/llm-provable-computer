...
from scripts import zkai_attention_kv_stwo_typed_size_estimate_gate as gate


class AttentionKvStwoTypedSizeEstimateGateTests(unittest.TestCase):
    @classmethod
    ...
    def assert_rejects(self, payload, message):
        with self.assertRaises(gate.StwoTypedSizeEstimateGateError) as ctx:
            gate.validate_payload(payload, allow_missing_mutation_summary=True, expected_rows=self.payload["rows"])
        self.assertIn(message, str(ctx.exception))

    def test_records_stwo_typed_size_estimate_with_binary_serializer_no_go(self):
        payload = self.payload
        gate.validate_payload(payload, expected_rows=payload["rows"])
        aggregate = payload["aggregate"]

        self.assertEqual(payload["schema"], gate.SCHEMA)
        self.assertEqual(payload["issue"], 476)
        self.assertEqual(payload["source_issues"], [469, 531])
        self.assertEqual(payload["decision"], gate.DECISION)
        self.assertEqual(payload["accounting_source"], gate.ACCOUNTING_SOURCE)
        self.assertEqual(payload["typed_estimate_status"], gate.TYPED_ESTIMATE_STATUS)
        self.assertEqual(payload["stable_binary_serializer_status"], gate.STABLE_BINARY_SERIALIZER_STATUS)
        self.assertEqual(payload["breakdown_status"], gate.BREAKDOWN_STATUS)
        self.assertEqual(payload["unexposed_fine_grained_categories"], list(gate.UNEXPOSED_FINE_GRAINED_CATEGORIES))
        ...
        self.assertEqual(aggregate["largest_typed_saving_bucket"], "fri_decommitments")
        self.assertEqual(aggregate["largest_typed_saving_bucket_bytes"], 19584)

    def test_rows_bind_typed_breakdown_sums(self):
        rows = {(row["profile_id"], row["role"]): row for row in self.payload["rows"]}
        d8 = rows[("d8_single_head_seq8", "fused")]
        self.assertEqual(d8["json_proof_size_bytes"], 47698)
        self.assertEqual(d8["typed_size_estimate_bytes"], 18124)
        self.assertEqual(d8["typed_breakdown"]["fixed_unclassified_overhead"], 48)
        self.assertEqual(d8["json_over_typed_size_ratio"], 2.631759)

        sixteen = rows[("d8_sixteen_head_seq8", "fused")]
        self.assertEqual(sixteen["json_proof_size_bytes"], 65006)
        self.assertEqual(sixteen["typed_size_estimate_bytes"], 22660)
        self.assertEqual(sixteen["typed_breakdown"]["fri_decommitments"], 4064)
        self.assertEqual(sixteen["typed_breakdown"]["trace_decommitments"], 3712)
    ...
    def test_rejects_metric_smuggling_and_overclaims(self):
        ...
        payload = self.strip_mutation_summary(self.payload)
        payload["breakdown_status"] = "GO_FINE_GRAINED_BINARY_BREAKDOWN"
        self.assert_rejects(payload, "breakdown_status drift")

        payload = self.strip_mutation_summary(self.payload)
        payload["unexposed_fine_grained_categories"].pop()
        self.assert_rejects(payload, "unexposed_fine_grained_categories drift")

        payload = self.strip_mutation_summary(self.payload)
        payload["rows"][0]["typed_size_estimate_bytes"] += 1
        self.assert_rejects(payload, "typed estimate breakdown sum drift")

        payload = self.strip_mutation_summary(self.payload)
        payload["rows"][0]["typed_breakdown"]["fri_decommitments"] += 1
        self.assert_rejects(payload, "typed estimate breakdown sum drift")

        payload = self.strip_mutation_summary(self.payload)
        payload["rows"][0]["typed_breakdown_sum_bytes"] = float(payload["rows"][0]["typed_breakdown_sum_bytes"])
        self.assert_rejects(payload, "typed_breakdown_sum_bytes must be an integer")
    ...
    def test_write_paths_are_constrained_to_evidence_dir(self):
        relative = pathlib.Path("docs/engineering/evidence/zkai-attention-kv-stwo-typed-size-estimate-2026-05.json")
        self.assertEqual(gate.require_output_path(relative), gate.JSON_OUT.resolve())

        with tempfile.TemporaryDirectory() as tmp:
            outside = pathlib.Path(tmp) / "out.json"
            with self.assertRaisesRegex(gate.StwoTypedSizeEstimateGateError, "output path"):
    ...
    def test_rust_cli_rejects_malformed_envelope(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            completed = subprocess.run(
                ...
                timeout=gate.STWO_SIZE_ESTIMATE_TIMEOUT_SECONDS,
            )

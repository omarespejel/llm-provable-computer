...
from scripts import zkai_attention_kv_two_head_longseq_air_private_softmax_table_lookup_gate as gate


class AttentionKvLongseqAirPrivateSoftmaxTableLookupGateTests(unittest.TestCase):
    ...
    def test_build_payload_records_logup_sidecar_go(self):
        ...
        self.assertEqual(receipt["lookup_proof_size_bytes"], 27078)
        self.assertEqual(receipt["lookup_envelope_size_bytes"], 781775)
        ...
        self.assertEqual(receipt["table_multiplicities"][-1]["multiplicity"], 266)
        self.assertEqual(payload["fixed_two_head_comparison"]["fixed_two_head_lookup_claims"], 104)
        self.assertEqual(payload["fixed_two_head_comparison"]["proof_size_ratio"], "1.495692")
        self.assertEqual(payload["fused_comparator"]["source_plus_sidecar_raw_proof_bytes"], 79444)
        self.assertEqual(payload["fused_comparator"]["fused_saves_vs_source_plus_sidecar_bytes"], 18942)
        self.assertEqual(payload["fused_comparator"]["fused_to_source_plus_sidecar_ratio"], "0.761568")
    ...
    def test_tsv_summary_matches_payload(self):
        ...
        self.assertIn("27078", tsv)
        self.assertIn("79444", tsv)
        self.assertIn("18942", tsv)
        self.assertIn("1.495692", tsv)

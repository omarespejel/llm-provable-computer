...
from scripts import zkai_attention_kv_four_head_air_private_softmax_table_lookup_gate as gate
...
class AttentionKvAirPrivateSoftmaxTableLookupGateTests(unittest.TestCase):
    ...
    def test_build_payload_records_logup_sidecar_go(self):
        ...
        self.assertEqual(receipt["lookup_claims"], 208)
        ...
        self.assertEqual(receipt["lookup_proof_size_bytes"], 21783)
        self.assertEqual(receipt["lookup_envelope_size_bytes"], 543187)
        self.assertEqual(receipt["lookup_proof_commitments"], 4)
        self.assertEqual(receipt["lookup_trace_commitments"], 3)
        self.assertEqual(sum(row["multiplicity"] for row in receipt["table_multiplicities"]), 208)
        ...
        self.assertEqual(payload["single_head_comparison"]["two_head_lookup_claims"], 104)
        self.assertEqual(payload["single_head_comparison"]["proof_size_ratio"], "1.477314")
        self.assertEqual(payload["single_head_comparison"]["two_to_four_proof_size_ratio"], "1.203215")
    ...
    def test_rejects_float_encoded_lookup_claim_count(self):
        payload = self.strip_mutation_summary(gate.build_payload())
        payload["lookup_receipt"]["lookup_claims"] = 208.0
        self.assert_rejects(payload, "lookup_receipt drift")
    ...
    def test_tsv_summary_matches_payload(self):
        ...
        self.assertIn("1.477314", tsv)
        self.assertIn("1.203215", tsv)

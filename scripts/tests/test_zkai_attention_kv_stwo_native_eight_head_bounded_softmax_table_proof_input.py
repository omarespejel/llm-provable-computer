...
from scripts import zkai_attention_kv_stwo_native_eight_head_bounded_softmax_table_proof_input as gate


class EightHeadBoundedSoftmaxTableProofInputTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.payload = gate.build_payload()
        gate.validate_payload(cls.payload)

    def test_builds_statement_bound_eight_head_fixture(self):
        payload = self.payload
        self.assertEqual(payload["decision"], gate.DECISION)
        self.assertEqual(payload["head_count"], 8)
        self.assertEqual(payload["key_width"], 8)
        self.assertEqual(payload["value_width"], 8)
        self.assertEqual(payload["sequence_length"], 8)
        self.assertEqual(payload["score_row_count"], 416)
    ...
    def test_rejects_semantic_drift(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvEightHeadBoundedSoftmaxTableInputError, "score rows drift"):
            gate.validate_payload(payload)

    def test_rejects_commitment_relabeling(self):
        payload = copy.deepcopy(self.payload)
        payload["statement_commitment"] = "blake2b-256:" + "11" * 32
        with self.assertRaisesRegex(gate.AttentionKvEightHeadBoundedSoftmaxTableInputError, "statement commitment drift"):
            gate.validate_payload(payload)

    def test_rejects_unknown_fields(self):
        payload = copy.deepcopy(self.payload)
        payload["hidden_sidecar_claim"] = True
        with self.assertRaisesRegex(gate.AttentionKvEightHeadBoundedSoftmaxTableInputError, "unknown field"):
            gate.validate_payload(payload)

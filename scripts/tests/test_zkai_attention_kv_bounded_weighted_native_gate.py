...
from scripts import zkai_attention_kv_bounded_weighted_native_gate as gate


class AttentionKvBoundedWeightedNativeGateTests(unittest.TestCase):
    def assert_rejects(self, payload, msg):
        with self.assertRaises(gate.AttentionKvBoundedWeightedNativeGateError) as ctx:
    ...
    def test_build_payload_records_weighted_go(self):
        ...
        self.assertEqual(receipt["score_rows"], 18)
        self.assertEqual(receipt["trace_rows"], 64)
        self.assertEqual(receipt["proof_size_bytes"], 23952)
        self.assertEqual(receipt["envelope_size_bytes"], 220004)
        self.assertEqual(receipt["attention_outputs"][0], [3, 2, 1, 2])
    ...
    def test_individual_mutations_reject(self):
        ...
        for name in gate.EXPECTED_MUTATION_NAMES:
            ...
            with self.assertRaises(gate.AttentionKvBoundedWeightedNativeGateError):
    ...
    def test_rejects_source_pair_weight_relabeling(self):
        ...
        mutated_input["score_rows"][0]["attention_weight"] = 16
    ...
    def test_rejects_mutation_summary_drift(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvBoundedWeightedNativeGateError, "mutation rejection drift"):
            gate.validate_payload(payload)

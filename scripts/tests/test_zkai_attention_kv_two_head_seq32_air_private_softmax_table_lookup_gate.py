...
from scripts import zkai_attention_kv_two_head_seq32_air_private_softmax_table_lookup_gate as gate


class AttentionKvSeq32AirPrivateSoftmaxTableLookupGateTests(unittest.TestCase):
    ...
    def test_build_payload_records_logup_sidecar_go(self):
        ...
        self.assertEqual(receipt["lookup_proof_size_bytes"], 35029)
        self.assertEqual(receipt["lookup_envelope_size_bytes"], 2196691)
        ...
        self.assertEqual(receipt["table_multiplicities"][-1]["multiplicity"], 1051)
        self.assertEqual(payload["fixed_two_head_comparison"]["fixed_two_head_lookup_claims"], 336)
        self.assertEqual(payload["fixed_two_head_comparison"]["proof_size_ratio"], "1.293621")
        self.assertEqual(payload["fused_comparator"]["source_plus_sidecar_raw_proof_bytes"], 98012)
        self.assertEqual(payload["fused_comparator"]["fused_saves_vs_source_plus_sidecar_bytes"], 31685)
        self.assertEqual(payload["fused_comparator"]["fused_to_source_plus_sidecar_ratio"], "0.676723")
    ...
    def test_rejects_malformed_mutation_case_entry(self):
        payload = gate.build_payload()
        payload["mutation_cases"][0] = "not an object"
        with self.assertRaisesRegex(
            gate.AttentionKvAirPrivateSoftmaxTableLookupGateError,
            "mutation case must be an object",
        ):
            gate.validate_payload(payload)
    ...
    def test_tsv_summary_matches_payload(self):
        ...
        self.assertIn("35029", tsv)
        self.assertIn("98012", tsv)
        self.assertIn("31685", tsv)
        self.assertIn("1.293621", tsv)
    ...
    def test_output_paths_are_constrained_to_evidence_dir(self):
        payload = gate.build_payload()
        with tempfile.TemporaryDirectory() as tmp:
            outside = gate.pathlib.Path(tmp) / "out.json"
            with self.assertRaisesRegex(
                gate.AttentionKvAirPrivateSoftmaxTableLookupGateError,
                "output path must stay under",
            ):
                gate.write_json(payload, outside)

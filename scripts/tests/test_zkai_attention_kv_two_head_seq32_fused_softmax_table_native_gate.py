...
from scripts import zkai_attention_kv_two_head_seq32_fused_softmax_table_native_gate as gate


class TwoHeadSeq32FusedSoftmaxTableNativeGateTests(unittest.TestCase):
    @classmethod
    ...
    def test_records_two_head_seq32_native_fused_scale_profile(self):
        result = self.result
        ...
        self.assertEqual(result["lookup_claims"], 1184)
        self.assertEqual(result["trace_rows"], 2048)
        self.assertEqual(result["fused_proof_size_bytes"], 66327)
        self.assertEqual(result["fused_envelope_size_bytes"], 2448150)
        self.assertEqual(result["source_plus_sidecar_raw_proof_bytes"], 98012)
        self.assertEqual(result["fused_saves_vs_source_plus_sidecar_bytes"], 31685)
        self.assertEqual(result["fused_to_source_plus_sidecar_ratio"], "0.676723")
    ...
    def test_rejects_embedded_source_input_float_and_bool_relabeling(self):
        for relabel in (2.0, True):
            ...
            with self.assertRaisesRegex(
                gate.AttentionKvTwoHeadSeq32FusedSoftmaxTableGateError,
                "fused source input split-brain drift",
            ):
                gate.validate_fused_envelope(mutated, self.source)

    def test_rejects_fused_summary_float_and_bool_relabeling(self):
        for relabel in (2.0, True):
            mutated = copy.deepcopy(self.envelope)
            mutated["fused_summary"]["source_head_count"] = relabel
            with self.assertRaisesRegex(
                gate.AttentionKvTwoHeadSeq32FusedSoftmaxTableGateError,
                "fused summary drift",
            ):
    ...
    def test_all_declared_mutations_reject(self):
        ...
        for name, result, envelope, source in gate.mutation_cases(self.result, self.envelope, self.source):
            with self.assertRaises(gate.AttentionKvTwoHeadSeq32FusedSoftmaxTableGateError, msg=name):
    ...
    def test_read_bounded_bytes_rejects_empty_and_oversized_files(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(gate.AttentionKvTwoHeadSeq32FusedSoftmaxTableGateError, "size drift"):
    ...
    def test_write_json_rejects_overclaim(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            with self.assertRaisesRegex(gate.AttentionKvTwoHeadSeq32FusedSoftmaxTableGateError, "result drift"):
    ...
    def test_writers_reject_exact_envelope_size_drift(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(gate, "FUSED_ENVELOPE_JSON", padded_envelope_path):
                with self.assertRaisesRegex(gate.AttentionKvTwoHeadSeq32FusedSoftmaxTableGateError, "file size drift"):

...
class AttentionKvTwoHeadBoundedSoftmaxTableNativeGateTests(unittest.TestCase):
    def assert_rejects(self, payload, msg):
        with self.assertRaises(gate.AttentionKvTwoHeadBoundedSoftmaxTableNativeGateError) as ctx:
    ...
    def test_build_payload_records_weighted_go(self):
        ...
        self.assertEqual(receipt["proof_size_bytes"], 47104)
        self.assertEqual(receipt["envelope_size_bytes"], 563637)
        self.assertEqual(receipt["attention_outputs"][0], [2, -3, 1, -4, 1, 2, 0, 1])
    ...
    def test_rejects_source_pair_head_relabeling(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvTwoHeadBoundedSoftmaxTableNativeGateError, "source input validation drift"):
            gate.validate_source_pair(mutated_input, mutated_envelope)
    ...
    def test_rejects_source_pair_non_list_proof(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvTwoHeadBoundedSoftmaxTableNativeGateError, "proof bytes must be a list"):
    ...
    def test_rejects_source_pair_out_of_range_proof_element(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvTwoHeadBoundedSoftmaxTableNativeGateError, r"proof byte\[0\] outside byte range"):
            gate.validate_source_pair(input_payload, envelope)
    ...
    def test_rejects_mutation_summary_drift(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvTwoHeadBoundedSoftmaxTableNativeGateError, "mutation rejection drift"):
            gate.validate_payload(payload)

...
SCRIPT_PATH = ROOT / "scripts" / "zkai_d64_activation_swiglu_proof_input.py"
SPEC = importlib.util.spec_from_file_location("zkai_d64_activation_swiglu_proof_input", SCRIPT_PATH)
...
class ZkAiD64ActivationSwiGluProofInputTests(unittest.TestCase):
    @classmethod
    ...
    def test_payload_builds_and_validates(self) -> None:
        ...
        self.assertEqual(payload["activation_lookup_rows"], ACTIVATION_SWIGLU.FF_DIM)
    ...
    def test_payload_rejects_activation_lookup_commitment_drift(self) -> None:
        ...
        with self.assertRaisesRegex(ACTIVATION_SWIGLU.ActivationSwiGluInputError, "activation_lookup_commitment"):
            ACTIVATION_SWIGLU.validate_payload(payload)

    def test_payload_rejects_row_commitment_drift(self) -> None:
        payload = self.fresh_payload()
        payload["activation_swiglu_row_commitment"] = "blake2b-256:" + "77" * 32
        with self.assertRaisesRegex(ACTIVATION_SWIGLU.ActivationSwiGluInputError, "activation/SwiGLU row"):
            ACTIVATION_SWIGLU.validate_payload(payload)

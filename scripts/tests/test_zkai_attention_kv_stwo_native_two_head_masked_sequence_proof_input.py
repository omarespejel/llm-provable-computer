...
from scripts import zkai_attention_kv_stwo_native_two_head_masked_sequence_proof_input as gate


class ZkaiAttentionKvStwoNativeTwoHeadMaskedSequenceProofInputTests(unittest.TestCase):
    def test_payload_builds_checked_two_head_surface(self):
        payload = gate.build_payload()
        gate.validate_payload(payload)
        self.assertEqual(payload["issue"], 455)
        self.assertEqual(payload["source_issue"], 453)
    ...
    def assert_rejects(self, payload, msg):
        with self.assertRaises(gate.AttentionKvStwoNativeTwoHeadInputError) as ctx:
            gate.validate_payload(payload)
    ...
    def test_tsv_rows_match_payload(self):
        ...
        self.assertEqual(rows[0]["key_width"], 8)
        self.assertEqual(rows[0]["value_width"], 8)

...
from scripts import zkai_attention_kv_d8_bounded_weighted_native_gate as gate
...
class AttentionKvBoundedWeightedNativeGateTests(unittest.TestCase):
    ...
    def test_build_payload_records_weighted_go(self):
        ...
        self.assertEqual(receipt["proof_size_bytes"], 36769)
        self.assertEqual(receipt["envelope_size_bytes"], 386078)
        self.assertEqual(receipt["attention_outputs"][0], [1, 1, 0, -1, 2, -1, 1, 1])
        self.assertEqual(receipt["attention_outputs"][-1], [-3, 4, 0, -2, 3, 1, -1, 0])

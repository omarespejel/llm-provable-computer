...
from scripts import zkai_attention_kv_eight_head_fused_softmax_table_native_gate as gate


class EightHeadFusedSoftmaxTableNativeGateTests(unittest.TestCase):
    @classmethod
    ...
    def test_records_eight_head_native_fused_scale_profile(self):
        result = self.result
        self.assertEqual(result["decision"], gate.DECISION)
        self.assertEqual(result["route_id"], gate.ROUTE_ID)
        self.assertEqual(result["source_head_count"], 8)
        self.assertEqual(result["lookup_claims"], 416)
        self.assertEqual(result["trace_rows"], 512)
        self.assertEqual(result["sidecar_issue"], 514)
        self.assertEqual(result["source_plus_sidecar_raw_proof_bytes"], 74086)
        self.assertEqual(result["fused_proof_size_bytes"], 59375)
        self.assertEqual(result["fused_envelope_size_bytes"], 1210413)
        self.assertEqual(result["fused_saves_vs_source_plus_sidecar_bytes"], 14711)
        self.assertEqual(result["fused_to_source_plus_sidecar_ratio"], 0.801433)
    ...
    def test_all_declared_mutations_reject(self):
        ...
        for name, result, envelope, source in gate.mutation_cases(self.result, self.envelope, self.source):
            with self.assertRaises(gate.AttentionKvEightHeadFusedSoftmaxTableGateError, msg=name):
    ...
    def test_write_json_rejects_overclaim(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            with self.assertRaisesRegex(gate.AttentionKvEightHeadFusedSoftmaxTableGateError, "result drift"):

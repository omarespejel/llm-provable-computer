...
from scripts import zkai_attention_kv_four_head_fused_softmax_table_native_gate as gate


class AttentionKvFourHeadFusedSoftmaxTableNativeGateTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_gate_records_fused_go_and_byte_delta(self):
        ...
        self.assertEqual(payload["lookup_claims"], 208)
        self.assertEqual(payload["table_rows"], 9)
        self.assertEqual(payload["source_head_count"], 4)
        self.assertEqual(payload["source_proof_size_bytes"], 52746)
        self.assertEqual(payload["sidecar_proof_size_bytes"], 21783)
        self.assertEqual(payload["source_plus_sidecar_raw_proof_bytes"], 74529)
        ...
        self.assertLess(payload["fused_to_source_plus_sidecar_ratio"], 0.80)
    ...
    def test_fused_summary_counts_table_multiplicities(self):
        ...
        self.assertEqual(sum(row["multiplicity"] for row in summary["table_multiplicities"]), 208)
    ...
    def test_all_declared_mutations_reject(self):
        for name, mutated, run_native in gate.mutation_cases(self.fused_envelope):
            with self.assertRaises(gate.AttentionKvFourHeadFusedSoftmaxTableGateError, msg=name):
    ...
    def test_rejects_sidecar_or_source_proof_injection(self):
        for key in ("sidecar_proof", "source_proof"):
            ...
            mutated[key] = []
            with self.assertRaisesRegex(gate.AttentionKvFourHeadFusedSoftmaxTableGateError, "unknown fused envelope field"):
    ...
    def test_rejects_source_input_split_brain(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvFourHeadFusedSoftmaxTableGateError, "source input split-brain"):
            gate.validate_fused_envelope(mutated, self.source_input, run_native=False)
    ...
    def test_native_verifier_rejects_same_size_tampered_proof_payload(self):
        ...
        try:
            with self.assertRaisesRegex(
                gate.AttentionKvFourHeadFusedSoftmaxTableGateError,
    ...
    def test_gate_checks_source_sidecar_and_fused_envelope_byte_sizes(self):
        ...
        for label, path, expected_size in cases:
            ...
            with self.assertRaisesRegex(gate.AttentionKvFourHeadFusedSoftmaxTableGateError, f"{label} size drift"):

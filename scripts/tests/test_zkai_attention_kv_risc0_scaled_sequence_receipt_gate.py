...
SCRIPT_PATH = ROOT / "scripts" / "zkai_attention_kv_risc0_scaled_sequence_receipt_gate.py"
SPEC = importlib.util.spec_from_file_location("zkai_attention_kv_risc0_scaled_sequence_receipt_gate", SCRIPT_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(f"failed to load attention/KV RISC Zero scaled sequence receipt gate from {SCRIPT_PATH}")
...
class ZkAiAttentionKvRisc0SequenceReceiptGateTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        if not GATE.JSON_OUT.exists():
            raise AssertionError(f"required attention/KV RISC Zero scaled sequence receipt evidence is missing: {GATE.JSON_OUT}")
    ...
    def test_checked_payload_records_carried_sequence_go(self) -> None:
        ...
        self.assertEqual(payload["route_id"], "risc0_attention_kv_scaled_sequence_semantics_receipt")
        self.assertEqual(payload["system"], "RISC Zero")
        self.assertEqual(payload["issue"], 444)
        self.assertEqual(payload["source_issue"], 442)
    ...
    def test_guest_journal_binds_eight_carried_kv_transitions(self) -> None:
        ...
        self.assertEqual([row["selected_position"] for row in journal["transitions"]], [0, 2, 3, 4, 5, 4, 5, 6])
        self.assertEqual([row["attention_output"] for row in journal["transitions"]], [[2, 1], [4, 2], [5, -2], [0, 6], [7, 1], [0, 6], [7, 1], [-3, 4]])
    ...
    def test_receipt_artifact_and_reverification_are_bound_to_current_journal(self) -> None:
        ...
        self.assertEqual(verification["host_summary_schema"], "zkai-attention-kv-risc0-scaled-sequence-host-summary-v1")
    ...
    def test_verify_existing_uses_canonical_evidence_not_stale_target_output(self) -> None:
        stale_target_output = GATE.ROOT / "target" / "stale-scaled-sequence-receipt-evidence.json"
    ...
    def test_tsv_contains_stable_sequence_receipt_row(self) -> None:
        ...
        self.assertIn("risc0_attention_kv_scaled_sequence_semantics_receipt", lines[1])

#!/usr/bin/env python3
"""Build the native Stwo d=8 bounded Softmax-table attention/KV proof input.

This answers issue #463 narrowly. It upgrades the issue #460 d=8 bounded
...
JSON_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d8-bounded-softmax-table-proof-2026-05.json"
TSV_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d8-bounded-softmax-table-proof-2026-05.tsv"
...
ROW_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d8-bounded-softmax-table-score-rows:v1"
INITIAL_KV_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d8-bounded-softmax-table-initial-kv:v1"
INPUT_STEPS_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d8-bounded-softmax-table-input-steps:v1"
FINAL_KV_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d8-bounded-softmax-table-final-kv:v1"
OUTPUTS_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d8-bounded-softmax-table-outputs:v1"
PUBLIC_INSTANCE_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d8-bounded-softmax-table-public-instance:v1"
PROOF_NATIVE_PARAMETER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d8-bounded-softmax-table-proof-parameters:v1"
WEIGHT_TABLE_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d8-bounded-softmax-table-weight-table:v1"
...
NEXT_BACKEND_STEP = "either add AIR-private lookup/table columns for the same policy or combine this bounded Softmax-table policy with two-head state binding after preserving table, quotient/remainder, carry, mask, and relabeling rejection surfaces"
...
def source_journal() -> dict[str, Any]:
    ...
    for key, expected in expected_scalars.items():
        if journal.get(key) != expected:
            raise AttentionKvBoundedSoftmaxTableInputError(f"source journal {key} drift")

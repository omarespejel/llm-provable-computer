#!/usr/bin/env python3
"""Compose a checked statement-bound Stwo transformer-block receipt into AgentStepReceiptV1.

This harness is intentionally a composition gate, not a new proof verifier. It
consumes the checked Stwo transformer-block statement-envelope benchmark from the native Stwo gate,
...
STWO_BENCHMARK_PATH = ROOT / "scripts" / "zkai_stwo_statement_bound_transformer_block_benchmark.py"
DEFAULT_STWO_EVIDENCE_PATH = (
    ROOT / "docs" / "engineering" / "evidence" / "zkai-stwo-statement-bound-transformer-block-benchmark-2026-05.json"
)
DEFAULT_ARTIFACT_DIR = (
    ROOT / "docs" / "engineering" / "evidence" / "agent-step-zkai-stwo-transformer-block-composition-2026-05"
)
_CHECKED_STWO_EVIDENCE_CACHE: dict[pathlib.Path, dict[str, Any]] = {}
COMPOSITION_SCHEMA = "agent-step-zkai-stwo-transformer-block-composition-v1"
COMPOSED_BUNDLE_SCHEMA = "agent-step-zkai-stwo-transformer-block-composed-bundle-v1"
TSV_COLUMNS = [
    "mutation",
    "surface",
    "baseline_accepted",
...
def _validate_stwo_evidence_payload(payload: Any) -> dict[str, Any]:
    if not isinstance(payload, dict):
        raise CompositionError("Stwo transformer-block evidence must be a JSON object")
    summary = payload.get("summary")
    if not isinstance(summary, dict):
        raise CompositionError("Stwo transformer-block evidence lacks a valid summary")
    statement = summary.get("stwo-statement-envelope")
    if not isinstance(statement, dict):
        raise CompositionError("Stwo transformer-block evidence lacks statement-envelope summary")
    proof_only = summary.get("stwo-proof-only")
    if not isinstance(proof_only, dict):
        raise CompositionError("Stwo transformer-block evidence lacks proof-only summary")
    ...
    if payload.get("suite_kind") != "native_stwo_statement_relabeling":
        raise CompositionError("Stwo transformer-block evidence is not the native statement relabeling suite")
    external = payload.get("external_system")
    if not isinstance(external, dict):
        raise CompositionError("Stwo transformer-block evidence lacks external_system metadata")
    if external.get("name") != "ptvm-stwo-backend":
        raise CompositionError("Stwo transformer-block evidence names an unexpected external system")
    if external.get("version") != STWO.STWO_PROOF_SYSTEM_VERSION:
        raise CompositionError("Stwo transformer-block evidence version does not match the checked native primitive")
    if not STWO.benchmark_passed(payload):
        raise CompositionError("checked Stwo transformer-block statement-envelope benchmark did not pass")
    if statement.get("mutations_rejected") != STWO.EXPECTED_MUTATION_COUNT:
        raise CompositionError("Stwo transformer-block statement-envelope mutation count is incomplete")
...
def _assert_evidence_matches_envelope(stwo_evidence: dict[str, Any], envelope: dict[str, Any]) -> None:
    ...
    if commitments != {envelope.get("statement_commitment")}:
        raise CompositionError("checked Stwo transformer-block evidence baseline statement commitment mismatch")
    statement_hashes = {
    ...
    if statement_hashes != {STWO.statement_payload_sha256(envelope)}:
        raise CompositionError("checked Stwo transformer-block evidence baseline statement payload mismatch")
    proof_hashes = {
    ...
    if proof_hashes != {proof_sha256}:
        raise CompositionError("checked Stwo transformer-block evidence baseline proof payload mismatch")
    ...
    if public_instance_commitments != {public_instance_commitment}:
        raise CompositionError("checked Stwo transformer-block evidence baseline public instance mismatch")
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = run_composition(
        ...
        command=[os.environ.get("PYTHON", "python3"), "scripts/agent_step_zkai_stwo_transformer_block_composition.py", *effective_argv],
    )

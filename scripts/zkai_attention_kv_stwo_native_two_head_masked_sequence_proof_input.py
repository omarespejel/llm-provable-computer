#!/usr/bin/env python3
"""Build the native Stwo attention/KV two-head masked-sequence proof input.

This answers issue #455 along the head axis: the fixture keeps d=8
integer-argmax semantics, causal-prefix mask, eight carried steps per head, and
public-row binding discipline, but moves from one head to two explicit heads.
...
JSON_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-two-head-masked-sequence-proof-2026-05.json"
TSV_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-two-head-masked-sequence-proof-2026-05.tsv"
...
ISSUE = 455
SOURCE_ISSUE = 453
TARGET_ID = "attention-kv-d8-causal-mask-two-head-v1"
REQUIRED_BACKEND_VERSION = "stwo-attention-kv-d8-causal-mask-two-head-v1"
PROOF_VERSION = "stwo-attention-kv-d8-causal-mask-two-head-air-proof-v1"
STATEMENT_VERSION = "zkai-attention-kv-stwo-native-masked-sequence-two-head-statement-v1"
SEMANTIC_SCOPE = "two_head_d8_integer_argmax_attention_kv_causal_mask_sequence_rows_bound_to_statement_receipt"
VERIFIER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-masked-sequence-two-head:v1"
...
class AttentionKvStwoNativeTwoHeadInputError(ValueError):
    pass
...
def require_int(value: Any, label: str) -> int:
    if not isinstance(value, int) or isinstance(value, bool):
        raise AttentionKvStwoNativeTwoHeadInputError(f"{label} must be an integer")
    if not -MAX_ABS_VALUE <= value <= MAX_ABS_VALUE:
        raise AttentionKvStwoNativeTwoHeadInputError(f"{label} outside bounded fixture range")
    return value


def dot(lhs: list[int], rhs: list[int]) -> int:
    if len(lhs) != KEY_WIDTH or len(rhs) != KEY_WIDTH:
        raise AttentionKvStwoNativeTwoHeadInputError("dot-product width mismatch")
...
def validate_vector(vector: Any, width: int, label: str) -> None:
    if not isinstance(vector, list) or len(vector) != width:
        raise AttentionKvStwoNativeTwoHeadInputError(f"{label} width mismatch")
...
def validate_score_row(row: Any, expected_index: int) -> None:
    if not isinstance(row, dict):
        raise AttentionKvStwoNativeTwoHeadInputError("score row must be an object")
    expected_fields = {
    ...
    if set(row) != expected_fields:
        raise AttentionKvStwoNativeTwoHeadInputError("score row field set mismatch")
    if row["row_index"] != expected_index:
        raise AttentionKvStwoNativeTwoHeadInputError("score row index drift")
    ...
    if row["head_index"] not in range(HEAD_COUNT):
        raise AttentionKvStwoNativeTwoHeadInputError("head index drift")
    ...
    if row["products"] != expected_products:
        raise AttentionKvStwoNativeTwoHeadInputError("score product row drift")
    if row["score"] != sum(expected_products):
        raise AttentionKvStwoNativeTwoHeadInputError("score sum drift")
    if row["score_gap"] != row["selected_score"] - row["score"] or row["score_gap"] < 0:
        raise AttentionKvStwoNativeTwoHeadInputError("selected-score dominance gap drift")
    if row["score_gap"] >= (1 << SCORE_GAP_BITS):
        raise AttentionKvStwoNativeTwoHeadInputError("score_gap overflow")
    if row["causal_gap"] != row["token_position"] - row["candidate_position"] or row["causal_gap"] < 0:
        raise AttentionKvStwoNativeTwoHeadInputError("causal-prefix mask gap drift")
    if row["causal_gap"] >= (1 << CAUSAL_GAP_BITS):
        raise AttentionKvStwoNativeTwoHeadInputError("causal_gap overflow")
    if row["score_tied"] != int(row["score_gap"] == 0):
        raise AttentionKvStwoNativeTwoHeadInputError("score-tie witness drift")
    if row["tie_break_gap"] != (row["candidate_position"] - row["selected_position"] if row["score_tied"] else 0):
        raise AttentionKvStwoNativeTwoHeadInputError("tie-break gap drift")
    if row["tie_break_gap"] < 0 or row["tie_break_gap"] >= (1 << TIE_GAP_BITS):
        raise AttentionKvStwoNativeTwoHeadInputError("tie-break gap outside range")
    if row["selected_flag"] not in (0, 1):
        raise AttentionKvStwoNativeTwoHeadInputError("selected flag must be boolean")
    if row["selected_flag"] == 1 and row["value"] != row["attention_output"]:
        raise AttentionKvStwoNativeTwoHeadInputError("selected value/output drift")


def validate_payload(payload: Any) -> None:
    if not isinstance(payload, dict):
        raise AttentionKvStwoNativeTwoHeadInputError("payload must be an object")
    expected_fields = {
    ...
    if set(payload) != expected_fields:
        raise AttentionKvStwoNativeTwoHeadInputError("payload field set mismatch")
    constants = {
        "schema": SCHEMA,
        "decision": DECISION,
        "issue": ISSUE,
    ...
    for field, expected in constants.items():
        if payload.get(field) != expected:
            raise AttentionKvStwoNativeTwoHeadInputError(f"payload field mismatch: {field}")
    journal = expected_journal()
    rows = build_score_rows(journal)
    outputs = [row["attention_output"] for row in journal["transitions"]]
    if payload["initial_kv_cache"] != journal["initial_kv_cache"]:
        raise AttentionKvStwoNativeTwoHeadInputError("initial KV cache drift")
    if payload["input_steps"] != journal["input_steps"]:
        raise AttentionKvStwoNativeTwoHeadInputError("input steps drift")
    if payload["final_kv_cache"] != journal["final_kv_cache"]:
        raise AttentionKvStwoNativeTwoHeadInputError("final KV cache drift")
    if payload["attention_outputs"] != outputs:
        raise AttentionKvStwoNativeTwoHeadInputError("attention outputs drift")
    if payload["score_rows"] != rows:
        raise AttentionKvStwoNativeTwoHeadInputError("score rows drift")
    if payload["selected_positions"] != [row["selected_position"] for row in journal["transitions"]]:
        raise AttentionKvStwoNativeTwoHeadInputError("selected positions drift")
    if len(payload["score_rows"]) != SCORE_ROW_COUNT:
        raise AttentionKvStwoNativeTwoHeadInputError("score row count drift")
    ...
    if payload["initial_kv_cache_commitment"] != kv_commitment(payload["initial_kv_cache"], INITIAL_KV_DOMAIN):
        raise AttentionKvStwoNativeTwoHeadInputError("initial KV commitment drift")
    if payload["input_steps_commitment"] != input_steps_commitment(payload["input_steps"]):
        raise AttentionKvStwoNativeTwoHeadInputError("input steps commitment drift")
    if payload["score_row_commitment"] != rows_commitment(payload["score_rows"]):
        raise AttentionKvStwoNativeTwoHeadInputError("score row commitment drift")
    if payload["final_kv_cache_commitment"] != kv_commitment(payload["final_kv_cache"], FINAL_KV_DOMAIN):
        raise AttentionKvStwoNativeTwoHeadInputError("final KV commitment drift")
    if payload["outputs_commitment"] != outputs_commitment(payload["input_steps"], payload["attention_outputs"]):
        raise AttentionKvStwoNativeTwoHeadInputError("outputs commitment drift")
    if payload["proof_native_parameter_commitment"] != proof_native_parameter_commitment():
        raise AttentionKvStwoNativeTwoHeadInputError("proof-native parameter commitment drift")
    if payload["statement_commitment"] != statement_commitment(payload):
        raise AttentionKvStwoNativeTwoHeadInputError("statement commitment drift")
    if payload["public_instance_commitment"] != public_instance_commitment(payload):
        raise AttentionKvStwoNativeTwoHeadInputError("public instance commitment drift")

#!/usr/bin/env python3
"""Checked AIR-constrained two-head Softmax-table lookup sidecar gate for issue #500.

This gate records the narrow positive result: the existing native two-head long-sequence d=8
...
LOOKUP_ENVELOPE_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-two-head-longseq-softmax-table-logup-sidecar-proof-2026-05.envelope.json"
SOURCE_INPUT_SCRIPT = ROOT / "scripts" / "zkai_attention_kv_stwo_native_two_head_longseq_bounded_softmax_table_proof_input.py"
JSON_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-two-head-longseq-softmax-table-logup-sidecar-gate-2026-05.json"
TSV_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-two-head-longseq-softmax-table-logup-sidecar-gate-2026-05.tsv"

MAX_SOURCE_INPUT_JSON_BYTES = 1_048_576
MAX_LOOKUP_ENVELOPE_JSON_BYTES = 4_194_304

SCHEMA = "zkai-attention-kv-stwo-native-two-head-longseq-softmax-table-logup-sidecar-gate-v1"
ISSUE = 500
SOURCE_ISSUE = 498
DECISION = "GO_NATIVE_STWO_AIR_CONSTRAINED_TWO_HEAD_LONGSEQ_SOFTMAX_TABLE_LOOKUP_RELATION_SIDECAR"
CLAIM_BOUNDARY = (
    "NATIVE_STWO_TWO_HEAD_LONGSEQ_D8_BOUNDED_SOFTMAX_TABLE_LOOKUP_MEMBERSHIP_CONSTRAINED_BY_LOGUP_SIDECAR_"
...
LOOKUP_PROOF_VERSION = "stwo-attention-kv-two-head-longseq-softmax-table-logup-sidecar-proof-v1"
LOOKUP_STATEMENT_VERSION = "zkai-attention-kv-stwo-native-two-head-longseq-softmax-table-logup-sidecar-statement-v1"
LOOKUP_SEMANTIC_SCOPE = "two_head_longseq_bounded_softmax_table_membership_constrained_by_native_stwo_logup_sidecar"
LOOKUP_VERIFIER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-two-head-longseq-softmax-table-logup-sidecar:v1"
LOOKUP_TARGET_ID = "attention-kv-two-head-longseq-d8-causal-mask-bounded-softmax-table-logup-sidecar-v1"
LOOKUP_RELATION = "AttentionKvTwoHeadLongseqSoftmaxTableLookupRelation"
LOOKUP_RELATION_WIDTH = 2
LOOKUP_PROOF_SIZE_BYTES = 27_078
LOOKUP_ENVELOPE_SIZE_BYTES = 781_775
...
FIXED_TWO_HEAD_LOOKUP_CLAIMS = 104
FIXED_TWO_HEAD_LOOKUP_PROOF_SIZE_BYTES = 18_104
FIXED_TWO_HEAD_LOOKUP_ENVELOPE_SIZE_BYTES = 333_577
CLAIM_COUNT_RATIO = "3.230769"
PROOF_SIZE_RATIO = "1.495692"
ENVELOPE_SIZE_RATIO = "2.343612"
...
VALIDATION_COMMANDS = (
    ...
    "cargo +nightly-2025-07-14 run --locked --features stwo-backend --bin zkai_attention_kv_native_two_head_longseq_softmax_table_lookup_proof -- prove docs/engineering/evidence/zkai-attention-kv-stwo-native-two-head-longseq-bounded-softmax-table-proof-2026-05.json docs/engineering/evidence/zkai-attention-kv-stwo-native-two-head-longseq-softmax-table-logup-sidecar-proof-2026-05.envelope.json",
...
EXPECTED_MUTATION_NAMES = (
    ...
    "fixed_two_head_comparison_metric_smuggling",
...
TSV_COLUMNS = (
    ...
    "lookup_claims",
    "fixed_two_head_lookup_claims",
    "claim_count_ratio",
    "lookup_proof_size_bytes",
    "fixed_two_head_lookup_proof_size_bytes",
    "proof_size_ratio",
    "lookup_envelope_size_bytes",
    "fixed_two_head_lookup_envelope_size_bytes",
    "envelope_size_ratio",
...
def verify_lookup_envelope_bytes_with_native_cli(envelope_bytes: bytes, label: str) -> None:
    ...
    command = [
        ...
        "stwo-backend",
        "--bin",
        "zkai_attention_kv_native_two_head_longseq_softmax_table_lookup_proof",
        "--",
        "verify",
        str(tmp_path),
    ]
...
def fixed_two_head_comparison() -> dict[str, Any]:
    return {
        "fixed_two_head_lookup_claims": FIXED_TWO_HEAD_LOOKUP_CLAIMS,
        "fixed_two_head_lookup_proof_size_bytes": FIXED_TWO_HEAD_LOOKUP_PROOF_SIZE_BYTES,
        "fixed_two_head_lookup_envelope_size_bytes": FIXED_TWO_HEAD_LOOKUP_ENVELOPE_SIZE_BYTES,
...
def build_payload() -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "fixed_two_head_comparison": fixed_two_head_comparison(),
    ...
    payload["gate_commitment"] = blake2b_commitment(
        {
            ...
            "proof_size_ratio": payload["fixed_two_head_comparison"]["proof_size_ratio"],
        ...
        "ptvm:zkai:attention-kv-two-head-longseq-softmax-table-logup-sidecar-gate:v1",
    )
...
def mutate_payload(payload: dict[str, Any], name: str) -> dict[str, Any]:
    ...
    elif name == "fixed_two_head_comparison_metric_smuggling":
        mutated["fixed_two_head_comparison"]["proof_size_ratio"] = "1.000000"
...
def to_tsv(payload: dict[str, Any]) -> str:
    ...
    comparison = payload["fixed_two_head_comparison"]
    row = {
        "decision": payload["decision"],
        "lookup_status": payload["lookup_status"],
        "fused_component_status": payload["fused_component_status"],
        "lookup_claims": receipt["lookup_claims"],
        "fixed_two_head_lookup_claims": comparison["fixed_two_head_lookup_claims"],
        "claim_count_ratio": comparison["claim_count_ratio"],
        "lookup_proof_size_bytes": receipt["lookup_proof_size_bytes"],
        "fixed_two_head_lookup_proof_size_bytes": comparison["fixed_two_head_lookup_proof_size_bytes"],
        "proof_size_ratio": comparison["proof_size_ratio"],
        "lookup_envelope_size_bytes": receipt["lookup_envelope_size_bytes"],
        "fixed_two_head_lookup_envelope_size_bytes": comparison["fixed_two_head_lookup_envelope_size_bytes"],

#!/usr/bin/env python3
"""Build the native Stwo bounded weighted attention/KV proof input.

This answers issue #456 narrowly. It moves the native attention/KV surface from
integer argmax to a bounded, deterministic score-to-weight policy:
...
JSON_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-bounded-weighted-proof-2026-05.json"
TSV_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-bounded-weighted-proof-2026-05.tsv"

SCHEMA = "zkai-attention-kv-stwo-native-bounded-weighted-air-proof-input-v1"
DECISION = "GO_INPUT_FOR_STWO_NATIVE_ATTENTION_KV_BOUNDED_WEIGHTED_AIR_PROOF"
ISSUE = 456
SOURCE_ISSUE = 455
...
ROW_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-bounded-weighted-score-rows:v1"
INITIAL_KV_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-bounded-weighted-initial-kv:v1"
INPUT_STEPS_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-bounded-weighted-input-steps:v1"
FINAL_KV_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-bounded-weighted-final-kv:v1"
OUTPUTS_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-bounded-weighted-outputs:v1"
PUBLIC_INSTANCE_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-bounded-weighted-public-instance:v1"
PROOF_NATIVE_PARAMETER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-bounded-weighted-proof-parameters:v1"
...
NEXT_BACKEND_STEP = "scale bounded weighted attention to d8 or two heads only after preserving weighted-product, quotient/remainder, carry, mask, and relabeling rejection surfaces"
...
class AttentionKvBoundedWeightedInputError(ValueError):
    pass
...
def require_int(value: Any, label: str) -> int:
    if not isinstance(value, int) or isinstance(value, bool):
        raise AttentionKvBoundedWeightedInputError(f"{label} must be an integer")
    if abs(value) > MAX_ABS_VALUE:
        raise AttentionKvBoundedWeightedInputError(f"{label} outside bounded fixture range")
    return value
...
def weight_from_gap(score_gap: int) -> int:
    if score_gap < 0:
        raise AttentionKvBoundedWeightedInputError("negative score gap")
...
def rows_commitment(rows: list[dict[str, Any]]) -> str:
    return commitment_from_parts(
        [
            ("encoding", "attention_kv_stwo_native_bounded_weighted_score_rows_v1"),
...
def outputs_commitment(outputs: list[list[int]]) -> str:
    return commitment_from_parts(
        [
            ("encoding", "bounded_weighted_attention_outputs_v1"),
...
def fixture_initial_kv() -> list[dict[str, Any]]:
    return [
        {"position": 0, "key": [1, 0, 1, 0], "value": [4, 2, 1, 3]},
        {"position": 1, "key": [0, 1, 1, 0], "value": [1, 3, 2, 4]},
    ]


def fixture_input_steps() -> list[dict[str, Any]]:
    return [
        {"token_position": 2, "query": [1, 1, 0, 0], "new_key": [1, 1, 0, 0], "new_value": [5, 2, 2, 1]},
        {"token_position": 3, "query": [0, 1, 1, 1], "new_key": [0, 1, 1, 1], "new_value": [2, 5, 3, 2]},
        {"token_position": 4, "query": [1, 0, 1, 1], "new_key": [1, 0, 1, 1], "new_value": [4, 4, 1, 5]},
        {"token_position": 5, "query": [1, 1, 1, 0], "new_key": [1, 1, 1, 0], "new_value": [3, 2, 5, 3]},
    ]
...
def validate_payload(payload: dict[str, Any]) -> None:
    ...
    for key, expected in expected_scalars.items():
        if payload.get(key) != expected:
            raise AttentionKvBoundedWeightedInputError(f"{key} drift")
    if payload.get("non_claims") != NON_CLAIMS:
        raise AttentionKvBoundedWeightedInputError("non claims drift")
    if payload.get("proof_verifier_hardening") != PROOF_VERIFIER_HARDENING:
        raise AttentionKvBoundedWeightedInputError("proof verifier hardening drift")
    if payload.get("validation_commands") != VALIDATION_COMMANDS:
        raise AttentionKvBoundedWeightedInputError("validation commands drift")
    ...
    if payload.get("initial_kv_cache") != initial:
        raise AttentionKvBoundedWeightedInputError("initial KV drift")
    if payload.get("input_steps") != steps:
        raise AttentionKvBoundedWeightedInputError("input steps drift")
    if payload.get("score_rows") != rows:
        raise AttentionKvBoundedWeightedInputError("score rows drift")
    if payload.get("final_kv_cache") != final_cache:
        raise AttentionKvBoundedWeightedInputError("final KV drift")
    if payload.get("attention_outputs") != outputs:
        raise AttentionKvBoundedWeightedInputError("attention outputs drift")
    ...
    if payload.get("statement_commitment") != statement:
        raise AttentionKvBoundedWeightedInputError("statement commitment drift")
    if payload.get("public_instance_commitment") != public_instance_commitment(statement):
        raise AttentionKvBoundedWeightedInputError("public instance commitment drift")

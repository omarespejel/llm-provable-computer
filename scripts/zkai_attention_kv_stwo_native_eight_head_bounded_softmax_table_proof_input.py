#!/usr/bin/env python3
"""Build the native Stwo d=8 eight-head bounded Softmax-table attention/KV proof input.

This answers issue #496 narrowly. It expands the checked four-head bounded
Softmax-table fixture into a deterministic eight-head source and then recomputes
...
SOURCE_SCRIPT = ROOT / "scripts" / "zkai_attention_kv_stwo_native_four_head_bounded_softmax_table_proof_input.py"
JSON_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-eight-head-bounded-softmax-table-proof-2026-05.json"
TSV_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-eight-head-bounded-softmax-table-proof-2026-05.tsv"

SOURCE_SCHEMA = "zkai-attention-kv-stwo-native-four-head-bounded-softmax-table-air-proof-input-v1"
SOURCE_DECISION = "GO_INPUT_FOR_STWO_NATIVE_ATTENTION_KV_FOUR_HEAD_BOUNDED_SOFTMAX_TABLE_AIR_PROOF"
...
SOURCE_PROOF_VERSION = "stwo-attention-kv-d8-causal-mask-four-head-bounded-softmax-table-air-proof-v1"
SOURCE_STATEMENT_VERSION = "zkai-attention-kv-stwo-native-four-head-bounded-softmax-table-statement-v1"
SOURCE_PAYLOAD_SHA256 = "ba7e3e3582beb0b161bfaa70b83f024913d8e09c749f6b9de66d4fa41aa8cbc4"
SOURCE_HEAD_COUNT = 4

SCHEMA = "zkai-attention-kv-stwo-native-eight-head-bounded-softmax-table-air-proof-input-v1"
DECISION = "GO_INPUT_FOR_STWO_NATIVE_ATTENTION_KV_EIGHT_HEAD_BOUNDED_SOFTMAX_TABLE_AIR_PROOF"
ISSUE = 496
SOURCE_ISSUE = 482
TARGET_ID = "attention-kv-d8-causal-mask-eight-head-bounded-softmax-table-v1"
REQUIRED_BACKEND_VERSION = "stwo-attention-kv-d8-causal-mask-eight-head-bounded-softmax-table-v1"
PROOF_VERSION = "stwo-attention-kv-d8-causal-mask-eight-head-bounded-softmax-table-air-proof-v1"
STATEMENT_VERSION = "zkai-attention-kv-stwo-native-eight-head-bounded-softmax-table-statement-v1"
SEMANTIC_SCOPE = "eight_head_d8_bounded_table_softmax_approx_attention_kv_causal_mask_rows_bound_to_statement_receipt"
VERIFIER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-eight-head-bounded-softmax-table:v1"
...
ROW_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-eight-head-bounded-softmax-table-score-rows:v1"
INITIAL_KV_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-eight-head-bounded-softmax-table-initial-kv:v1"
INPUT_STEPS_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-eight-head-bounded-softmax-table-input-steps:v1"
FINAL_KV_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-eight-head-bounded-softmax-table-final-kv:v1"
OUTPUTS_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-eight-head-bounded-softmax-table-outputs:v1"
PUBLIC_INSTANCE_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-eight-head-bounded-softmax-table-public-instance:v1"
PROOF_NATIVE_PARAMETER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-eight-head-bounded-softmax-table-proof-parameters:v1"
WEIGHT_TABLE_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-eight-head-bounded-softmax-table-weight-table:v1"
...
NEXT_BACKEND_STEP = "test whether the eight-head LogUp sidecar keeps proof bytes compact when lookup claims double again"
...
class AttentionKvEightHeadBoundedSoftmaxTableInputError(ValueError):
    pass


def _load_source_module() -> Any:
    spec = importlib.util.spec_from_file_location("zkai_attention_kv_stwo_native_four_head_bounded_softmax_table_proof_input", SOURCE_SCRIPT)
    if spec is None or spec.loader is None:
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError(f"failed to load source script: {SOURCE_SCRIPT}")
...
def require_int(value: Any, label: str) -> int:
    if not isinstance(value, int) or isinstance(value, bool):
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError(f"{label} must be an integer")
    if abs(value) > MAX_ABS_VALUE:
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError(f"{label} outside bounded fixture range")
    return value
...
def require_nonnegative_bit_bound(value: int, *, bits: int, label: str) -> int:
    ...
    if checked < 0 or checked >= (1 << bits):
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError(f"{label} outside {bits}-bit range")
    return checked


def next_power_of_two(value: int) -> int:
    if value <= 0:
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError("trace row count input must be positive")
    return 1 << (value - 1).bit_length()


def require_vector(value: Any, *, width: int, label: str) -> list[int]:
    if not isinstance(value, list):
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError(f"{label} must be a list")
    if len(value) != width:
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError(f"{label} width drift")
...
def require_head_index(value: Any, label: str) -> int:
    ...
    if head_index < 0 or head_index >= HEAD_COUNT:
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError(f"{label} outside head range")
    return head_index


def require_kv_item(value: Any, *, label: str) -> dict[str, Any]:
    if not isinstance(value, dict):
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError(f"{label} must be an object")
    return {
...
def source_payload() -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError("source payload must be an object")
    expected_scalars = {
    ...
    for key, expected in expected_scalars.items():
        if payload.get(key) != expected:
            raise AttentionKvEightHeadBoundedSoftmaxTableInputError(f"source payload {key} drift")
    observed = sha256_hex_bytes(canonical_json_bytes(payload))
    if observed != SOURCE_PAYLOAD_SHA256:
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError("source payload commitment drift")
    return payload
...
def transform_vector(vector: Sequence[int], *, head_offset: int) -> list[int]:
    ...
    for index, item in enumerate(vector):
        ...
        transformed.append(require_int(value + delta, "expanded eight-head vector"))
    return transformed
...
def dot(lhs: list[int], rhs: list[int]) -> int:
    if len(lhs) != KEY_WIDTH or len(rhs) != KEY_WIDTH:
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError("dot-product width mismatch")
...
def weight_from_gap(score_gap: int) -> int:
    if score_gap < 0:
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError("negative score gap")
...
def rows_commitment(rows: list[dict[str, Any]]) -> str:
    return commitment_from_parts(
        [
            ("encoding", "attention_kv_stwo_native_eight_head_bounded_softmax_table_score_rows_v1"),
...
def outputs_material(steps: list[dict[str, Any]], outputs: list[list[int]]) -> list[list[int]]:
    if len(steps) != len(outputs):
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError("output/input step length mismatch")
...
def fixture_initial_kv() -> list[dict[str, Any]]:
    ...
    if len(items) != INITIAL_KV_ITEMS:
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError("source initial KV length drift")
    return items
...
def fixture_input_steps() -> list[dict[str, Any]]:
    ...
    if len(items) != HEAD_COUNT * SEQUENCE_LENGTH:
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError("source input step length drift")
    return items
...
def build_score_rows(initial_kv: list[dict[str, Any]], input_steps: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[list[int]]]:
    ...
    for global_step_index, step in enumerate(steps):
        ...
        if step_index >= SEQUENCE_LENGTH:
            raise AttentionKvEightHeadBoundedSoftmaxTableInputError(f"input_steps[{global_step_index}] exceeds per-head sequence length")
        expected_position = INITIAL_KV_ITEMS_PER_HEAD + step_index
        if step["token_position"] != expected_position:
            raise AttentionKvEightHeadBoundedSoftmaxTableInputError(
        ...
        if not allowed:
            raise AttentionKvEightHeadBoundedSoftmaxTableInputError(f"input_steps[{global_step_index}] has no allowed candidates")
        ...
        if any(weight <= 0 for weight in weights):
            raise AttentionKvEightHeadBoundedSoftmaxTableInputError(f"input_steps[{global_step_index}] has zero weight")
        denominator = sum(weights)
        if denominator <= 0:
            raise AttentionKvEightHeadBoundedSoftmaxTableInputError(f"input_steps[{global_step_index}] has invalid weight denominator")
    ...
    if local_step_counts != [SEQUENCE_LENGTH for _ in range(HEAD_COUNT)]:
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError("per-head input step count drift")
    return rows, current, outputs


def validate_payload(payload: dict[str, Any]) -> None:
    if not isinstance(payload, dict):
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError("payload must be object")
    expected_scalars = {
        "schema": SCHEMA,
        "decision": DECISION,
        "issue": ISSUE,
    ...
    if extra:
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError(f"unknown field(s): {extra}")
    for key, expected in expected_scalars.items():
        if payload.get(key) != expected:
            raise AttentionKvEightHeadBoundedSoftmaxTableInputError(f"{key} drift")
    if payload.get("non_claims") != NON_CLAIMS:
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError("non claims drift")
    if payload.get("proof_verifier_hardening") != PROOF_VERIFIER_HARDENING:
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError("proof verifier hardening drift")
    if payload.get("validation_commands") != VALIDATION_COMMANDS:
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError("validation commands drift")
    if payload.get("weight_table_commitment") != weight_table_commitment():
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError("weight table commitment drift")
    ...
    if len(initial) != payload.get("initial_kv_items"):
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError("derived initial KV count drift")
    if len(final_cache) != payload.get("final_kv_items"):
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError("derived final KV count drift")
    if len(rows) != payload.get("score_row_count"):
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError("derived score row count drift")
    if next_power_of_two(len(rows)) != payload.get("trace_row_count"):
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError("derived trace row count drift")
    if payload.get("initial_kv_cache") != initial:
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError("initial KV drift")
    if payload.get("input_steps") != steps:
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError("input steps drift")
    if payload.get("score_rows") != rows:
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError("score rows drift")
    if payload.get("final_kv_cache") != final_cache:
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError("final KV drift")
    if payload.get("attention_outputs") != outputs:
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError("attention outputs drift")
    ...
    if payload.get("statement_commitment") != statement:
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError("statement commitment drift")
    if payload.get("public_instance_commitment") != public_instance_commitment(statement):
        raise AttentionKvEightHeadBoundedSoftmaxTableInputError("public instance commitment drift")

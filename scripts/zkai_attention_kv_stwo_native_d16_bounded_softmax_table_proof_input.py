#!/usr/bin/env python3
"""Build the native Stwo d=16 bounded Softmax-table attention/KV proof input.

This answers issue #501 narrowly. It upgrades the issue #460 d=16 bounded
weighted attention/KV policy from a hard-coded power-of-two clip to a
statement-bound exp-like lookup table:
...
SOURCE_SCRIPT = ROOT / "scripts" / "zkai_attention_kv_stwo_native_d16_masked_sequence_proof_input.py"
JSON_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d16-bounded-softmax-table-proof-2026-05.json"
TSV_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d16-bounded-softmax-table-proof-2026-05.tsv"

SOURCE_SCHEMA = "zkai-attention-kv-stwo-native-masked-sequence-air-proof-input-v1"
SOURCE_SEMANTICS = "integer_argmax_attention"
SOURCE_MASKING_POLICY = "causal_prefix_position_lte_query_token"
SOURCE_JOURNAL_SHA256 = "36c1c31295fc47d7f447ba317c8bef740b6cb39b3218696cfb17a88baef36e8f"

SCHEMA = "zkai-attention-kv-stwo-native-d16-bounded-softmax-table-air-proof-input-v1"
DECISION = "GO_INPUT_FOR_STWO_NATIVE_ATTENTION_KV_D16_BOUNDED_SOFTMAX_TABLE_AIR_PROOF"
ISSUE = 501
SOURCE_ISSUE = 453
...
ROW_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d16-bounded-softmax-table-score-rows:v1"
INITIAL_KV_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d16-bounded-softmax-table-initial-kv:v1"
INPUT_STEPS_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d16-bounded-softmax-table-input-steps:v1"
FINAL_KV_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d16-bounded-softmax-table-final-kv:v1"
OUTPUTS_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d16-bounded-softmax-table-outputs:v1"
PUBLIC_INSTANCE_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d16-bounded-softmax-table-public-instance:v1"
PROOF_NATIVE_PARAMETER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d16-bounded-softmax-table-proof-parameters:v1"
WEIGHT_TABLE_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d16-bounded-softmax-table-weight-table:v1"
...
NEXT_BACKEND_STEP = "fuse this d16 bounded Softmax-table source with AIR-private LogUp table membership, then compare against a matched d16 source-plus-sidecar route"
...
def _load_source_module() -> Any:
    spec = importlib.util.spec_from_file_location("zkai_attention_kv_stwo_native_d16_masked_sequence_proof_input", SOURCE_SCRIPT)
...
def source_journal() -> dict[str, Any]:
    ...
    if not isinstance(journal, dict):
        raise AttentionKvBoundedSoftmaxTableInputError("source journal must be an object")
    ...
    for key, expected in expected_source_scalars.items():
        if getattr(SOURCE, key) != expected:
            raise AttentionKvBoundedSoftmaxTableInputError(f"source {key} drift")
    observed = sha256_hex_bytes(canonical_json_bytes(journal))
    if observed != SOURCE_JOURNAL_SHA256:
        raise AttentionKvBoundedSoftmaxTableInputError("source journal commitment drift")
    return journal

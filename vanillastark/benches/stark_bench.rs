...
use vanillastark::field::FieldElement;
use vanillastark::rescue_prime::RescuePrime;
use vanillastark::stark::Stark;

/// Shared setup: build the Stark instance, trace, AIR, and boundary once.
struct StarkFixture {
    stark: Stark,
    trace: Vec<Vec<FieldElement>>,
    air: Vec<vanillastark::multivariate::MPolynomial>,
...
fn setup_fixture() -> StarkFixture {
    ...
    StarkFixture { stark, trace, air, boundary }
}
...
fn bench_rescue_prime_hash(c: &mut Criterion) {
    ...
    c.bench_function("rescue_prime_hash", |b| {
        b.iter(|| rp.hash(input))
    });
}
...
fn bench_rescue_prime_trace(c: &mut Criterion) {
    ...
    c.bench_function("rescue_prime_trace", |b| {
        b.iter(|| rp.trace(input))
    });
}
...
fn bench_stark_prove(c: &mut Criterion) {
    ...
    c.bench_function("stark_prove", |b| {
        b.iter(|| {
            fixture.stark.prove(&fixture.trace, &fixture.air, &fixture.boundary)
        })
    });
}
...
fn bench_stark_verify(c: &mut Criterion) {
    let fixture = setup_fixture();
    let proof = fixture.stark.prove(&fixture.trace, &fixture.air, &fixture.boundary);

    c.bench_function("stark_verify", |b| {
        b.iter(|| {
            fixture.stark.verify(&proof, &fixture.air, &fixture.boundary)
        })
    });
}
...
fn bench_stark_e2e(c: &mut Criterion) {
    ...
    c.bench_function("stark_e2e", |b| {
        b.iter(|| {
            let proof = fixture.stark.prove(&fixture.trace, &fixture.air, &fixture.boundary);
            let verdict = fixture.stark.verify(&proof, &fixture.air, &fixture.boundary);
            assert!(verdict);
        })
    });
}
...
fn bench_polynomial_ops(c: &mut Criterion) {
    use vanillastark::polynomial::Polynomial;
    ...
    group.bench_function("evaluate_deg63", |b| {
...
fn bench_fri(c: &mut Criterion) {
    use vanillastark::fri::Fri;
    use vanillastark::polynomial::Polynomial;
    use vanillastark::proof_stream::ProofStream;
    ...
    let fri = Fri::new(generator, omega, codeword_length, expansion_factor, num_colinearity_tests);

    let polynomial = Polynomial::new(
        (0..=degree).map(|i| FieldElement::new(i as u128)).collect(),
    );
    let domain: Vec<FieldElement> = (0..codeword_length)
...
fn bench_scaling(c: &mut Criterion) {
    ...
    for &checks in &[2, 4] {
        ...
        group.bench_with_input(
            ...
            &checks,
            |b, _| {
                b.iter(|| stark.prove(&trace, &air, &boundary))
            },
        );
        ...
        group.bench_with_input(
            ...
            &checks,
            |b, _| {
                b.iter(|| stark.verify(&proof, &air, &boundary))
            },
        );
    }

...
/// Ported from stark-anatomy/code/ntt.py.
...
pub fn ntt(primitive_root: FieldElement, values: &[FieldElement]) -> Vec<FieldElement> {
    assert!(values.len().is_power_of_two(), "cannot compute ntt of non-power-of-two sequence");
    ...
    let n = values.len();
    assert_eq!(primitive_root.pow(n as u128), FieldElement::one(), "primitive root must be nth root of unity");
    assert_ne!(primitive_root.pow((n / 2) as u128), FieldElement::one(), "primitive root is not primitive");
...
pub fn intt(primitive_root: FieldElement, values: &[FieldElement]) -> Vec<FieldElement> {
    assert!(values.len().is_power_of_two(), "cannot compute intt of non-power-of-two sequence");
...
pub fn fast_multiply(
    ...
    assert_ne!(primitive_root.pow((root_order / 2) as u128), FieldElement::one());
...
pub fn fast_interpolate(
    ...
    let left_interpolant = fast_interpolate(&domain[..half], &left_targets, primitive_root, root_order);
    let right_interpolant = fast_interpolate(&domain[half..], &right_targets, primitive_root, root_order);
...
mod tests {
    ...
    fn test_ntt_matches_evaluate() {
        ...
        let coefficients: Vec<FieldElement> = (0..n).map(|i| FieldElement::new(i as u128 + 1)).collect();
    ...
    fn test_intt_inverts_ntt() {
        let n = 16usize;
        let primitive_root = FieldElement::primitive_nth_root(n as u128);
        let values: Vec<FieldElement> = (0..n).map(|i| FieldElement::new(i as u128 * 7 + 3)).collect();
    ...
    fn test_fast_multiply() {
        ...
        let lhs = Polynomial::new(vec![FieldElement::new(1), FieldElement::new(2), FieldElement::new(3)]);

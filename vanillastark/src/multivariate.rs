...
/// Ported from stark-anatomy/code/multivariate.py.
...
impl MPolynomial {
    pub fn zero() -> Self {
        MPolynomial { dictionary: HashMap::new() }
    }
...
impl Add for MPolynomial {
    ...
    fn add(self, other: Self) -> Self {
        let num_variables = num_variables_from_keys(
            self.dictionary.keys().chain(other.dictionary.keys()),
        );

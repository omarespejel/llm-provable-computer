...
/// Ported from stark-anatomy/code/stark.py.

use blake2::{Blake2b512, Digest};
use crate::field::FieldElement;
use crate::polynomial::Polynomial;
use crate::multivariate::MPolynomial;
...
impl Stark {
    ...
    fn transition_quotient_degree_bounds(&self, transition_constraints: &[MPolynomial]) -> Vec<isize> {
    ...
    pub fn prove(
        ...
        let randomizer_bytes: Vec<Vec<u8>> = randomizer_codeword.iter().map(|e| e.to_bytes_str()).collect();
    ...
    pub fn verify(
        ...
        // Get weights
        let num_weights = 1 + 2 * transition_constraints.len() + 2 * self.boundary_interpolants(boundary).len();
        ...
        for r in 0..boundary_quotient_roots.len() {
            ...
            for &i in &duplicated_indices {
                let element = if let ProofObject::Element(e) = proof_stream.pull() { e } else { return false };
                let path = if let ProofObject::Path(p) = proof_stream.pull() { p } else { return false };
                if !Merkle::verify(&boundary_quotient_roots[r], i, &path, &element.to_bytes_str()) {
                    return false;
                }
        ...
        let mut randomizer: std::collections::HashMap<usize, FieldElement> = std::collections::HashMap::new();
        for &i in &duplicated_indices {
            let element = if let ProofObject::Element(e) = proof_stream.pull() { e } else { return false };
            let path = if let ProofObject::Path(p) = proof_stream.pull() { p } else { return false };
        ...
        let transition_zerofier = self.transition_zerofier();
        let tq_degree_bounds = self.transition_quotient_degree_bounds(transition_constraints);
        let bq_degree_bounds = self.boundary_quotient_degree_bounds(randomized_trace_length, boundary);
        let max_degree = self.max_degree(transition_constraints);
        ...
        for i in 0..indices.len() {
            ...
            for s in 0..self.num_registers {
                ...
                current_trace[s] = leafs[s][&current_index] * zerofier.evaluate(domain_current_index)

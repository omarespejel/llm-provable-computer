...
/// Ported from stark-anatomy/code/univariate.py.
...
impl Polynomial {
    ...
    pub fn divide(numerator: &Polynomial, denominator: &Polynomial) -> (Polynomial, Polynomial) {
        assert!(denominator.degree() >= 0, "cannot divide by zero polynomial");
    ...
    pub fn interpolate_domain(domain: &[FieldElement], values: &[FieldElement]) -> Polynomial {
        assert_eq!(domain.len(), values.len(), "domain and values must have same length");
...
impl PartialEq for Polynomial {
    fn eq(&self, other: &Self) -> bool {
        ...
        for i in 0..max_len {
            let a = if i < self.coefficients.len() { self.coefficients[i] } else { FieldElement::zero() };
            let b = if i < other.coefficients.len() { other.coefficients[i] } else { FieldElement::zero() };
            if a != b {
                return false;
            }
        }
...
mod tests {
    ...
    fn test_interpolate() {
        ...
        for i in 0..domain.len() {
            assert_eq!(poly.evaluate(domain[i]), values[i], "interpolate test 1 at i={}", i);
        }

        assert_ne!(poly.evaluate(fe(363)), zero, "interpolate test 2");
        assert_eq!(poly.degree(), domain.len() as isize - 1, "interpolate test 3");
    }

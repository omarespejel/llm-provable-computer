...
/// Ported from stark-anatomy/code/fri.py.

use blake2::{Blake2b512, Digest};
use crate::field::FieldElement;
use crate::polynomial::{Polynomial, test_colinearity};
use crate::merkle::Merkle;
use crate::proof_stream::{ProofStream, ProofObject};
...
impl Fri {
    pub fn new(
        ...
        assert!(fri.num_rounds() >= 1, "cannot do FRI with less than one round");
        fri
    }
    ...
    pub fn query(
        ...
        let current_bytes: Vec<Vec<u8>> = current_codeword.iter().map(|e| e.to_bytes_str()).collect();
        let next_bytes: Vec<Vec<u8>> = next_codeword.iter().map(|e| e.to_bytes_str()).collect();

        for s in 0..self.num_colinearity_tests {
            proof_stream.push(ProofObject::Path(Merkle::open(a_indices[s], &current_bytes)));
            proof_stream.push(ProofObject::Path(Merkle::open(b_indices[s], &current_bytes)));
    ...
    pub fn prove(
        ...
        assert_eq!(self.domain_length, codeword.len(), "codeword length mismatch");
    ...
    pub fn verify(
        ...
        let last_codeword_bytes: Vec<Vec<u8>> = last_codeword.iter().map(|e| e.to_bytes_str()).collect();
        ...
        for r in 0..num_rounds - 1 {
            ...
            for i in 0..self.num_colinearity_tests {
                let path_a = if let ProofObject::Path(p) = proof_stream.pull() { p } else { panic!("expected Path") };
                if !Merkle::verify(&roots[r], a_indices[i], &path_a, &aa[i].to_bytes_str()) {
                    return false;
                }
                let path_b = if let ProofObject::Path(p) = proof_stream.pull() { p } else { panic!("expected Path") };
                if !Merkle::verify(&roots[r], b_indices[i], &path_b, &bb[i].to_bytes_str()) {
                    return false;
                }
                let path_c = if let ProofObject::Path(p) = proof_stream.pull() { p } else { panic!("expected Path") };
...
mod tests {
    ...
    fn test_fri_prove_verify() {
        ...
        let fri = Fri::new(generator, omega, initial_codeword_length, expansion_factor, num_colinearity_tests);

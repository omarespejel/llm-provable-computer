...
use llm_provable_computer::stwo_backend::{
    load_phase11_decoding_chain, save_phase11_decoding_chain,
};
...
enum Command {
    ...
    /// Prepare a source-bound shared-normalization primitive artifact.
    PrepareStwoSharedNormalizationPrimitiveArtifact {
        /// File where the serialized artifact JSON will be written.
        #[arg(short = 'o', long = "output")]
        output: PathBuf,
    },
    /// Verify a source-bound shared-normalization primitive artifact.
    ...
    /// Run the typed Phase44D source-emission benchmark on the experimental carry-aware Phase12 backend.
    #[cfg_attr(not(feature = "stwo-backend"), command(hide = true))]
    BenchStwoPhase44dSourceEmissionExperimentalReuse {
        /// File where the benchmark TSV will be written.
        #[arg(long = "output-tsv")]
        output_tsv: PathBuf,
        /// Optional file where the benchmark JSON will be written.
        #[arg(long = "output-json")]
        output_json: Option<PathBuf>,
        /// Optional comma-delimited step counts. Defaults to the honest exploratory sweep.
        #[arg(long = "step-counts", value_delimiter = ',', num_args = 1..)]
        step_counts: Vec<usize>,
        /// Capture host-dependent wall-clock timings in the report output.
        #[arg(long = "capture-timings", default_value_t = false)]
    ...
    /// Run the typed Phase44D source-emission benchmark on the experimental carry-aware Phase12 backend over the 3x3 decoding_step_v2 layout.
    #[cfg_attr(not(feature = "stwo-backend"), command(hide = true))]
    #[command(name = "bench-stwo-phase44d-source-emission-experimental-3x3-reuse")]
    BenchStwoPhase44dSourceEmissionExperimental3x3Reuse {
        /// File where the benchmark TSV will be written.
        #[arg(long = "output-tsv")]
        output_tsv: PathBuf,
        /// Optional file where the benchmark JSON will be written.
    ...
    /// Decompose the replay baseline across the checked Tablero layout families.
    #[cfg_attr(not(feature = "stwo-backend"), command(hide = true))]
...
fn run() -> llm_provable_computer::Result<()> {
    ...
    match cli.command {
        ...
        Command::BenchStwoPhase12SharedLookupArtifactReuse {
            output_tsv,
            output_json,
            capture_timings,
        } => bench_stwo_phase12_shared_lookup_artifact_reuse_command(
            &output_tsv,
            output_json.as_deref(),
            capture_timings,
        )?,
        ...
        Command::BenchStwoPhase44dSourceEmissionExperimentalReuse {
            output_tsv,
            output_json,
            step_counts,
            capture_timings,
        } => bench_stwo_phase44d_source_emission_experimental_reuse_command(
            &output_tsv,
            output_json.as_deref(),
            &step_counts,
            capture_timings,
        )?,
        Command::BenchStwoPhase44dSourceEmissionExperimental3x3Reuse {
            output_tsv,
            output_json,
            step_counts,
            capture_timings,
        ...
        Command::BenchStwoTableroReplayBreakdown {
            output_tsv,
            output_json,
            capture_timings,
...
fn bench_stwo_phase12_shared_lookup_artifact_reuse_command(
    output_tsv: &Path,
    output_json: Option<&Path>,
    capture_timings: bool,
) -> llm_provable_computer::Result<()> {
    #[cfg(not(feature = "stwo-backend"))]
    {
        ...
        let _ = capture_timings;
        return Err(VmError::UnsupportedProof(
            "S-two Phase12 shared lookup artifact reuse benchmark requires building with `--features stwo-backend`"
                .to_string(),
        ));
    }
    ...
    #[cfg(feature = "stwo-backend")]
    {
        require_stwo_backend("S-two Phase12 shared lookup artifact reuse benchmark")?;
        validate_distinct_benchmark_output_paths(output_tsv, output_json)?;
        if let Some(parent) = output_tsv.parent() {
            if !parent.as_os_str().is_empty() {
                fs::create_dir_all(parent)?;
        ...
        let report = if capture_timings {
            run_stwo_phase12_shared_lookup_artifact_reuse_benchmark_with_options(true)?
        } else {
            run_stwo_phase12_shared_lookup_artifact_reuse_benchmark()?
        };
        save_stwo_phase12_shared_lookup_artifact_reuse_benchmark_report_tsv(&report, output_tsv)?;
        if let Some(path) = output_json {
            save_stwo_phase12_shared_lookup_artifact_reuse_benchmark_report_json(&report, path)?;
            println!("output_json: {}", path.display());
        }
        println!("output_tsv: {}", output_tsv.display());
        println!("benchmark_version: {}", report.benchmark_version);
        ...
        for row in &report.rows {
            println!(
                "phase12_shared_lookup_artifact_reuse_benchmark: {} {} steps={} unique_artifacts={} proof_bytes={} serialized_bytes={} verify_ms={} verified={}",
                row.primitive,
                row.backend_variant,
                row.steps,
...
fn bench_stwo_phase44d_source_emission_reuse_command(
    ...
    #[cfg(feature = "stwo-backend")]
    {
        ...
        let report = if step_counts.is_empty() {
            ...
            run_stwo_phase44d_source_emission_benchmark_for_steps(step_counts, capture_timings)?
        };
        ...
        for row in &report.rows {
            println!(
                "phase44d_source_emission_benchmark: {} {} steps={} serialized_bytes={} emit_ms={} verify_ms={} verified={}",
                row.primitive,
                row.backend_variant,
                row.steps,
                row.serialized_bytes,
...
fn bench_stwo_phase44d_source_emission_experimental_reuse_command(
    output_tsv: &Path,
    output_json: Option<&Path>,
    step_counts: &[usize],
    capture_timings: bool,
    ...
    #[cfg(not(feature = "stwo-backend"))]
    {
        ...
        let _ = capture_timings;
        return Err(VmError::UnsupportedProof(
            "S-two Phase44D experimental source emission benchmark requires building with `--features stwo-backend`"
                .to_string(),
        ));
    }
    ...
    #[cfg(feature = "stwo-backend")]
    {
        require_stwo_backend("S-two Phase44D experimental source emission benchmark")?;
        validate_distinct_benchmark_output_paths(output_tsv, output_json)?;
        if let Some(parent) = output_tsv.parent() {
            if !parent.as_os_str().is_empty() {
                fs::create_dir_all(parent)?;
        ...
        let report = if step_counts.is_empty() {
            if capture_timings {
                run_stwo_phase44d_source_emission_experimental_benchmark_with_options(true)?
            } else {
                run_stwo_phase44d_source_emission_experimental_benchmark()?
            }
        } else {
        ...
        for row in &report.rows {
            println!(
                "phase44d_source_emission_experimental_benchmark: {} {} steps={} serialized_bytes={} emit_ms={} verify_ms={} verified={}",
                row.primitive,
                row.backend_variant,
                row.steps,
                row.serialized_bytes,
...
fn bench_stwo_phase44d_source_emission_experimental_3x3_reuse_command(
    output_tsv: &Path,
    output_json: Option<&Path>,
    step_counts: &[usize],
    capture_timings: bool,
    ...
    #[cfg(not(feature = "stwo-backend"))]
    {
        ...
        let _ = capture_timings;
        return Err(VmError::UnsupportedProof(
            "S-two Phase44D experimental 3x3 source emission benchmark requires building with `--features stwo-backend`"
                .to_string(),
        ));
    }
    ...
    #[cfg(feature = "stwo-backend")]
    {
        require_stwo_backend("S-two Phase44D experimental 3x3 source emission benchmark")?;
        validate_distinct_benchmark_output_paths(output_tsv, output_json)?;
        if let Some(parent) = output_tsv.parent() {
            if !parent.as_os_str().is_empty() {
                fs::create_dir_all(parent)?;
        ...
        let report = if step_counts.is_empty() {
            if capture_timings {
                run_stwo_phase44d_source_emission_experimental_3x3_benchmark_with_options(true)?
            } else {
                run_stwo_phase44d_source_emission_experimental_3x3_benchmark()?
            }
        } else {
...
fn bench_stwo_tablero_replay_breakdown_command(
    output_tsv: &Path,
    output_json: Option<&Path>,
    capture_timings: bool,
) -> llm_provable_computer::Result<()> {
    #[cfg(not(feature = "stwo-backend"))]
    {
        ...
        let _ = capture_timings;
        return Err(VmError::UnsupportedProof(
            "S-two Tablero replay breakdown benchmark requires building with `--features stwo-backend`"
                .to_string(),
        ));
    }
    ...
    #[cfg(feature = "stwo-backend")]
    {
        require_stwo_backend("S-two Tablero replay breakdown benchmark")?;
        validate_distinct_benchmark_output_paths(output_tsv, output_json)?;
        if let Some(parent) = output_tsv.parent() {
            if !parent.as_os_str().is_empty() {
                fs::create_dir_all(parent)?;
        ...
        let report = if capture_timings {
            run_stwo_tablero_replay_breakdown_benchmark_with_options(true)?
        } else {
            run_stwo_tablero_replay_breakdown_benchmark()?
        };
...
fn bench_stwo_phase44d_rescaled_exploratory_command(
    ...
    #[cfg(feature = "stwo-backend")]
    {
        ...
        let report =
            if step_counts.is_empty() && incoming_divisor.is_none() && lookup_divisor.is_none() {
                if capture_timings {
...
mod cli_dispatch_tests {
    ...
    #[cfg(feature = "stwo-backend")]
    #[test]
    fn phase12_shared_lookup_artifact_reuse_benchmark_command_parses_directly_and_is_not_run_shorthand(
    ) {
        assert!(!needs_run_subcommand(
        ...
        let normalized = normalize_args(
            [
                ...
                "bench-stwo-phase12-shared-lookup-artifact-reuse",
                "--output-tsv",
                "phase12-artifact.tsv",
                "--output-json",
                "phase12-artifact.json",
            ]
            .into_iter()
            .map(OsString::from),
        );
        assert_eq!(
            ...
            vec![
                OsString::from("tvm"),
                OsString::from("bench-stwo-phase12-shared-lookup-artifact-reuse"),
                OsString::from("--output-tsv"),
                OsString::from("phase12-artifact.tsv"),
                OsString::from("--output-json"),
                OsString::from("phase12-artifact.json"),
            ]
        );
    }
    ...
    #[cfg(feature = "stwo-backend")]
    #[test]
    fn phase12_shared_lookup_artifact_reuse_benchmark_command_preserves_capture_timings_flag() {
        let normalized = normalize_args(
            [
                "tvm",
                "bench-stwo-phase12-shared-lookup-artifact-reuse",
    ...
    #[cfg(feature = "stwo-backend")]
    #[test]
    fn phase44d_source_emission_benchmark_command_preserves_step_counts_flag() {
        let normalized = normalize_args(
            [
                "tvm",
                "bench-stwo-phase44d-source-emission-reuse",
    ...
    #[cfg(not(feature = "stwo-backend"))]
    #[test]
    fn phase44d_source_emission_experimental_benchmark_command_is_hidden_without_stwo_backend() {
        let help = Cli::command().render_long_help().to_string();
        assert!(!help.contains("bench-stwo-phase44d-source-emission-experimental-reuse"));
    }

    #[cfg(feature = "stwo-backend")]
    #[test]
    fn phase44d_source_emission_experimental_benchmark_command_parses_directly_and_is_not_run_shorthand(
    ) {
        assert!(!needs_run_subcommand(
        ...
        let normalized = normalize_args(
            [
                ...
                "bench-stwo-phase44d-source-emission-experimental-reuse",
                "--output-tsv",
                "phase44d-experimental.tsv",
                "--output-json",
                "phase44d-experimental.json",
            ]
            .into_iter()
            .map(OsString::from),
        );
        assert_eq!(
            ...
            vec![
                OsString::from("tvm"),
                OsString::from("bench-stwo-phase44d-source-emission-experimental-reuse"),
                OsString::from("--output-tsv"),
                OsString::from("phase44d-experimental.tsv"),
                OsString::from("--output-json"),
                OsString::from("phase44d-experimental.json"),
            ]
        );
    }
    ...
    #[cfg(feature = "stwo-backend")]
    #[test]
    fn phase44d_source_emission_experimental_benchmark_command_preserves_step_counts_flag() {
        let normalized = normalize_args(
            [
                "tvm",
                "bench-stwo-phase44d-source-emission-experimental-reuse",
    ...
    #[cfg(not(feature = "stwo-backend"))]
    #[test]
    fn phase44d_source_emission_experimental_3x3_benchmark_command_is_hidden_without_stwo_backend()
    {
        let help = Cli::command().render_long_help().to_string();
    ...
    fn phase44d_source_emission_experimental_3x3_benchmark_command_parses_directly_and_is_not_run_shorthand(
        ...
        let normalized = normalize_args(
            [
                ...
                "bench-stwo-phase44d-source-emission-experimental-3x3-reuse",
                "--output-tsv",
                "phase44d-3x3-experimental.tsv",
                "--output-json",
                "phase44d-3x3-experimental.json",
            ]
            .into_iter()
            .map(OsString::from),
        );
        assert_eq!(
            ...
            vec![
                OsString::from("tvm"),
                OsString::from("bench-stwo-phase44d-source-emission-experimental-3x3-reuse"),
                OsString::from("--output-tsv"),
                OsString::from("phase44d-3x3-experimental.tsv"),
                OsString::from("--output-json"),
                OsString::from("phase44d-3x3-experimental.json"),
            ]
        );
    }
    ...
    #[cfg(feature = "stwo-backend")]
    #[test]
    fn phase44d_source_emission_experimental_3x3_benchmark_command_preserves_step_counts_flag() {
        let normalized = normalize_args(
            [
                "tvm",
                "bench-stwo-phase44d-source-emission-experimental-3x3-reuse",
    ...
    #[cfg(feature = "stwo-backend")]
    #[test]
    fn phase12_shared_lookup_artifact_reuse_benchmark_command_rejects_identical_output_paths_before_run(
    ) {
        std::thread::Builder::new()
            .name("phase12-shared-lookup-artifact-cli-negative".to_string())
            .stack_size(8 * 1024 * 1024)
            .spawn(|| {
                let cli = Cli::try_parse_from(normalize_args(
                    [
                ...
                .expect("phase12 shared lookup artifact command should parse");

                let Command::BenchStwoPhase12SharedLookupArtifactReuse {
                    output_tsv,
                    output_json,
                    capture_timings,
                } = cli.command
                else {
                    panic!("expected phase12 shared lookup artifact benchmark command");
                };

                let err = super::bench_stwo_phase12_shared_lookup_artifact_reuse_command(
                    &output_tsv,
                    output_json.as_deref(),
                    capture_timings,
                )
    ...
    #[test]
    fn tablero_replay_breakdown_optimized_benchmark_command_parses_directly_and_is_not_run_shorthand(
    ) {
        assert!(!needs_run_subcommand(
        ...
        assert_eq!(
            ...
            vec![
                OsString::from("tvm"),
                OsString::from("bench-stwo-tablero-replay-breakdown-optimized"),
                OsString::from("--output-tsv"),
                OsString::from("optimized.tsv"),
                OsString::from("--output-json"),
                OsString::from("optimized.json"),
            ]
        );
    }
    ...
    #[cfg(feature = "stwo-backend")]
    #[test]
    fn tablero_replay_breakdown_optimized_command_rejects_identical_output_paths_before_run() {
        std::thread::Builder::new()
            .name("tablero-optimized-cli-negative".to_string())
            .stack_size(8 * 1024 * 1024)
            .spawn(|| {
                let cli = Cli::try_parse_from(normalize_args(
                    [
                ...
                .expect("tablero optimized replay breakdown command should parse");

                let Command::BenchStwoTableroReplayBreakdownOptimized {
                    output_tsv,
                    output_json,
                    capture_timings,
                } = cli.command
                else {
                    panic!("expected tablero optimized replay breakdown benchmark command");
                };

                let err = super::bench_stwo_tablero_replay_breakdown_optimized_command(
                    &output_tsv,
                    output_json.as_deref(),
                    capture_timings,
                )
            ...
            .expect("spawn tablero optimized negative-path test thread")
            .join()

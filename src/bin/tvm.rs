...
use llm_provable_computer::{
    ...
    load_phase99_multi_interval_gemma_richer_family_accumulation_artifact,
...
enum Command {
    ...
    PrepareStwoRepeatedGemmaSliceAccumulationArtifact {
        ...
        #[arg(long = "proof")]
        proof: PathBuf,
        /// Number of repeated slices to accumulate.
        #[arg(long = "total-slices", default_value_t = 4)]
        total_slices: usize,
        /// Token position reused across the repeated slice family.
        #[arg(long = "token-position", default_value_t = 0)]
        token_position: u64,
        /// First block index in the repeated slice family.
    ...
    PrepareStwoMultiIntervalGemmaRicherFamilyAccumulationArtifact {
        ...
        #[arg(long = "proof")]
        proof: PathBuf,
        /// Number of repeated intervals to accumulate.
        #[arg(long = "total-intervals", default_value_t = 4)]
        ...
        /// Initial token position for the interval family.
        ...
        /// First block index reused by every interval.
        #[arg(long = "start-block-index", default_value_t = 0)]
        start_block_index: u64,
        /// File where the serialized artifact JSON will be written.
        #[arg(short = 'o', long = "output")]
    ...
    /// Prepare a folded repeated multi-interval richer-family artifact over Phase105 and Phase106 sources.
    #[command(
        name = "prepare-stwo-folded-repeated-multi-interval-linear-block-richer-family-artifact",
        alias = "prepare-stwo-folded-repeated-multi-interval-gemma-richer-family-artifact"
    )]
    PrepareStwoFoldedRepeatedMultiIntervalGemmaRicherFamilyArtifact {
        /// Path to the serialized Phase105 repeated multi-interval richer-family accumulation artifact JSON file.
        #[arg(long = "source")]
        source: PathBuf,
        /// Path to the serialized Phase106 folded repeated multi-interval prototype artifact JSON file.
    ...
    /// Verify a folded repeated multi-interval richer-family artifact against Phase105 and Phase106 sources.
    #[command(
        name = "verify-stwo-folded-repeated-multi-interval-linear-block-richer-family-artifact",
        alias = "verify-stwo-folded-repeated-multi-interval-gemma-richer-family-artifact"
    )]
    VerifyStwoFoldedRepeatedMultiIntervalGemmaRicherFamilyArtifact {
        /// Path to the serialized Phase107 folded repeated multi-interval richer-family artifact JSON file.
        artifact: PathBuf,
        /// Path to the serialized Phase105 repeated multi-interval richer-family accumulation artifact JSON file.
        #[arg(long = "source")]
        source: PathBuf,
    ...
    /// Prepare a transformer-specific fold operator artifact from two Phase107 children.
    PrepareStwoTransformerSpecificFoldOperatorArtifact {
        /// Path to the serialized left Phase107 folded repeated multi-interval richer-family artifact JSON file.
        #[arg(long = "left")]
        left: PathBuf,
        /// Path to the serialized right Phase107 folded repeated multi-interval richer-family artifact JSON file.
        #[arg(long = "right")]
        right: PathBuf,
        /// File where the serialized artifact JSON will be written.
        #[arg(short = 'o', long = "output")]
    ...
    /// Verify a transformer-specific fold operator artifact against two Phase107 children.
    ...
    /// Prepare a repeated-window fold-tree artifact from a leaf family of Phase107 artifacts.
    PrepareStwoRepeatedWindowFoldTreeArtifact {
        /// Repeated Phase107 folded repeated multi-interval richer-family leaf artifact JSON paths.
    ...
    /// Verify a repeated-window fold-tree artifact against a leaf family of Phase107 artifacts.
    ...
    /// Prepare a transformer accumulation-semantics artifact from a leaf family of Phase107 artifacts.
    PrepareStwoTransformerAccumulationSemanticsArtifact {
        /// Repeated Phase107 folded repeated multi-interval richer-family leaf artifact JSON paths.
        #[arg(long = "leaf", required = true)]
        leaves: Vec<PathBuf>,
        /// File where the serialized artifact JSON will be written.
    ...
    /// Verify a transformer accumulation-semantics artifact against a leaf family of Phase107 artifacts.
    ...
    /// Prepare a richer linear-block window-family artifact from Phase107 leaves and a Phase112 semantics source.
    #[command(
        name = "prepare-stwo-richer-linear-block-window-family-artifact",
        alias = "prepare-stwo-richer-gemma-window-family-artifact"
    )]
    ...
    /// Verify a richer linear-block window-family artifact against Phase107 leaves and a Phase112 semantics source.
    #[command(
        name = "verify-stwo-richer-linear-block-window-family-artifact",
        alias = "verify-stwo-richer-gemma-window-family-artifact"
    )]
    VerifyStwoRicherGemmaWindowFamilyArtifact {
        /// Path to the serialized Phase113 richer window-family artifact JSON file.
        artifact: PathBuf,
        /// Path to the serialized Phase112 transformer accumulation-semantics artifact JSON file.
        #[arg(long = "semantics")]
        semantics: PathBuf,
...
fn run() -> llm_provable_computer::Result<()> {
    ...
    match cli.command {
        ...
        Command::PrepareStwoGemmaBlockRicherSliceArtifact {
            proof,
            chain,
            output,
        } => prepare_stwo_gemma_block_richer_slice_artifact_command(&proof, &chain, &output)?,
        Command::VerifyStwoGemmaBlockRicherSliceArtifact { artifact } => {
            verify_stwo_gemma_block_richer_slice_artifact_command(&artifact)?
        }
        Command::PrepareStwoRepeatedGemmaSliceAccumulationArtifact {
            proof,
            total_slices,
            token_position,
            start_block_index,
            output,
        } => prepare_stwo_repeated_gemma_slice_accumulation_artifact_command(
            &proof,
            total_slices,
            token_position,
            start_block_index,
        ...
        Command::PrepareStwoMultiIntervalGemmaRicherFamilyAccumulationArtifact {
            proof,
            total_intervals,
            ...
            start_block_index,
            output,
        } => prepare_stwo_multi_interval_gemma_richer_family_accumulation_artifact_command(
            &proof,
            total_intervals,
            interval_total_slices,
            token_position_start,
        ...
        Command::VerifyStwoMultiIntervalGemmaRicherFamilyAccumulationArtifact { artifact } => {
            verify_stwo_multi_interval_gemma_richer_family_accumulation_artifact_command(&artifact)?
        }
        Command::PrepareStwoFoldedMultiIntervalGemmaAccumulationPrototypeArtifact {
            source,
            output,
        } => prepare_stwo_folded_multi_interval_gemma_accumulation_prototype_artifact_command(
            &source, &output,
        )?,
        Command::VerifyStwoFoldedMultiIntervalGemmaAccumulationPrototypeArtifact {
            artifact,
            source,
        } => verify_stwo_folded_multi_interval_gemma_accumulation_prototype_artifact_command(
            &artifact, &source,
        )?,
        Command::PrepareStwoFoldedMultiIntervalGemmaRicherFamilyArtifact {
            source,
            folded,
            output,
        } => prepare_stwo_folded_multi_interval_gemma_richer_family_artifact_command(
            &source, &folded, &output,
        )?,
        Command::VerifyStwoFoldedMultiIntervalGemmaRicherFamilyArtifact {
            artifact,
            source,
            folded,
        ...
        Command::PrepareStwoFoldedRepeatedMultiIntervalGemmaAccumulationPrototypeArtifact {
            source,
            output,
        } => prepare_stwo_folded_repeated_multi_interval_gemma_accumulation_prototype_artifact_command(
            &source, &output,
        )?,
        Command::VerifyStwoFoldedRepeatedMultiIntervalGemmaAccumulationPrototypeArtifact {
            artifact,
            source,
        } => verify_stwo_folded_repeated_multi_interval_gemma_accumulation_prototype_artifact_command(
            &artifact, &source,
        )?,
        Command::PrepareStwoFoldedRepeatedMultiIntervalGemmaRicherFamilyArtifact {
            source,
            folded,
            output,
        } => prepare_stwo_folded_repeated_multi_interval_gemma_richer_family_artifact_command(
            &source, &folded, &output,
        )?,
        Command::VerifyStwoFoldedRepeatedMultiIntervalGemmaRicherFamilyArtifact {
            artifact,
            source,
            folded,
        ...
        Command::PrepareStwoTransformerAccumulationSemanticsArtifact { leaves, output } => {
            prepare_stwo_transformer_accumulation_semantics_artifact_command(&leaves, &output)?
        }
...
#[cfg(feature = "stwo-backend")]
fn print_phase9475_gemma_block_richer_slice_report(
    ...
    println!("program_label: {}", artifact.program_label);
    println!(
        "core_slice_artifact_commitment: {}",
        artifact.core_slice_artifact_commitment
    );
    println!(
        "chain_artifact_commitment: {}",
        artifact.chain_artifact_commitment
    ...
    println!(
        ...
        artifact.secondary_activation_input
    );
    println!(
        "secondary_activation_output: {}",
        artifact.secondary_activation_output
    );
    println!(
        "execution_proof_bytes: {}",
        artifact.core_slice_artifact.execution_proof.proof.len()
    );
}

fn prepare_stwo_gemma_block_richer_slice_artifact_command(
    proof_path: &Path,
    chain_path: &Path,
    output: &Path,
) -> llm_provable_computer::Result<()> {
    ...
    #[cfg(feature = "stwo-backend")]
    {
        ...
        let core_slice_artifact =
            prepare_phase945_gemma_block_core_slice_artifact(&chain_artifact, &execution_proof)?;
        let artifact = prepare_phase9475_gemma_block_richer_slice_artifact(&core_slice_artifact)?;
        save_phase9475_gemma_block_richer_slice_artifact(&artifact, output)?;

        println!("output: {}", output.display());
        println!("proof: {}", proof_path.display());
        println!("chain: {}", chain_path.display());
        println!("verified_stark: true");
        print_phase9475_gemma_block_richer_slice_report(&artifact);
        Ok(())
    }
}

fn verify_stwo_gemma_block_richer_slice_artifact_command(
    artifact_path: &Path,
) -> llm_provable_computer::Result<()> {
    #[cfg(not(feature = "stwo-backend"))]
    {
    ...
    #[cfg(feature = "stwo-backend")]
    {
        ...
        println!("verified_stark: true");
        println!(
            "expected_artifact_version: {STWO_GEMMA_BLOCK_RICHER_SLICE_ARTIFACT_VERSION_PHASE9475}"
        );
        println!(
            "expected_semantic_scope: {STWO_GEMMA_BLOCK_RICHER_SLICE_ARTIFACT_SCOPE_PHASE9475}"
        );
        print_phase9475_gemma_block_richer_slice_report(&artifact);
        Ok(())
    }
...
#[cfg(feature = "stwo-backend")]
fn print_phase95_repeated_gemma_slice_accumulation_report(
...
#[cfg(feature = "stwo-backend")]
fn print_phase99_multi_interval_gemma_richer_family_accumulation_report(
    ...
    println!("total_intervals: {}", artifact.total_intervals);
    ...
    let naive_interval_repeated_proof_bytes =
        artifact.shared_execution_proof.proof.len() * artifact.total_intervals;
    println!("naive_interval_repeated_proof_bytes: {naive_interval_repeated_proof_bytes}");
}

#[cfg(feature = "stwo-backend")]
fn print_phase1015_folded_multi_interval_gemma_accumulation_prototype_report(
    ...
    println!("program_label: {}", artifact.program_label);
    println!(
        "source_phase99_artifact_commitment: {}",
        artifact.source_phase99_artifact_commitment
    );
    println!(
        "source_interval_members_commitment: {}",
        artifact.source_interval_members_commitment
    );
    println!(
        "shared_primitive_artifact_commitment: {}",
        artifact.shared_primitive_artifact_commitment
...
#[cfg(feature = "stwo-backend")]
fn print_phase102_folded_multi_interval_gemma_richer_family_report(
    ...
    println!(
        "source_phase1015_artifact_commitment: {}",
        artifact.source_phase1015_artifact_commitment
    );
    println!(
        "source_interval_members_commitment: {}",
        artifact.source_interval_members_commitment
...
#[cfg(feature = "stwo-backend")]
fn print_phase106_folded_repeated_multi_interval_gemma_accumulation_prototype_report(
    ...
    println!("program_label: {}", artifact.program_label);
    println!(
        "source_phase105_artifact_commitment: {}",
        artifact.source_phase105_artifact_commitment
    );
    println!(
        "source_window_members_commitment: {}",
        artifact.source_window_members_commitment
    );
    println!(
        "shared_primitive_artifact_commitment: {}",
        artifact.shared_primitive_artifact_commitment
...
#[cfg(feature = "stwo-backend")]
fn print_phase107_folded_repeated_multi_interval_gemma_richer_family_report(
    ...
    println!(
        "source_phase106_artifact_commitment: {}",
        artifact.source_phase106_artifact_commitment
    );
    println!("total_windows: {}", artifact.total_windows);
    println!("intervals_per_window: {}", artifact.intervals_per_window);
    println!("interval_total_slices: {}", artifact.interval_total_slices);
...
#[cfg(feature = "stwo-backend")]
fn print_phase112_transformer_accumulation_semantics_report(
    ...
    println!(
        ...
        artifact.repeated_window_schedule_commitment
    );
    println!(
        "accumulation_semantics_commitment: {}",
        artifact.accumulation_semantics_commitment
    );
    println!("local_score_sum: {}", artifact.local_score_sum);
    println!("global_score_sum: {}", artifact.global_score_sum);
    println!("grouped_value_mix_sum: {}", artifact.grouped_value_mix_sum);
...
fn prepare_stwo_repeated_gemma_slice_accumulation_artifact_command(
    ...
    #[cfg(feature = "stwo-backend")]
    {
        ...
        save_phase95_repeated_gemma_slice_accumulation_artifact(&artifact, output)?;
...
fn verify_stwo_repeated_gemma_slice_accumulation_artifact_command(
    ...
    #[cfg(feature = "stwo-backend")]
    {
        ...
        println!("verified_stark: true");
        println!(
            "expected_artifact_version: {STWO_REPEATED_GEMMA_SLICE_ACCUMULATION_ARTIFACT_VERSION_PHASE95}"
        );
        println!(
...
fn prepare_stwo_multi_interval_gemma_richer_family_accumulation_artifact_command(
    proof_path: &Path,
    total_intervals: usize,
    interval_total_slices: usize,
    token_position_start: u64,
    ...
    #[cfg(not(feature = "stwo-backend"))]
    {
        ...
        return Err(VmError::UnsupportedProof(
            "S-two multi-interval richer-family accumulation artifact requires building with `--features stwo-backend`"
                .to_string(),
        ));
    }
    ...
    #[cfg(feature = "stwo-backend")]
    {
        require_stwo_backend("S-two multi-interval richer-family accumulation artifact")?;
        if total_intervals > MAX_PHASE99_MULTI_INTERVAL_TOTAL_INTERVALS {
            return Err(VmError::InvalidConfig(format!(
                "S-two multi-interval richer-family accumulation supports at most {} intervals",
                MAX_PHASE99_MULTI_INTERVAL_TOTAL_INTERVALS
            )));
        }
        let execution_proof = load_execution_stark_proof(proof_path)?;
        ...
        save_phase99_multi_interval_gemma_richer_family_accumulation_artifact(&artifact, output)?;

        println!("output: {}", output.display());
        println!("proof: {}", proof_path.display());
        println!("verified_stark: true");
        print_phase99_multi_interval_gemma_richer_family_accumulation_report(&artifact);
        Ok(())
    }
}

fn verify_stwo_multi_interval_gemma_richer_family_accumulation_artifact_command(
    artifact_path: &Path,
) -> llm_provable_computer::Result<()> {
    #[cfg(not(feature = "stwo-backend"))]
    {
    ...
    #[cfg(feature = "stwo-backend")]
    {
        require_stwo_backend("S-two multi-interval richer-family accumulation artifact")?;
        let artifact =
            load_phase99_multi_interval_gemma_richer_family_accumulation_artifact(artifact_path)?;
        ...
        println!("verified_stark: true");
        println!(
            "expected_artifact_version: {STWO_MULTI_INTERVAL_GEMMA_RICHER_FAMILY_ACCUMULATION_ARTIFACT_VERSION_PHASE99}"
        );
        println!(
            "expected_semantic_scope: {STWO_MULTI_INTERVAL_GEMMA_RICHER_FAMILY_ACCUMULATION_ARTIFACT_SCOPE_PHASE99}"
        );
        print_phase99_multi_interval_gemma_richer_family_accumulation_report(&artifact);
        Ok(())
    }
...
fn prepare_stwo_folded_multi_interval_gemma_accumulation_prototype_artifact_command(
    source_path: &Path,
    output: &Path,
) -> llm_provable_computer::Result<()> {
    #[cfg(not(feature = "stwo-backend"))]
    ...
    #[cfg(feature = "stwo-backend")]
    {
        ...
        println!("source: {}", source_path.display());
        println!("verified_artifact: true");
        print_phase1015_folded_multi_interval_gemma_accumulation_prototype_report(&artifact);
        Ok(())
    }
}

fn verify_stwo_folded_multi_interval_gemma_accumulation_prototype_artifact_command(
    artifact_path: &Path,
    source_path: &Path,
) -> llm_provable_computer::Result<()> {
    #[cfg(not(feature = "stwo-backend"))]
    ...
    #[cfg(feature = "stwo-backend")]
    {
        ...
        println!("verified_artifact: true");
        println!(
            "expected_artifact_version: {STWO_FOLDED_MULTI_INTERVAL_GEMMA_ACCUMULATION_PROTOTYPE_ARTIFACT_VERSION_PHASE1015}"
        );
        println!(
            "expected_semantic_scope: {STWO_FOLDED_MULTI_INTERVAL_GEMMA_ACCUMULATION_PROTOTYPE_ARTIFACT_SCOPE_PHASE1015}"
        );
        print_phase1015_folded_multi_interval_gemma_accumulation_prototype_report(&artifact);
        Ok(())
    }
...
fn prepare_stwo_folded_multi_interval_gemma_richer_family_artifact_command(
    source_path: &Path,
    folded_path: &Path,
    output: &Path,
) -> llm_provable_computer::Result<()> {
    ...
    #[cfg(feature = "stwo-backend")]
    {
        ...
        save_phase102_folded_multi_interval_gemma_richer_family_artifact(&artifact, output)?;
...
fn verify_stwo_folded_multi_interval_gemma_richer_family_artifact_command(
    artifact_path: &Path,
    source_path: &Path,
    folded_path: &Path,
) -> llm_provable_computer::Result<()> {
    ...
    #[cfg(feature = "stwo-backend")]
    {
        ...
        let artifact =
            load_phase102_folded_multi_interval_gemma_richer_family_artifact(artifact_path)?;
        verify_phase102_folded_multi_interval_gemma_richer_family_artifact(
            &artifact, &source, &folded,
        )?;
        ...
        println!("verified_artifact: true");
        println!(
            "expected_artifact_version: {STWO_FOLDED_MULTI_INTERVAL_GEMMA_RICHER_FAMILY_ARTIFACT_VERSION_PHASE102}"
        );
        println!(
...
fn prepare_stwo_folded_repeated_multi_interval_gemma_accumulation_prototype_artifact_command(
    source_path: &Path,
    output: &Path,
) -> llm_provable_computer::Result<()> {
    #[cfg(not(feature = "stwo-backend"))]
    ...
    #[cfg(feature = "stwo-backend")]
    {
        ...
        println!("source: {}", source_path.display());
        println!("verified_artifact: true");
        print_phase106_folded_repeated_multi_interval_gemma_accumulation_prototype_report(
            &artifact,
        );
        Ok(())
    }
...
fn verify_stwo_folded_repeated_multi_interval_gemma_accumulation_prototype_artifact_command(
    artifact_path: &Path,
    source_path: &Path,
) -> llm_provable_computer::Result<()> {
    #[cfg(not(feature = "stwo-backend"))]
    ...
    #[cfg(feature = "stwo-backend")]
    {
        ...
        println!("verified_artifact: true");
        println!(
            "expected_artifact_version: {STWO_FOLDED_REPEATED_MULTI_INTERVAL_GEMMA_ACCUMULATION_PROTOTYPE_ARTIFACT_VERSION_PHASE106}"
        );
        println!(
            "expected_semantic_scope: {STWO_FOLDED_REPEATED_MULTI_INTERVAL_GEMMA_ACCUMULATION_PROTOTYPE_ARTIFACT_SCOPE_PHASE106}"
        );
        print_phase106_folded_repeated_multi_interval_gemma_accumulation_prototype_report(
            &artifact,
        );
...
fn reject_phase107_folded_repeated_multi_interval_richer_family_plain_json_gzip_output(
    output: &Path,
) -> llm_provable_computer::Result<()> {
    if output.extension().and_then(|extension| extension.to_str()) == Some("gz") {
        return Err(VmError::InvalidConfig(
            "prepare-stwo-folded-repeated-multi-interval-linear-block-richer-family-artifact writes plain JSON; use a `.json` output path"
                .to_string(),
        ));
    }
    Ok(())
...
fn reject_phase109_transformer_specific_fold_operator_plain_json_gzip_output(
    output: &Path,
) -> llm_provable_computer::Result<()> {
    if output.extension().and_then(|extension| extension.to_str()) == Some("gz") {
        return Err(VmError::InvalidConfig(
            "prepare-stwo-transformer-specific-fold-operator-artifact writes plain JSON; use a `.json` output path"
                .to_string(),
        ));
    }
    Ok(())
...
fn reject_phase110_repeated_window_fold_tree_plain_json_gzip_output(
    output: &Path,
) -> llm_provable_computer::Result<()> {
    if output.extension().and_then(|extension| extension.to_str()) == Some("gz") {
        return Err(VmError::InvalidConfig(
            "prepare-stwo-repeated-window-fold-tree-artifact writes plain JSON; use a `.json` output path"
                .to_string(),
        ));
    }
    Ok(())
...
fn reject_phase112_transformer_accumulation_semantics_plain_json_gzip_output(
    output: &Path,
) -> llm_provable_computer::Result<()> {
    if output.extension().and_then(|extension| extension.to_str()) == Some("gz") {
        return Err(VmError::InvalidConfig(
            "prepare-stwo-transformer-accumulation-semantics-artifact writes plain JSON; use a `.json` output path"
                .to_string(),
        ));
    }
    Ok(())
...
fn reject_phase113_richer_window_family_plain_json_gzip_output(
    output: &Path,
) -> llm_provable_computer::Result<()> {
    if output.extension().and_then(|extension| extension.to_str()) == Some("gz") {
        return Err(VmError::InvalidConfig(
            "prepare-stwo-richer-linear-block-window-family-artifact writes plain JSON; use a `.json` output path"
                .to_string(),
        ));
    }
    Ok(())
...
fn prepare_stwo_folded_repeated_multi_interval_gemma_richer_family_artifact_command(
    source_path: &Path,
    folded_path: &Path,
    output: &Path,
) -> llm_provable_computer::Result<()> {
    #[cfg(not(feature = "stwo-backend"))]
    {
        let _ = (source_path, folded_path, output);
        return Err(VmError::UnsupportedProof(
            "S-two folded repeated multi-interval richer-family artifact requires building with `--features stwo-backend`"
                .to_string(),
        ));
    }
    ...
    #[cfg(feature = "stwo-backend")]
    {
        require_stwo_backend("S-two folded repeated multi-interval richer-family artifact")?;
        reject_phase107_folded_repeated_multi_interval_richer_family_plain_json_gzip_output(
            output,
        )?;
        let source =
            load_phase105_repeated_multi_interval_gemma_richer_family_accumulation_artifact(
...
fn verify_stwo_folded_repeated_multi_interval_gemma_richer_family_artifact_command(
    artifact_path: &Path,
    source_path: &Path,
    folded_path: &Path,
) -> llm_provable_computer::Result<()> {
    ...
    #[cfg(feature = "stwo-backend")]
    {
        ...
        let artifact = load_phase107_folded_repeated_multi_interval_gemma_richer_family_artifact(
            artifact_path,
        )?;
        verify_phase107_folded_repeated_multi_interval_gemma_richer_family_artifact(
            &artifact, &source, &folded,
        ...
        println!("verified_artifact: true");
        println!(
            "expected_artifact_version: {STWO_FOLDED_REPEATED_MULTI_INTERVAL_GEMMA_RICHER_FAMILY_ARTIFACT_VERSION_PHASE107}"
        );
        println!(
...
fn prepare_stwo_transformer_specific_fold_operator_artifact_command(
    ...
    #[cfg(feature = "stwo-backend")]
    {
        ...
        reject_phase109_transformer_specific_fold_operator_plain_json_gzip_output(output)?;
        let left =
...
fn prepare_stwo_repeated_window_fold_tree_artifact_command(
    ...
    #[cfg(feature = "stwo-backend")]
    {
        ...
        reject_phase110_repeated_window_fold_tree_plain_json_gzip_output(output)?;
        let leaves = load_phase107_leaf_family(leaf_paths)?;
        ...
        println!("output: {}", output.display());
        println!("leaf_count: {}", leaf_paths.len());
        println!("verified_artifact: true");
...
fn verify_stwo_repeated_window_fold_tree_artifact_command(
    ...
    #[cfg(not(feature = "stwo-backend"))]
    {
        let _ = (artifact_path, leaf_paths);
        return Err(VmError::UnsupportedProof(
            "S-two repeated-window fold tree artifact requires building with `--features stwo-backend`"
                .to_string(),
        ));
...
fn prepare_stwo_transformer_accumulation_semantics_artifact_command(
    leaf_paths: &[PathBuf],
    output: &Path,
) -> llm_provable_computer::Result<()> {
    #[cfg(not(feature = "stwo-backend"))]
    {
        let _ = (leaf_paths, output);
        return Err(VmError::UnsupportedProof(
            "S-two transformer accumulation semantics artifact requires building with `--features stwo-backend`"
                .to_string(),
        ));
    }
    ...
    #[cfg(feature = "stwo-backend")]
    {
        ...
        reject_phase112_transformer_accumulation_semantics_plain_json_gzip_output(output)?;
        let leaves = load_phase107_leaf_family(leaf_paths)?;
        let artifact = prepare_phase112_transformer_accumulation_semantics_artifact(&leaves)?;
        save_phase112_transformer_accumulation_semantics_artifact(&artifact, output)?;
...
fn verify_stwo_transformer_accumulation_semantics_artifact_command(
    artifact_path: &Path,
    leaf_paths: &[PathBuf],
) -> llm_provable_computer::Result<()> {
    #[cfg(not(feature = "stwo-backend"))]
    ...
    #[cfg(feature = "stwo-backend")]
    {
        require_stwo_backend("S-two transformer accumulation semantics artifact")?;
        let leaves = load_phase107_leaf_family(leaf_paths)?;
        let artifact = load_phase112_transformer_accumulation_semantics_artifact(artifact_path)?;
        ...
        println!("verified_artifact: true");
        println!(
            "expected_artifact_version: {STWO_TRANSFORMER_ACCUMULATION_SEMANTICS_ARTIFACT_VERSION_PHASE112}"
        );
        println!(
...
fn prepare_stwo_richer_gemma_window_family_artifact_command(
    ...
    #[cfg(not(feature = "stwo-backend"))]
    {
        let _ = (semantics_path, leaf_paths, output);
        return Err(VmError::UnsupportedProof(
            "S-two richer linear-block window-family artifact requires building with `--features stwo-backend`"
                .to_string(),
        ));
    }
    ...
    #[cfg(feature = "stwo-backend")]
    {
        require_stwo_backend("S-two richer linear-block window-family artifact")?;
        reject_phase113_richer_window_family_plain_json_gzip_output(output)?;
        let leaves = load_phase107_leaf_family(leaf_paths)?;
...
fn verify_stwo_richer_gemma_window_family_artifact_command(
    ...
    #[cfg(not(feature = "stwo-backend"))]
    {
        let _ = (artifact_path, semantics_path, leaf_paths);
        return Err(VmError::UnsupportedProof(
            "S-two richer linear-block window-family artifact requires building with `--features stwo-backend`"
                .to_string(),
        ));
...
mod cli_dispatch_tests {
    ...
    #[test]
    #[cfg(not(feature = "stwo-backend"))]
    fn phase107_to_phase113_commands_still_parse_without_stwo_backend_feature() {
        let parse_cases = [
            (
                vec![
                    "tvm",
        ...
        for (args, label) in parse_cases {
            Cli::try_parse_from(args).unwrap_or_else(|error| {
                panic!("{label} should parse without stwo-backend feature: {error}")
            });
        }
    }
    ...
    #[test]
    #[cfg(not(feature = "stwo-backend"))]
    fn phase112_prepare_command_reports_feature_requirement_without_reading_inputs() {
        let err = prepare_stwo_transformer_accumulation_semantics_artifact_command(
            &[
                PathBuf::from("missing-leaf-0.json"),
                PathBuf::from("missing-leaf-1.json"),
            ],
            Path::new("out.json"),
        )
        .expect_err("phase112 prepare should require stwo-backend in no-feature builds");
        assert!(err
            .to_string()
            .contains("requires building with `--features stwo-backend`"));
    }

    #[test]
    #[cfg(not(feature = "stwo-backend"))]
    fn phase113_linear_block_alias_maps_to_the_expected_command_variant() {
        ...
        .expect("linear-block alias should parse");

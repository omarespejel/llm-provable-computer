...
enum Command {
    ...
    #[cfg(feature = "stwo-backend")]
    /// Prepare a richer-family repeated-window artifact on top of a Phase105 source and Phase106 folded prototype.
    ...
    #[cfg(feature = "stwo-backend")]
    /// Verify a richer-family repeated-window artifact on top of a Phase105 source and Phase106 folded prototype.
    ...
    #[cfg(feature = "stwo-backend")]
    /// Prepare a compact transformer-specific accumulation semantics artifact over a power-of-two family of contiguous Phase107 repeated-window artifacts.
    ...
    #[cfg(feature = "stwo-backend")]
    /// Verify a compact transformer-specific accumulation semantics artifact over a power-of-two family of contiguous Phase107 repeated-window artifacts.
...
fn prepare_stwo_folded_repeated_multi_interval_gemma_richer_family_artifact_command(
    ...
    #[cfg(not(feature = "stwo-backend"))]
    {
        let _ = (source_path, folded_path, output);
        return Err(VmError::UnsupportedProof(
            "S-two folded repeated multi-interval Gemma richer-family artifact requires building with `--features stwo-backend`"
                .to_string(),
        ));
    }
    ...
    #[cfg(feature = "stwo-backend")]
    {
        require_stwo_backend("S-two folded repeated multi-interval Gemma richer-family artifact")?;
        let source =
            load_phase105_repeated_multi_interval_gemma_richer_family_accumulation_artifact(
                source_path,
            )?;
...
fn prepare_stwo_transformer_accumulation_semantics_artifact_command(
    ...
    #[cfg(feature = "stwo-backend")]
    {
        ...
        let leaf_artifacts = leaves
            ...
            .collect::<llm_provable_computer::Result<Vec<_>>>()?;
        let artifact =
            prepare_phase112_transformer_accumulation_semantics_artifact(&leaf_artifacts)?;
...
fn verify_stwo_transformer_accumulation_semantics_artifact_command(
    ...
    #[cfg(feature = "stwo-backend")]
    {
        ...
        verify_phase112_transformer_accumulation_semantics_artifact(&artifact, &leaf_artifacts)?;

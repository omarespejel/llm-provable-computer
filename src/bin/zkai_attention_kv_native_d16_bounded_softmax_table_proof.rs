...
fn main() -> ExitCode {
    eprintln!(
        "zkai_attention_kv_native_d16_bounded_softmax_table_proof requires --features stwo-backend"
    );
    ExitCode::from(2)
}
...
fn run() -> Result<String, String> {
    ...
    match mode.as_str() {
        "prove" => {
            ...
            let input = zkai_attention_kv_native_d16_bounded_softmax_table_input_from_json_str(raw)
            ...
            Ok(serde_json::json!({
                "schema": "zkai-attention-kv-stwo-native-d16-bounded-softmax-table-cli-summary-v1",
                "mode": "prove",

...
fn main() -> ExitCode {
    eprintln!(
        "zkai_attention_kv_native_two_head_longseq_fused_softmax_table_proof requires --features stwo-backend"
    );
    ExitCode::from(2)
}
...
fn run() -> Result<String, String> {
    ...
    match mode.as_str() {
        "prove" => {
            ...
            Ok(serde_json::json!({
                "schema": "zkai-attention-kv-stwo-native-two-head-longseq-fused-softmax-table-cli-summary-v1",
                "mode": "prove",

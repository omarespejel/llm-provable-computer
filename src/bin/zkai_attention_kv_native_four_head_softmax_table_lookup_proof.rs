...
fn main() -> ExitCode {
    eprintln!(
        "zkai_attention_kv_native_four_head_softmax_table_lookup_proof requires --features stwo-backend"
    );
    ExitCode::from(2)
}
...
fn run() -> Result<String, String> {
    ...
    if args.is_empty() {
        return Err("usage: zkai_attention_kv_native_four_head_softmax_table_lookup_proof prove <source-input.json> <lookup-envelope.json> | verify <lookup-envelope.json>".to_string());
    }
    ...
    match mode.as_str() {
        "prove" => {
            ...
            let envelope = prove_zkai_attention_kv_native_four_head_softmax_table_lookup_envelope(
                &source_input,
            )
            .map_err(|error| error.to_string())?;
            ...
            Ok(serde_json::json!({
                "schema": "zkai-attention-kv-stwo-native-four-head-softmax-table-logup-sidecar-cli-summary-v1",
                "mode": "prove",

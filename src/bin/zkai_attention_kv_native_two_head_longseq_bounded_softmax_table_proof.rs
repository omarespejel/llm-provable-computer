...
fn main() -> ExitCode {
    eprintln!(
        "zkai_attention_kv_native_two_head_longseq_bounded_softmax_table_proof requires --features stwo-backend"
    );
    ExitCode::from(2)
}
...
fn run() -> Result<String, String> {
    ...
    if args.is_empty() {
        return Err("usage: zkai_attention_kv_native_two_head_longseq_bounded_softmax_table_proof prove <input.json> <envelope.json> | verify <envelope.json>".to_string());
    }

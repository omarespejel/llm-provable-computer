...
use llm_provable_computer::stwo_backend::{
    ...
    zkai_attention_kv_native_bounded_weighted_input_from_json_str,
...
fn main() -> ExitCode {
    eprintln!("zkai_attention_kv_native_bounded_weighted_proof requires --features stwo-backend");
    ExitCode::from(2)
}
...
fn run() -> Result<String, String> {
    ...
    if args.is_empty() {
        return Err("usage: zkai_attention_kv_native_bounded_weighted_proof prove <input.json> <envelope.json> | verify <envelope.json>".to_string());
    }
    ...
    match mode.as_str() {
        "prove" => {
            ...
            let input = zkai_attention_kv_native_bounded_weighted_input_from_json_str(raw)
            ...
            verify_zkai_attention_kv_native_bounded_weighted_envelope(&envelope)
                .map_err(|error| error.to_string())?;
            ...
            Ok(serde_json::json!({
                "schema": "zkai-attention-kv-stwo-native-bounded-weighted-cli-summary-v1",
                "mode": "prove",
        ...
        "verify" => {
            ...
            let envelope = zkai_attention_kv_native_bounded_weighted_envelope_from_json_slice(&raw)
                .map_err(|error| error.to_string())?;
            let verify_started = Instant::now();
            verify_zkai_attention_kv_native_bounded_weighted_envelope(&envelope)
                .map_err(|error| error.to_string())?;

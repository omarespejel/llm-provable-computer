...
fn main() -> ExitCode {
    eprintln!("zkai_d128_gate_value_activation_fused_proof requires --features stwo-backend");
    ExitCode::from(2)
}
...
fn run() -> Result<String, String> {
    ...
    if args.is_empty() {
        return Err("usage: zkai_d128_gate_value_activation_fused_proof build-input <gate-value.json> <activation.json> <input.json> | prove <input.json> <envelope.json> | verify <envelope.json>".to_string());
    }
    ...
    match mode.as_str() {
        "build-input" => {
            if args.len() != 3 {
                return Err(
                    "usage: build-input <gate-value.json> <activation.json> <input.json>"
                        .to_string(),
                );
            }
            ...
            let activation = zkai_d128_activation_swiglu_input_from_json_str(&activation_raw)
                .map_err(|error| error.to_string())?;
            let input = build_zkai_d128_gate_value_activation_fused_input(gate, activation)
                .map_err(|error| error.to_string())?;
            ...
            atomic_write_file(&input_path, &input_bytes, "fused input")?;
            Ok(serde_json::json!({
                "schema": "zkai-d128-gate-value-activation-fused-proof-cli-summary-v1",
                "mode": "build-input",
                ...
                "claim_boundary": "fused_input_not_full_block_not_nanozk_win",
            })
            .to_string())
        }
        "prove" => {
            ...
            let input = zkai_d128_gate_value_activation_fused_input_from_json_str(&raw)
                .map_err(|error| error.to_string())?;
            ...
            Ok(serde_json::json!({
                ...
                "claim_boundary": "d128_gate_value_activation_fused_native_proof_not_full_block_not_nanozk_win",
            })
            .to_string())
        }
        "verify" => {
            ...
            let envelope = zkai_d128_gate_value_activation_fused_envelope_from_json_slice(&raw)
                .map_err(|error| error.to_string())?;
            verify_zkai_d128_gate_value_activation_fused_envelope(&envelope)
                .map_err(|error| error.to_string())?;
            Ok(serde_json::json!({
...
fn atomic_write_file(path: &Path, bytes: &[u8], label: &str) -> Result<(), String> {
    ...
    let tmp = parent.join(format!(
        ...
        path.file_name()
            ...
            .unwrap_or("gate-value-activation-fused"),
        unique
    ));
    {

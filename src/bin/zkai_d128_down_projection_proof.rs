...
use llm_provable_computer::stwo_backend::{
    ...
    zkai_d128_down_projection_input_from_json_str, ZkAiD128DownProjectionEnvelope,
    ZKAI_D128_DOWN_PROJECTION_MAX_JSON_BYTES, ZKAI_D128_DOWN_PROJECTION_MAX_PROOF_BYTES,
};
...
fn main() -> ExitCode {
    eprintln!("zkai_d128_down_projection_proof requires --features stwo-backend");
    ExitCode::from(2)
}
...
fn run() -> Result<String, String> {
    ...
    if args.is_empty() {
        return Err(
            "usage: zkai_d128_down_projection_proof prove <input.json> <envelope.json> | verify <envelope.json>"
                .to_string(),
        );
    }
    ...
    match mode.as_str() {
        "prove" => {
            ...
            let input = zkai_d128_down_projection_input_from_json_str(&raw)
                .map_err(|error| error.to_string())?;
            let envelope = prove_zkai_d128_down_projection_envelope(&input)
                .map_err(|error| error.to_string())?;
            verify_zkai_d128_down_projection_envelope(&envelope)
                .map_err(|error| error.to_string())?;
            let envelope_bytes = serde_json::to_vec_pretty(&envelope)
                .map_err(|error| format!("failed to serialize envelope: {error}"))?;
            if envelope_bytes.len() > ZKAI_D128_DOWN_PROJECTION_MAX_PROOF_BYTES {
                return Err(format!(
                    "down-projection envelope JSON exceeds bounded verifier limit: got {} bytes",
                    envelope_bytes.len()
                ));
            }
            atomic_write_file(&envelope_path, &envelope_bytes, "down-projection envelope")?;
            Ok(serde_json::json!({
                "schema": "zkai-d128-down-projection-proof-cli-summary-v1",
                "mode": "prove",
                ...
                "claim_boundary": "d128_down_projection_native_proof_not_full_block_not_nanozk_win",
            })
            .to_string())
        }
        "verify" => {
            ...
            let raw = read_bounded_file(
                ...
                "down-projection envelope JSON",
            )?;
            let envelope: ZkAiD128DownProjectionEnvelope = serde_json::from_slice(&raw)
                .map_err(|error| format!("failed to parse down-projection envelope: {error}"))?;
            verify_zkai_d128_down_projection_envelope(&envelope)
                .map_err(|error| error.to_string())?;
            Ok(serde_json::json!({
                "schema": "zkai-d128-down-projection-proof-cli-summary-v1",
...
fn atomic_write_file(path: &Path, bytes: &[u8], label: &str) -> Result<(), String> {
    ...
    let tmp = parent.join(format!(
        ...
        path.file_name()
            ...
            .unwrap_or("d128-down-projection"),
        unique
    ));
    {

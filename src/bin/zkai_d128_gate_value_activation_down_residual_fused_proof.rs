...
use llm_provable_computer::stwo_backend::{
    ...
    zkai_d128_residual_add_input_from_json_str, ZKAI_D128_ACTIVATION_SWIGLU_MAX_JSON_BYTES,
...
fn main() -> ExitCode {
    eprintln!(
        "zkai_d128_gate_value_activation_down_residual_fused_proof requires --features stwo-backend"
    );
    ExitCode::from(2)
}
...
fn run() -> Result<String, String> {
    ...
    if args.is_empty() {
        return Err("usage: zkai_d128_gate_value_activation_down_residual_fused_proof build-input <gate-value.json> <activation.json> <down.json> <residual.json> <input.json> | prove <input.json> <envelope.json> | verify <envelope.json>".to_string());
    }
    ...
    match mode.as_str() {
        "build-input" => {
            if args.len() != 5 {
                return Err(
                    "usage: build-input <gate-value.json> <activation.json> <down.json> <residual.json> <input.json>"
                        .to_string(),
                );
            }
            ...
            let residual_path = PathBuf::from(&args[3]);
            ...
            let residual_raw = read_bounded_utf8(
                &residual_path,
            ...
            let down = zkai_d128_down_projection_input_from_json_str(&down_raw)
                .map_err(|error| error.to_string())?;
            let residual = zkai_d128_residual_add_input_from_json_str(&residual_raw)
                .map_err(|error| error.to_string())?;
            let input = build_zkai_d128_gate_value_activation_down_residual_fused_input(
            ...
            atomic_write_file(&input_path, &input_bytes, "down-residual-fused input")?;
            Ok(serde_json::json!({
                "schema": "zkai-d128-gate-value-activation-down-residual-fused-proof-cli-summary-v1",
                "mode": "build-input",
                ...
                "residual_add_input_path": residual_path.display().to_string(),
                ...
                "claim_boundary": "down_residual_fused_input_not_full_block_not_nanozk_win",
            })
            .to_string())
        }
        "prove" => {
            ...
            let raw = read_bounded_utf8(
                &input_path,
                ZKAI_D128_GATE_VALUE_ACTIVATION_FUSED_MAX_JSON_BYTES,
                "down-residual-fused input JSON",
            )?;
            let input =
                zkai_d128_gate_value_activation_down_residual_fused_input_from_json_str(&raw)
                    .map_err(|error| error.to_string())?;
            let envelope =
            ...
            Ok(serde_json::json!({
                ...
                "residual_add_row_count": envelope.input.residual_add_row_count,
                "claim_boundary": "d128_gate_value_activation_down_residual_fused_native_proof_not_full_block_not_nanozk_win",
            })
            .to_string())
        }
        "verify" => {
            ...
            let raw = read_bounded_file(
                &envelope_path,
                ZKAI_D128_GATE_VALUE_ACTIVATION_FUSED_MAX_ENVELOPE_JSON_BYTES,
                "down-residual-fused envelope JSON",
            )?;
            let envelope =
                zkai_d128_gate_value_activation_down_residual_fused_envelope_from_json_slice(&raw)
                    .map_err(|error| error.to_string())?;
            verify_zkai_d128_gate_value_activation_down_residual_fused_envelope(&envelope)
                .map_err(|error| error.to_string())?;
            Ok(serde_json::json!({
...
fn atomic_write_file(path: &Path, bytes: &[u8], label: &str) -> Result<(), String> {
    ...
    let tmp = parent.join(format!(
        ...
        path.file_name()
            ...
            .unwrap_or("gate-value-activation-down-residual-fused"),
        unique
    ));
    {

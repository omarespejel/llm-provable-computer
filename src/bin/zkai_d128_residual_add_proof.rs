...
use llm_provable_computer::stwo_backend::{
    ...
    zkai_d128_residual_add_input_from_json_str, ZkAiD128ResidualAddEnvelope,
...
fn main() -> ExitCode {
    eprintln!("zkai_d128_residual_add_proof requires --features stwo-backend");
    ExitCode::from(2)
}
...
fn run() -> Result<String, String> {
    ...
    if args.is_empty() {
        return Err(
            "usage: zkai_d128_residual_add_proof prove <input.json> <envelope.json> | verify <envelope.json>"
                .to_string(),
        );
    }
    ...
    match mode.as_str() {
        "prove" => {
            ...
            let input = zkai_d128_residual_add_input_from_json_str(&raw)
                .map_err(|error| error.to_string())?;
            let envelope =
                prove_zkai_d128_residual_add_envelope(&input).map_err(|error| error.to_string())?;
            verify_zkai_d128_residual_add_envelope(&envelope).map_err(|error| error.to_string())?;
            let envelope_bytes = serde_json::to_vec_pretty(&envelope)
                .map_err(|error| format!("failed to serialize envelope: {error}"))?;
            if envelope_bytes.len() > ZKAI_D128_RESIDUAL_ADD_MAX_PROOF_BYTES {
                return Err(format!(
                    "residual-add envelope JSON exceeds bounded verifier limit: got {} bytes",
                    envelope_bytes.len()
                ));
            }
            atomic_write_file(&envelope_path, &envelope_bytes, "residual-add envelope")?;
            Ok(serde_json::json!({
                "schema": "zkai-d128-residual-add-proof-cli-summary-v1",
                "mode": "prove",
                ...
                "claim_boundary": "d128_residual_add_native_proof_not_full_block_not_nanozk_win",
            })
            .to_string())
        }
        "verify" => {
            ...
            let raw = read_bounded_file(
                ...
                "residual-add envelope JSON",
            )?;
            let envelope: ZkAiD128ResidualAddEnvelope = serde_json::from_slice(&raw)
                .map_err(|error| format!("failed to parse residual-add envelope: {error}"))?;
            verify_zkai_d128_residual_add_envelope(&envelope).map_err(|error| error.to_string())?;
            Ok(serde_json::json!({
                "schema": "zkai-d128-residual-add-proof-cli-summary-v1",
                "mode": "verify",

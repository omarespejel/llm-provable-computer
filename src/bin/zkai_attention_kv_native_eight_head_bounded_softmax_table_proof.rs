...
fn main() -> ExitCode {
    eprintln!(
        "zkai_attention_kv_native_eight_head_bounded_softmax_table_proof requires --features stwo-backend"
    );
    ExitCode::from(2)
}
...
fn run() -> Result<String, String> {
    ...
    if args.is_empty() {
        return Err("usage: zkai_attention_kv_native_eight_head_bounded_softmax_table_proof prove <input.json> <envelope.json> | verify <envelope.json>".to_string());
    }
    ...
    match mode.as_str() {
        "prove" => {
            ...
            Ok(serde_json::json!({
                "schema": "zkai-attention-kv-stwo-native-eight-head-bounded-softmax-table-cli-summary-v1",
                "mode": "prove",

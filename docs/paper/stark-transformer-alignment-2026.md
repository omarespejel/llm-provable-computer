...
The `stwo-experimental-v1` tier is the narrow S-two evidence tier. It contains command logs, wall-clock timings, SHA-256 hashes, and proof artifacts for four representative outputs: an arithmetic `statement-v1` proof (`addition`), a shared-table normalization lookup proof envelope, a Gemma-inspired fixed-shape proof (`gemma_block_v4`) with shared lookup bindings, and a three-step proof-carrying decoding chain [40]. Taken together, these four artifacts illustrate a representative subset of the repository's reusable proof surfaces: arithmetic, shared-table lookup, block-shaped execution, and carried-state decode chains. The broader public surface taxonomy also includes bounded multi-runtime semantic-agreement artifacts and pre-recursive aggregation bundles. Appendix C1 compares the two backend-facing tiers.

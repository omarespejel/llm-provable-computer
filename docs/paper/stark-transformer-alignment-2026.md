...
The artifact bundle includes STARK proofs for `addition`, `dot_product`, `single_neuron`, and `fibonacci`, along with `research-v2` semantic agreement artifacts. It also includes a committed transformer-specific attention-semantics fixture, `run_soft_attention_memory`, with benchmark entry, command log, and hash-anchored outputs in the same appendix bundle. After that frozen `production-v1` bundle, the repository added newer fixed-shape `stwo` artifacts under `docs/paper/artifacts/gemma-block-v1/` and `docs/paper/artifacts/gemma-block-v2/`. The stronger of the two is `gemma-block-v2/stwo-execution-proof.json`: a Gemma-inspired block checksum fixture whose top-level execution proof embeds the normalization lookup proof for the claimed `norm_sq = 16` and `inv_sqrt_q8 = 64` row inside the main serialized proof payload rather than carrying it only through sidecar metadata. The large proof JSON files themselves are intentionally left out of the original `production-v1` appendix bundle; what is committed in-repo is the stable metadata layer and selected newer artifact files needed for reproducibility and citation.

...
- Do not say the Stwo typed-size estimate is stable binary proof serialization.
  It uses Stwo's typed `StarkProof` estimate hook as engineering-only side
  evidence, but the stable binary serializer and fine-grained commitment /
  FRI-witness attribution remain NO-GOs. Do not put it in the circulation result
  table.
...
The Stwo typed-size estimate follow-up removes JSON-section length as the only
lens, but it stays engineering-only side evidence. This is still not stable
binary proof serialization and still not fine-grained binary commitment or
FRI-witness attribution; Stwo exposes the typed estimate hook, not a canonical
verifier-facing binary serializer or full binary component schema.

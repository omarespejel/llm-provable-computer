...
We now treat that rule as a checked comparison gate rather than an informal judgment.
The SOTA artifact watchlist records which systems are empirical adapter rows, which are
source-backed context, which are deployment calibration, and which are receipt or
settlement watchlist items. Its current classification is deliberately conservative:
EZKL, Circom/snarkjs, and JSTprove/Remainder are empirical statement-envelope adapter
rows; DeepProve-1, NANOZK, Jolt Atlas, and Giza/LuminAIR remain source-backed context
until public proof bytes and verifier inputs make baseline verification and mutation
testing reproducible; Obelyzk remains deployment calibration; RISC Zero, SP1, and
SNIP-36 remain receipt/settlement watchlist rows. The checked handle is
`docs/engineering/zkai-sota-artifact-watchlist-2026-05-03.md` with machine-readable
outputs at `docs/engineering/evidence/zkai-sota-artifact-watchlist-2026-05.json` and
...
proof bytes, so the fused route is `23705` bytes smaller (`0.732784x`). The
fused Softmax-table route matrix now has a checked section-delta read: fusion
saves `152991` bytes against matched source-plus-sidecar proofs, and `141125`
of those bytes are in the opening bucket. The useful claim is that fusion
mostly removes a second opening surface, not that the backend exposes
source-vs-lookup byte attribution. This aggregate section-delta result does not
make a per-head or per-component scaling claim; it only explains exposed
serialized STARK proof-section savings across matched profiles. The
section-delta result is anchored to
...
regenerated by
`python3 scripts/zkai_attention_kv_fused_softmax_table_section_delta_gate.py --no-write --json`.
A stricter Stwo typed-size estimate check keeps the same claim boundary but is
kept as engineering-only side evidence rather than a paper result row. It removes
JSON-section length as the only lens, but it is still typed Stwo size-estimate
evidence, not stable binary proof serialization, not fine-grained binary
...
under timing policy
`proof_component_size_accounting_only_not_timing_not_public_benchmark`, Rust
`nightly-2025-07-14`, and Cargo.lock-pinned Stwo `2.2.0`; the checked profiles
cover lookup-claim counts from `52` to `832` and trace rows from `64` to
`1024`.
A separate two-head
...
envelope, and rejects `19 / 19` mutations.
...
`NO_GO_EXECUTABLE_RECURSIVE_PCD_OUTER_PROOF_BACKEND_MISSING`: no nested verifier
program/AIR/circuit can express the two selected d128 slice verifier checks. It
rejects
...
unknown-field injection, and parser/schema mutations. This is useful negative
evidence because it prevents the
...
the SNARK branch became a checked statement-receipt GO, and the RISC Zero branch
became a checked zkVM statement-receipt GO over the same public-journal
contract. The route-selector gate
...
result; the issue `#428` and issue `#433` follow-ups now change the external
adapter branches of that answer. A real `snarkjs/Groth16` statement receipt
exists for the exact issue `#424` contract, and a real RISC Zero receipt exists
for the corresponding issue `#422` public journal. The updated backend gate
records
`GO_D128_EXTERNAL_SNARK_AND_ZKVM_STATEMENT_RECEIPT_BACKENDS_FOR_PROOF_NATIVE_TWO_SLICE_CONTRACT`,
while still keeping local nested-verifier AIR/circuit and local PCD/IVC in the
missing-backend bucket. It rejects `35 / 35`
...
external SNARK statement receipt and an external zkVM statement receipt, but not
a recursive proof and not a PCD proof. The issue `#430` follow-up adds
verifier/prover timing only for the SNARK statement-receipt adapter under local
throwaway setup; those timings are not recursive-proof or public zkML benchmark
evidence. The issue `#422` follow-up maps the same #424 public-input contract
into a concrete zkVM public journal/public-values contract. The issue `#433`
follow-up then verifies that journal with a real RISC Zero receipt. The RISC
Zero receipt binds the journal commitment
...
RISC Zero receipt is anchored to
`docs/engineering/zkai-d128-risc0-statement-receipt-2026-05-05.md` and
...
generation and `338.871 ms` for verification, with a `35485.173 ms` single
local setup run and `19 / 19` timing/setup/binding mutations rejected. Timing
...
objects for the same public-input contract. The #422 zkVM adapter gate and #433
RISC Zero receipt add one more guardrail: mapping a contract into a journal is
not the same thing as having a verified zkVM receipt, and having a verified zkVM
statement receipt is still not recursive verification of the underlying Stwo
slice proofs.
...
broader space: local recursion/PCD for the checked d128 two-slice target,
comparative external controls across SNARK and zkVM statement receipts, stronger
external calibration against deployed verifiers, statement-bound transformer
blocks with more model structure, a recursive layer that preserves the same boundary

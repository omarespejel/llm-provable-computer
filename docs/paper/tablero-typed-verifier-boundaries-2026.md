...
Reproducibility note for Table 3a: backend version pinned in
`docs/engineering/tablero-optimized-replay-redteam-gate-2026-04-27.md`
(the carry-aware experimental execution-proof backend); timing mode
`measured_median`, timing policy
`median_of_5_runs_from_microsecond_capture`, timing unit `milliseconds`,
step count `1024`; benchmark identity `benchmark_version
stwo-tablero-replay-breakdown-optimized-benchmark-v1`. The engineering
...
The shell script fails closed if the regenerated payload's identity drifts
from this scope. This is an explicit experimental-to-paper promotion of an
engineering-only red-team measurement; the optimized verifier is **not** the
publication-default verifier and the manifest format it consumes uses
binary commitments (the optimized manifest version and semantic-scope
identifiers are pinned in
`docs/engineering/tablero-optimized-replay-redteam-gate-2026-04-27.md`)

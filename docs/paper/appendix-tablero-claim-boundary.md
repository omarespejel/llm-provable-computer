...
| Native proof hardening | The d64 RMSNorm public-row slice now proves per-row arithmetic and a bounded public-scalar sqrt inequality in native Stwo AIR, binds a local RMSNorm output row commitment, and the follow-up bridge consumes that local commitment into a domain-separated projection-input commitment; evidence: `docs/engineering/evidence/zkai-d64-native-rmsnorm-public-row-proof-2026-05.json` and `docs/engineering/evidence/zkai-d64-rmsnorm-to-projection-bridge-proof-2026-05.json`; still no claim of private parameter openings, gate/value/down projection, activation, residual, or full-block output binding. |
...
| d64 native proof closure | No claim that the d64 RMSNorm-to-projection bridge proves gate/value/down projection, activation, residual rows, or the final `output_activation_commitment`; it only proves the domain-separated handoff into the next projection-input surface. |

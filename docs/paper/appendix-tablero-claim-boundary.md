...
| Native proof hardening | The d64 native path now proves the RMSNorm public-row slice, consumes its local output into a domain-separated projection-input bridge, and proves the `32,768` gate/value projection multiplication rows that consume `projection_input_row_commitment`; evidence: `docs/engineering/evidence/zkai-d64-native-rmsnorm-public-row-proof-2026-05.json`, `docs/engineering/evidence/zkai-d64-rmsnorm-to-projection-bridge-proof-2026-05.json`, and `docs/engineering/evidence/zkai-d64-gate-value-projection-proof-2026-05.json`; still no claim of private parameter openings, activation/SwiGLU, down projection, residual, or full-block output binding. |
...
| d64 native proof closure | No claim that the d64 native proof path proves activation/SwiGLU, down projection, residual rows, or the final `output_activation_commitment`; the current closure stops at the domain-separated gate/value projection output commitment. |

...
| Strongest public evidence | Official Lagrange product/blog materials                                                         | Public repo, verifier docs, audit notes, Starknet verification path                                                                                                                 | Checked evidence files for Figures 3, 4, and 4B, carried-state and proof-carrying bundles under `docs/paper/artifacts/`, and statement-versioned repo outputs               |
...
| Main caveat               | Strong public claims, but not directly comparable to STARK systems on identical workloads        | Public materials mix benchmark claims, verification demos, and roadmap claims; verifier docs still show uneven component maturity (`Attention` remains `Prover only`)               | Narrow scope: reuse-sensitive wins are local S-two calibration rows, not a matched full-model benchmark, and full learned-model proving is still out of scope                 |

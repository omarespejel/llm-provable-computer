...
| `RUSTSEC-2026-0097` (`rand 0.8.6`) | `cargo-audit` | `stwo`, `tract-onnx`, `tch`, and `ark-*` transitive chains | The current graph still requires `rand 0.8.6` through multiple upstream stacks; `cargo-deny` does not currently surface this advisory on this graph, so `cargo-audit` remains the enforcement point | Upgrade or patch the upstream stacks away from affected `rand` releases |

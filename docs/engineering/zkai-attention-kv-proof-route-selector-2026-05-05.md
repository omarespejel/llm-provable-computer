...
GO for fourteen narrow proof-backed routes, with the native Stwo routes now first:
...
7. a native Stwo d16 two-head long-sequence fused Softmax-table attention/LogUp
   proof over the combined width/head/sequence axis;
8. a native Stwo d16 implementation-exact quantized Softmax-table kernel
   receipt over the d16 fused attention/LogUp proof;
9. a native Stwo d16 two-head implementation-exact quantized Softmax-table
   kernel receipt over the combined width/head fused attention/LogUp proof;
10. an external `snarkjs/Groth16/BN128` statement receipt over the source-backed
   attention/KV transition contract;
11. a RISC Zero receipt whose guest computes the tiny integer-argmax
   attention/KV transition semantics under an explicit no-mask policy;
12. a RISC Zero receipt whose guest computes a three-step carried KV-cache
   sequence and commits every intermediate transition row;
13. a RISC Zero receipt whose guest computes a fixed eight-step carried KV-cache
   sequence and commits every intermediate transition row;
14. a RISC Zero receipt whose guest computes a fixed eight-step `d=8`
...
sequence-length, combined width/head, and combined width/head/sequence axes. The
checked matrix covers nine native Stwo fused rows: d8 single-head seq8, d16
single-head seq8, d8 two-head seq8, d8 four-head seq8, d8 eight-head seq8, d8
sixteen-head seq8, d8 two-head seq16, d16 two-head seq8, and d16 two-head
seq16. Matched source-plus-sidecar controls now exist for all nine rows. The
useful axis-level signal is: d8 to d16 grows fused proof bytes `1.352321x` at
fixed `52` lookup claims; one to sixteen heads grows lookup claims
`16.000000x` while fused proof bytes grow `1.362866x`; eight to sixteen heads
doubles lookup claims while fused proof bytes grow `1.094838x`; two-head seq8
to seq16 grows lookup claims `3.230769x` while fused proof bytes grow
`1.222065x`; and d16 two-head seq8 to seq16 grows lookup claims `3.230769x`
and trace rows `4.000000x` while fused proof bytes grow `1.085116x`. The
combined d16 two-head long-sequence row checks `336` lookup claims over `512`
trace rows with an `84868`-byte fused proof, `23290` bytes smaller than its
matched source-plus-sidecar control (`108158` bytes, `0.784667x`). It rejects
`26 / 26` matrix drift, provenance-drift, and overclaim mutations. This is
...
Issue `#524` promotes that same d16 two-head fused proof into an
implementation-exact quantized Softmax-table receipt. The receipt pins key/value
width `16`, head count `2`, eight steps per head, `104` lookup claims / score
rows, a `128`-row trace, the literal nine-row weight table, positive per-head
denominators, Euclidean quotient/remainder semantics, output order derived from
statement `input_steps`, and the `< 1` output-unit division-error bound. It
keeps the backing proof size at `78211` raw bytes, records `921008` checked
envelope bytes, and rejects `43 / 43` semantic/proof mutations. This is exact
for the pinned integer table/floor-division kernel on the combined width/head
fixture, not real-valued Softmax, full inference, long-context inference,
timing evidence, recursion, or PCD.

Issue `#525` combines the width, head-count, and sequence-length axes in one
native Stwo fused route. The checked proof uses `d=16`, two heads, and sixteen
...
eight-head, sixteen-head, long-sequence, d16 width-axis, d16 two-head, and d16
two-head long-sequence routes now prove attention arithmetic and table membership in
...
issue `#521` adds the first combined d16 two-head point. Issue `#525` adds the
first combined d16 two-head long-sequence point. Issue `#524` pins the fixed
eight-step combined point as an implementation-exact quantized Softmax-table receipt.
...
integer-kernel receipt discipline for the checked d16 width point, and issue
`#524` closes it for the combined d16/two-head point. Exact real-valued exp/div
Softmax, long-context inference, full inference, and recursion/PCD remain open.
...
`NATIVE_STWO_D8_CAUSAL_MASKED_INTEGER_ARGMAX_ATTENTION_KV_SEQUENCE_PROOF_AND_NATIVE_STWO_D8_IMPLEMENTATION_EXACT_QUANTIZED_SOFTMAX_TABLE_RECEIPT_AND_NATIVE_STWO_MULTIHEAD_IMPLEMENTATION_EXACT_QUANTIZED_SOFTMAX_TABLE_RECEIPT_AND_NATIVE_STWO_TWO_HEAD_LONGSEQ_FUSED_SOFTMAX_TABLE_PROOF_AND_NATIVE_STWO_D16_FUSED_SOFTMAX_TABLE_PROOF_AND_NATIVE_STWO_D16_TWO_HEAD_FUSED_SOFTMAX_TABLE_PROOF_AND_NATIVE_STWO_D16_TWO_HEAD_LONGSEQ_FUSED_SOFTMAX_TABLE_PROOF_AND_NATIVE_STWO_D16_IMPLEMENTATION_EXACT_QUANTIZED_SOFTMAX_TABLE_RECEIPT_AND_NATIVE_STWO_D16_TWO_HEAD_IMPLEMENTATION_EXACT_QUANTIZED_SOFTMAX_TABLE_RECEIPT_AND_EXTERNAL_SNARK_RISC0_CONTROLS_NOT_REAL_VALUED_SOFTMAX_NOT_LONG_CONTEXT_OR_FULL_INFERENCE_NOT_RECURSION_OR_PCD_NOT_AGENT_CORRECTNESS`
...
| Local Stwo d16 two-head long-sequence fused bounded Softmax-table attention/KV LogUp proof | GO; one native Stwo proof object checks combined d16/two-head/seq16 attention arithmetic and table membership; `84868` raw proof bytes versus `108158` bytes for the matched source-plus-sidecar pair |
...
| Local Stwo d16 two-head implementation-exact quantized Softmax-table receipt | GO; one d16 two-head fused Stwo proof backs the pinned combined width/head integer table/floor-division kernel; `78211` raw proof bytes, `104` lookup claims, `9` table rows, and `43 / 43` semantic/proof mutations rejected |
...
- d16 two-head quantized Softmax receipt JSON: `docs/engineering/evidence/zkai-attention-kv-d16-two-head-quantized-softmax-receipt-gate-2026-05.json`
- d16 two-head quantized Softmax receipt TSV: `docs/engineering/evidence/zkai-attention-kv-d16-two-head-quantized-softmax-receipt-gate-2026-05.tsv`
...
- Native d16 two-head long-sequence bounded Softmax-table source input: `docs/engineering/evidence/zkai-attention-kv-stwo-native-d16-two-head-longseq-bounded-softmax-table-proof-2026-05.json`
- Native d16 two-head long-sequence bounded Softmax-table source envelope: `docs/engineering/evidence/zkai-attention-kv-stwo-native-d16-two-head-longseq-bounded-softmax-table-proof-2026-05.envelope.json`
- Native d16 two-head long-sequence LogUp sidecar gate: `docs/engineering/evidence/zkai-attention-kv-stwo-native-d16-two-head-longseq-softmax-table-logup-sidecar-gate-2026-05.json`
- Native d16 two-head long-sequence LogUp sidecar proof envelope: `docs/engineering/evidence/zkai-attention-kv-stwo-native-d16-two-head-longseq-softmax-table-logup-sidecar-proof-2026-05.envelope.json`
- Native d16 two-head long-sequence fused bounded Softmax-table gate: `docs/engineering/evidence/zkai-attention-kv-stwo-native-d16-two-head-longseq-fused-softmax-table-gate-2026-05.json`
- Native d16 two-head long-sequence fused bounded Softmax-table proof envelope: `docs/engineering/evidence/zkai-attention-kv-stwo-native-d16-two-head-longseq-fused-softmax-table-proof-2026-05.envelope.json`
...
| d16 two-head quantized Softmax-table proof size | `78211` bytes |
| d16 two-head quantized Softmax-table key/value width | `16` / `16` |
| d16 two-head quantized Softmax-table head count | `2` |
| d16 two-head quantized Softmax-table lookup claims | `104` |
| d16 two-head quantized Softmax-table max observed division residual | `39/40` |
| d16 two-head quantized Softmax-table receipt mutations | `43 / 43` |
...
| Mutations checked | 93 |
| Mutations rejected | 93 |
| Selector commitment | `blake2b-256:309838ad0e3d3ecd8d3c8518f4d49911c7cb0ef6d3948d40e49904dff0f85fa8` |
...
route/removal and receipt drift, all RISC Zero
...
python3 -m unittest \
  ...
  scripts.tests.test_zkai_attention_kv_quantized_softmax_receipt_gate scripts.tests.test_zkai_attention_kv_multihead_quantized_softmax_receipt_gate scripts.tests.test_zkai_attention_kv_d16_quantized_softmax_receipt_gate scripts.tests.test_zkai_attention_kv_d16_two_head_quantized_softmax_receipt_gate \

...
The Rust Phase42 source-bound implementation now tests that criterion against real shared Phase12/28/29/30 artifacts. That hardening found a blocker: the live source stack rejects with a Phase12/Phase14 `kv_history_commitment` mismatch. Phase12 carries a linear history-chain commitment, while Phase14 carries the chunked history accumulator used by Phase23. That means Phase42 has not yet produced an acceptable `hash_preimage_relation` from real sources. The next Issue #180 decision patch must either expose/prove the history-equivalence relation or pivot.

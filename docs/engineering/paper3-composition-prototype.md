...
The current checker verifies the Phase29 input contract commitment, Phase30 envelope commitments and boundary links, and optional Phase41 source-bound recomputation. Its important behavior is negative: a valid Phase41 witness is reported as `decision=patch_required` and `relation_outcome=impossible` for the current artifact surface, because Phase41 binds an explicit boundary pair but does not expose the Phase12 public-state preimage or the Phase14/23 boundary-state preimage needed for a `projection`, `deterministic_transform`, or `hash_preimage_relation` claim.

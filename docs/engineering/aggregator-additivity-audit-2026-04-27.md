...
The "audited identity" column lists the `benchmark_version` string and
the first row's `backend_variant` recorded in each canonical evidence
file (both readable directly from the JSON: `payload["benchmark_version"]`
and `payload["rows"][0]["backend_variant"]`). These are the strings
that a future re-run must match for the evidence to be considered
non-drifted; the underlying Stwo proof-backend version
...
| Aggregator                                                       | Pinned input `timing_mode` / `timing_policy`                          | Output identity contract                                                                                                              | Canonical evidence path                                                                                                | Audited identity (`benchmark_version` + first-row `backend_variant`)                                                                |
| ---------------------------------------------------------------- | --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `aggregate_phase44d_carry_aware_experimental_scaling.py`         | `measured_single_run` / `single_run_from_microsecond_capture`         | Synthesizes `timing_mode = "measured_median"` and `timing_policy = "median_of_{N}_runs_from_microsecond_capture"` for the output.    | Default family: `docs/engineering/evidence/phase44d-carry-aware-experimental-scaling-2026-04.{tsv,json}`. 2x2 family: `docs/engineering/evidence/phase44d-carry-aware-experimental-2x2-scaling-2026-04.{tsv,json}`. Both written by the same parameterized aggregator; the `generate_..._2x2_scaling_benchmark.sh` driver passes a different `TSV_OUT`/`JSON_OUT` to the same `aggregate_phase44d_carry_aware_experimental_scaling.py` invocation. | Default family: `stwo-phase44d-source-emission-experimental-benchmark-v1` / `typed_source_boundary_plus_compact_projection`. 2x2 family: `stwo-phase44d-source-emission-experimental-2x2-layout-benchmark-v1` / `typed_source_boundary_plus_compact_projection`. |
| `aggregate_phase44d_carry_aware_experimental_3x3_scaling.py`     | `measured_single_run` / `single_run_from_microsecond_capture`         | Same as above.                                                                                                                       | `docs/engineering/evidence/phase44d-carry-aware-experimental-3x3-scaling-2026-04.{tsv,json}`.                          | `stwo-phase44d-source-emission-experimental-3x3-layout-benchmark-v1` / `typed_source_boundary_plus_compact_projection`.              |
| `aggregate_phase43_source_root_feasibility.py`                   | `measured_single_run` / `single_run_from_microsecond_capture`         | Synthesizes `timing_mode = "measured_median"` for the output.                                                                        | Publication lane: `docs/engineering/evidence/phase43-source-root-feasibility-publication-2026-04.{tsv,json}`. Experimental lane: `docs/engineering/evidence/phase43-source-root-feasibility-experimental-2026-04.{tsv,json}`. Both written by the same aggregator with different driver-script outputs. | Publication: `stwo-phase43-source-root-feasibility-benchmark-v2` / `emitted_source_boundary_plus_compact_projection`. Experimental: `stwo-phase43-source-root-feasibility-experimental-benchmark-v2` / `emitted_source_boundary_plus_compact_projection`. |
| `aggregate_phase44d_carry_aware_experimental_family_matrix.py`   | inline equality over `timing_mode == "measured_median"` and `timing_unit == "milliseconds"` per input row | Emits one cross-family transferability matrix; no per-run resampling.                                                                | `docs/engineering/evidence/phase44d-carry-aware-experimental-family-matrix-2026-04.{tsv,json}`.                        | `phase44d-carry-aware-experimental-family-matrix-v1` (this aggregator emits one row per family, so there is no first-row `backend_variant` to read out; the per-row `backend_variant` strings are forwarded from the input scaling TSVs).             |

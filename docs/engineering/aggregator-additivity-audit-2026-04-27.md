...
file, the `benchmark_version` string (a single payload-level field,
deterministic, and order-independent) and the **lexicographically
sorted set** of distinct `backend_variant` strings present across
all rows. Each engineering benchmark intentionally exercises
multiple backend variants (e.g. baseline + typed-boundary +
compact-projection-only), and the aggregator preserves the input's
...
recorded in the engineering evidence JSON, so it cannot be validated
post-hoc from the evidence alone. The wrapper script
`generate_tablero_replay_breakdown_optimized_benchmark.sh` additionally
hard-pins the optimized-replay output's `EXPECTED_BENCHMARK_VERSION`
and `EXPECTED_SEMANTIC_SCOPE`, blocking identity drift on its
canonical evidence path.

| Aggregator                                                       | Pinned input `timing_mode` / `timing_policy`                          | Output identity contract                                                                                                              | Canonical evidence path                                                                                                | Audited identity (`benchmark_version` + sorted set of distinct `backend_variant` values)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ---------------------------------------------------------------- | --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `aggregate_phase44d_carry_aware_experimental_scaling.py`         | `measured_single_run` / `single_run_from_microsecond_capture`         | Synthesizes `timing_mode = "measured_median"` and `timing_policy = "median_of_{N}_runs_from_microsecond_capture"` for the output.    | Default family: `docs/engineering/evidence/phase44d-carry-aware-experimental-scaling-2026-04.{tsv,json}`. 2x2 family: `docs/engineering/evidence/phase44d-carry-aware-experimental-2x2-scaling-2026-04.{tsv,json}`. Both written by the same parameterized aggregator; the `generate_..._2x2_scaling_benchmark.sh` driver passes a different `TSV_OUT`/`JSON_OUT` to the same `aggregate_phase44d_carry_aware_experimental_scaling.py` invocation. | Default family: `stwo-phase44d-source-emission-experimental-benchmark-v1` / `{compact_phase43_projection_proof_only, phase30_manifest_plus_compact_projection_baseline, phase30_manifest_replay_only, phase44d_typed_boundary_binding_only, typed_source_boundary_plus_compact_projection}`. 2x2 family: `stwo-phase44d-source-emission-experimental-2x2-layout-benchmark-v1` / same five-variant set as the default family. |
| `aggregate_phase44d_carry_aware_experimental_3x3_scaling.py`     | `measured_single_run` / `single_run_from_microsecond_capture`         | Same as above.                                                                                                                       | `docs/engineering/evidence/phase44d-carry-aware-experimental-3x3-scaling-2026-04.{tsv,json}`.                          | `stwo-phase44d-source-emission-experimental-3x3-layout-benchmark-v1` / `{compact_phase43_projection_proof_only, phase30_manifest_plus_compact_projection_baseline, phase30_manifest_replay_only, phase44d_typed_boundary_binding_only, typed_source_boundary_plus_compact_projection}` (same five-variant set as the other phase44d family scaling sweeps). |
| `aggregate_phase43_source_root_feasibility.py`                   | `measured_single_run` / `single_run_from_microsecond_capture`         | Synthesizes `timing_mode = "measured_median"` for the output.                                                                        | Publication lane: `docs/engineering/evidence/phase43-source-root-feasibility-publication-2026-04.{tsv,json}`. Experimental lane: `docs/engineering/evidence/phase43-source-root-feasibility-experimental-2026-04.{tsv,json}`. Both written by the same aggregator with different driver-script outputs. | Publication: `stwo-phase43-source-root-feasibility-benchmark-v2` / `{compact_phase43_projection_proof_only, derive_source_root_claim_only, emitted_source_boundary_plus_compact_projection, full_trace_plus_phase30_derivation_baseline, source_boundary_binding_only}`. Experimental: `stwo-phase43-source-root-feasibility-experimental-benchmark-v2` / same five-variant set as the publication lane. |
| `aggregate_phase44d_carry_aware_experimental_family_matrix.py`   | inline equality over `timing_mode == "measured_median"` and `timing_unit == "milliseconds"` per input row | Emits one cross-family transferability matrix; no per-run resampling.                                                                | `docs/engineering/evidence/phase44d-carry-aware-experimental-family-matrix-2026-04.{tsv,json}`.                        | `phase44d-carry-aware-experimental-family-matrix-v1` (per-row `backend_variant` is not present in this benchmark's row schema; identity is fully captured by `benchmark_version` plus the per-family sets above, which are forwarded as inputs).                                                                                                                                                                                                                                                                                                              |
| `aggregate_tablero_replay_breakdown.py`                          | `measured_single_run` / `single_run_from_microsecond_capture`         | Synthesizes `timing_mode = "measured_median"`, `timing_aggregation_strategy = "median_total_representative_run"`, and `timing_policy = "median_of_{N}_runs_from_microsecond_capture"` for the output. | `docs/engineering/evidence/tablero-replay-baseline-breakdown-optimized-2026-04.{tsv,json}` (BENCH_RUNS in `{5, 9}`).   | `stwo-tablero-replay-breakdown-optimized-benchmark-v1` (per-row `backend_variant` is not present in this benchmark's row schema; identity is fully captured by `benchmark_version` + the wrapper script's `EXPECTED_SEMANTIC_SCOPE`).                                                                                                                                                                                                                                                                                                                                              |

The wrapper script's exact pinned values are
